-- Test query to verify party contract counts
...
WITH party_contract_counts AS (
  SELECT 
    ...
    COUNT(DISTINCT CASE WHEN c.first_party_id::uuid = p.id THEN c.id END) as first_party_contracts,
    
    -- Count as second party
    COUNT(DISTINCT CASE WHEN c.second_party_id::uuid = p.id THEN c.id END) as second_party_contracts,
    
    -- Total unique contracts (handling duplicates)
    COUNT(DISTINCT 
      CASE 
        WHEN c.employer_id = p.id OR c.client_id = p.id OR 
             c.first_party_id::uuid = p.id OR c.second_party_id::uuid = p.id 
        THEN c.id 
      END
    ) as total_contracts,
  ...
  LEFT JOIN contracts c ON (
    ...
    c.client_id = p.id OR 
    c.first_party_id::uuid = p.id OR 
    c.second_party_id::uuid = p.id
  )
