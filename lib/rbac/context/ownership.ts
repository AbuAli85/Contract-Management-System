...
export class OwnershipEvaluator {
  ...
  private async checkUserOwnership(
    ...
    return {
      ...
      reason: isOwner
        ...
        : 'User does not own this profile',
   ...
   */
  private async checkServiceOwnership(
    context: OwnershipContext
  ): Promise<OwnershipResult> {
  ...
  private async checkBookingOwnership(
    ...
    try {
      ...
      const isOwner =
        booking.client_user_id === user.id || booking.provider_id === user.id;
      return {
        isOwner,
        reason: isOwner
  ...
  private async checkContractOwnership(
    ...
    try {
      ...
      const isOwner =
        contract.user_id === user.id || contract.promoter_id === user.id;
      return {
        isOwner,
        reason: isOwner
          ? 'User owns this contract'
   ...
   */
  private async checkPromoterOwnership(
    context: OwnershipContext
  ): Promise<OwnershipResult> {
    const { user, resourceId } = context;
    ...
    try {
      ...
      return {
        isOwner,
        reason: isOwner
          ? 'User owns this promoter profile'
          : 'User does not own this promoter profile',
        context: {
          promoterId: resourceId,
   ...
   */
  private async checkPartyOwnership(
    context: OwnershipContext
  ): Promise<OwnershipResult> {
    const { user, resourceId } = context;
    ...
    try {
      ...
      return {
        isOwner,
        reason: isOwner
          ? 'User owns this party'
          : 'User does not own this party',
        context: {
          partyId: resourceId,
  ...
  async checkProviderOwnership(
    ...
    try {
      ...
      return {
        isOwner,
        reason: isOwner
          ? 'User in same provider organization'
          : 'User not in same provider organization',
        context: {
          resourceId,
