...
export class DataEncryption {
  ...
  constructor() {
    ...
    if (!process.env.DATA_ENCRYPTION_KEY) {
      console.warn('⚠️ No DATA_ENCRYPTION_KEY found in environment. Using generated key.')
      console.warn('🔐 Generated encryption key:', this.encryptionKey)
      console.warn('📝 Add this to your .env.local: DATA_ENCRYPTION_KEY=' + this.encryptionKey)
    }
  }
...
export class DatabaseSecurity {
  ...
  async encryptSensitiveFields(data: any, fieldsToEncrypt: string[]): Promise<any> {
  ...
  async decryptSensitiveFields(data: any, fieldsToDecrypt: string[]): Promise<any> {
  ...
  async secureInsert(
    ...
    try {
      ...
      const encryptedData = await this.encryptSensitiveFields(data, sensitiveFields)
      ...
      // Create audit trail
      await this.createAuditTrail(tableName, result.id, 'INSERT', null, data, userId)
  ...
  async secureSelect(
    ...
    try {
      ...
      const decryptedData = await Promise.all(
        (data || []).map(item => this.decryptSensitiveFields(item, sensitiveFields))
      )
  ...
  async secureUpdate(
    ...
    try {
      ...
      // Create audit trail
      await this.createAuditTrail(tableName, id, 'UPDATE', oldData, data, userId)
...
export const encryptionUtils = {
  ...
  decryptPersonalData: (encryptedData: string) => dataEncryption.decrypt(encryptedData),
  ...
  hashForSearch: (data: string) => dataEncryption.hashForIndex(data.toLowerCase().trim()),
}
