...
export function validateCreateContractRequest(body: unknown): ValidationResult {
  ...
  const hasPromoterId =
    (body as unknown as { promoter_id?: string; promoter?: { id?: string } })
      .promoter_id ||
    (body as unknown as { promoter_id?: string; promoter?: { id?: string } })
      .promoter?.id;
  ...
  if (!hasSecondPartyId) {
    errors.push({
      field: 'second_party_id',
      message: 'Party B ID is required',
    });
  }
  ...
  const contractStartDate = (
    body as unknown as { contract_start_date?: string }
  ).contract_start_date;
  if (
  ...
  const contractEndDate = (body as unknown as { contract_end_date?: string })
    .contract_end_date;
  if (
    typeof contractEndDate === 'string' &&
    contractEndDate &&
    !isValidDate(contractEndDate)
  ) {
    errors.push({
...
export function validateUpdateContractRequest(body: unknown): ValidationResult {
  ...
  if (
    typeof updateEmail === 'string' &&
    updateEmail &&
    !isValidEmail(updateEmail)
  ) {
    errors.push({ field: 'email', message: 'Invalid email format' });
  }

  // Validate date formats if provided
  const updateContractStartDate = (
    body as unknown as { contract_start_date?: string }
  ).contract_start_date;
  if (
  ...
  const updateContractEndDate = (
    body as unknown as { contract_end_date?: string }
  ).contract_end_date;
  if (
...
export function extractIds(body: unknown): {
  ...
  return {
    clientId:
      (body as any)?.first_party?.id ||
      (body as any)?.first_party?.first_party_id,
    employerId:
      (body as any)?.second_party?.id ||
      (body as any)?.second_party?.second_party_id,
    promoterId:
      (body as any)?.promoter?.id || (body as any)?.promoter?.promoter_id,
  };
}
