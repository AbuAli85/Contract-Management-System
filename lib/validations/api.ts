...
export function validateCreateContractRequest(body: unknown): ValidationResult {
  ...
  const hasFirstPartyId =
    (body as unknown as { first_party_id?: string; first_party?: { id?: string } })
      .first_party_id ||
    (body as unknown as { first_party_id?: string; first_party?: { id?: string } }).first_party?.id
  const hasSecondPartyId =
    (body as unknown as { second_party_id?: string; second_party?: { id?: string } })
      .second_party_id ||
    (body as unknown as { second_party_id?: string; second_party?: { id?: string } }).second_party
      ?.id
  const hasPromoterId =
    (body as unknown as { promoter_id?: string; promoter?: { id?: string } }).promoter_id ||
    (body as unknown as { promoter_id?: string; promoter?: { id?: string } }).promoter?.id
  ...
  const contractStartDate = (body as unknown as { contract_start_date?: string })
    .contract_start_date
  if (
    typeof contractStartDate === "string" &&
    contractStartDate &&
    !isValidDate(contractStartDate)
  ) {
...
export function validateUpdateContractRequest(body: unknown): ValidationResult {
  ...
  // Validate date formats if provided
  const updateContractStartDate = (body as unknown as { contract_start_date?: string })
    .contract_start_date
  if (
    typeof updateContractStartDate === "string" &&
    updateContractStartDate &&
    !isValidDate(updateContractStartDate)
  ) {
    errors.push({ field: "contract_start_date", message: "Invalid start date format" })
  }

  const updateContractEndDate = (body as unknown as { contract_end_date?: string })
    .contract_end_date
  if (
    typeof updateContractEndDate === "string" &&
    updateContractEndDate &&
    !isValidDate(updateContractEndDate)
  ) {
    errors.push({ field: "contract_end_date", message: "Invalid end date format" })
  }
