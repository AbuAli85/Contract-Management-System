...
export const partySchema = z.object({
  ...
  name: z.string().min(1, 'Party name is required').max(255, 'Party name is too long'),
...
export const financialTermsSchema = z.object({
  ...
  paymentSchedule: z.enum(['monthly', 'quarterly', 'annually', 'milestone', 'upon_completion']),
  paymentTerms: z.string().optional(),
  latePaymentPenalty: z.number().min(0, 'Late payment penalty must be positive').optional(),
  advancePayment: z.number().min(0, 'Advance payment must be positive').optional(),
});

// Contract schema
export const contractSchema = z.object({
  id: z.string().uuid('Invalid contract ID').optional(),
  contractNumber: z.string().min(1, 'Contract number is required').max(50, 'Contract number is too long'),
  title: z.string().min(1, 'Contract title is required').max(255, 'Contract title is too long'),
  ...
  // Contract details
  type: z.enum(['employment', 'service', 'partnership', 'vendor', 'consulting', 'other']),
...
export const contractSearchSchema = z.object({
  ...
  type: z.enum(['employment', 'service', 'partnership', 'vendor', 'consulting', 'other']).optional(),
...
export const contractClauseSchema = z.object({
  ...
  title: z.string().min(1, 'Clause title is required').max(255, 'Clause title is too long'),
  content: z.string().min(1, 'Clause content is required'),
  type: z.enum(['standard', 'custom', 'regulatory', 'optional']),
  category: z.enum(['payment', 'termination', 'liability', 'confidentiality', 'intellectual_property', 'other']),
...
export const contractAmendmentSchema = z.object({
  ...
  description: z.string().min(1, 'Amendment description is required').max(1000, 'Description is too long'),
