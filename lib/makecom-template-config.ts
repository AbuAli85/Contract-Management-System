...
export const MAKECOM_TEMPLATE_CONFIGS: Record<string, MakecomTemplateConfig> = {
  ...
  full_time_permanent_employment_v2: {
    ...
    description:
      'Automated employment contract generation with legal compliance checks',
    category: 'employment',
    isActive: true,
    makecomModuleConfig: {
  ...
  fixed_term_contract_v2: {
    ...
    description:
      'Automated fixed-term contract generation for project-based employment',
    category: 'employment',
    isActive: true,
    makecomModuleConfig: {
      webhookUrl: process.env.MAKE_WEBHOOK_URL || '',
  ...
  business_service_contract_v2: {
    ...
    description:
      'Automated B2B service contract generation with comprehensive terms',
    category: 'service',
    isActive: true,
    makecomModuleConfig: {
      webhookUrl: process.env.MAKE_WEBHOOK_URL || '',
  ...
  consulting_agreement_v2: {
    ...
    description:
      'Automated consulting agreement generation with expertise validation',
    category: 'consulting',
    isActive: true,
    makecomModuleConfig: {
      webhookUrl: process.env.MAKE_WEBHOOK_URL || '',
      ...
      googleDriveSettings: {
        ...
        naming:
          'Consulting_Agreement_{contract_number}_{consultant_name}_{date}',
        permissions: ['view', 'edit', 'comment'],
      },
      outputFormat: 'pdf',
      automationSteps: [
  ...
  freelance_service_agreement_v2: {
    ...
    description:
      'Automated freelance agreement generation for independent contractors',
    category: 'freelance',
    isActive: true,
    makecomModuleConfig: {
      webhookUrl: process.env.MAKE_WEBHOOK_URL || '',
      ...
      googleDriveSettings: {
        ...
        naming:
          'Freelance_Agreement_{contract_number}_{freelancer_name}_{date}',
        permissions: ['view', 'edit'],
      },
      outputFormat: 'pdf',
      automationSteps: [
  ...
  business_partnership_agreement_v2: {
    ...
    description:
      'Automated partnership agreement generation with governance structure',
    ...
    makecomModuleConfig: {
      ...
      googleDriveSettings: {
        ...
        naming:
          'Partnership_Agreement_{contract_number}_{partnership_name}_{date}',
        permissions: ['view', 'edit', 'comment'],
      },
      outputFormat: 'pdf',
      automationSteps: [
...
// Helper functions
export function getMakecomTemplateConfig(
  contractTypeId: string
): MakecomTemplateConfig | null {
...
export function getMakecomTemplatesByCategory(
  category: string
): MakecomTemplateConfig[] {
  return Object.values(MAKECOM_TEMPLATE_CONFIGS).filter(
...
export function generateMakecomWebhookPayload(
  ...
  const payload: any = {
    ...
    automation_settings: {
      ...
      requires_financial_approval:
        config.businessRules.requiresFinancialApproval,
    },
  ...
  if (config.makecomModuleConfig.googleDriveSettings) {
    payload.google_drive_folder_id =
      config.makecomModuleConfig.googleDriveSettings.folderId;
    payload.file_naming_pattern =
      config.makecomModuleConfig.googleDriveSettings.naming;
  }
...
export function validateMakecomTemplateData(
  ...
  if (!config) {
    return {
      isValid: false,
      errors: [
        `Make.com template configuration for '${contractTypeId}' not found`,
      ],
      warnings: [],
    };
  }
  ...
  config.makecomModuleConfig.webhookTriggerFields.forEach(field => {
    if (!contractData[field]) {
      errors.push(
        `Required field '${field}' is missing for Make.com automation`
      );
    }
  });
