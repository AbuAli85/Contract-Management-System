...
export async function generateProfessionalContractPDF(
  ...
  doc.text(`Contract Type: ${data.contract_type}`, pageWidth / 2, 27, { align: 'center' });
  ...
  doc.text(`Date: ${new Date().toLocaleDateString()}`, pageWidth - 98, yPos + 8);
  ...
  const employeeDetails = [
    ...
    data.employee_nationality ? `Nationality: ${data.employee_nationality}` : null,
  ...
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(0, 0, 0);
  doc.text(new Date(data.start_date).toLocaleDateString('en-GB', {
    day: '2-digit',
    month: 'long',
    year: 'numeric'
  }), 100, yPos + 8);
  doc.text(new Date(data.end_date).toLocaleDateString('en-GB', {
    day: '2-digit',
    month: 'long',
    year: 'numeric'
  }), 100, yPos + 18);
  ...
  const allowancesStr = data.allowances ? `${data.allowances.toLocaleString()} ${data.currency}` : 'N/A';
  ...
  if (data.special_terms) {
    ...
    const specialTermsLines = doc.splitTextToSize(data.special_terms, pageWidth - 50);
