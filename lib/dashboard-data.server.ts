...
> {
  ...
  try {
    ...
    const failedContracts =
      contracts?.filter(c => c.status === 'failed').length || 0;
...
export async function getPendingReviewsSrv(): Promise<
...
export async function getAdminActionsSrv(): Promise<
...
export async function getAuditLogsSrv(): Promise<
  ServerActionResponse<AuditLog[]>
> {
  const supabase = await createSupabaseClient();
  try {
    const { data, error } = await supabase
...
export async function getNotificationsSrv(): Promise<
...
> {
  ...
  try {
    ...
    const notifications: Notification[] = (data || []).map(n => ({
      ...
      type:
        (n.type as 'success' | 'error' | 'warning' | 'info' | 'default') ||
        'info',
      user_id: n.user_id || '',
