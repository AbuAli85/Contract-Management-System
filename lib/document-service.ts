...
export interface CompanyDocument {
  id: string
  company_id: string
  document_type: 'commercial_registration' | 'business_license' | 'tax_certificate' | 'insurance' | 'logo' | 'other'
  file_name: string
  file_url: string
  file_size: number
  mime_type: string
...
export type DocumentExpiryStatus = 'expired' | 'expiring_soon' | 'expiring' | 'valid' | 'no_expiry'
...
export class DocumentService {
  ...
  static async uploadDocument(
    ...
    try {
      ...
      onProgress?.({ progress: 60, status: 'processing', message: 'Processing file...' })
      ...
      onProgress?.({ progress: 80, status: 'processing', message: 'Saving document record...' })
   ...
   */
  static async getDocumentsByCompany(companyId: string): Promise<DocumentServiceResponse> {
    try {
      ...
      const documentsWithMetadata = data?.map(doc => ({
        ...doc,
   ...
   */
  static async getExpiryStatistics(companyId?: string): Promise<{ data?: ExpiryStatistics; error?: string }> {
    try {
      let query = supabase
        .from('company_documents')
        .select('expiry_date')
      ...
      return { error: error instanceof Error ? error.message : 'Failed to get expiry statistics' }
    }
  }
   ...
   */
  static async sendExpiryNotification(documentId: string): Promise<DocumentServiceResponse> {
    try {
