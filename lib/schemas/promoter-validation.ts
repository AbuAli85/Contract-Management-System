...
const createPhoneSchema = (customErrorMessage?: string) => {
  return z.string()
    ...
    .regex(
      ...
      customErrorMessage || "Please enter a valid phone number (10-15 digits, optionally starting with +)"
    )
...
const createNationalitySchema = (customErrorMessage?: string) => {
  return z.string()
    ...
    .regex(/^[a-zA-Z\s\-']+$/, customErrorMessage || "Nationality can only contain letters, spaces, hyphens, and apostrophes")
}
...
const createNameSchema = (fieldName: string, customErrorMessage?: string) => {
  return z.string()
    ...
    .regex(/^[a-zA-Z\s\-']+$/, customErrorMessage || `${fieldName} can only contain letters, spaces, hyphens, and apostrophes`)
...
export const promoterProfileSchema = z.object({
  ...
  existing_id_card_url: z.string()
    .url("Invalid ID card image URL")
    .optional()
    .nullable(),
  existing_passport_url: z.string()
    .url("Invalid passport image URL")
    .optional()
    .nullable(),
...
export const sanitizePromoterData = (data: any): Partial<PromoterProfileFormData> => {
  ...
  const stringFields = ['firstName', 'lastName', 'nationality', 'email', 'mobile_number', 
    'name_en', 'name_ar', 'id_card_number', 'job_title', 'department', 'work_location',
    'notes', 'passport_number']
  ...
  // Handle other fields
  const otherFields = ['employer_id', 'outsourced_to_id', 'status', 'existing_id_card_url', 
    'existing_passport_url', 'profile_picture_url', 'id_card_image', 'passport_image']
