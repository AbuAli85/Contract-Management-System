...
export const contactSchema = z.object({
  ...
  name_en: z.string()
    ...
    .regex(/^[a-zA-Z\s\-'\.]+$/, "English name can only contain letters, spaces, hyphens, apostrophes, and periods"),
  name_ar: z.string()
  ...
  email: z.string()
    ...
    .or(z.literal("")),
  phone: z.string()
    .regex(/^(\+?[1-9]\d{1,14}|[0-9]{10,15})?$/, "Please enter a valid phone number (10-15 digits, optionally starting with +)")
    .transform(val => val?.replace(/\s+/g, '') || "")
    .optional()
    .or(z.literal("")),
  mobile: z.string()
    .regex(/^(\+?[1-9]\d{1,14}|[0-9]{10,15})?$/, "Please enter a valid mobile number (10-15 digits, optionally starting with +)")
    .transform(val => val?.replace(/\s+/g, '') || "")
    .optional()
    .or(z.literal("")),
  position: z.string()
...
export function transformPartyForForm(party: Party): PartyFormData {
  return {
    ...
    license_expiry_date: party.license_expiry_date ? new Date(party.license_expiry_date) : undefined
  }
}
