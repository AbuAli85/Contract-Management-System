...
export class PerformanceTestSuite {
  ...
  private async testDashboardOptimizations() {
    ...
    await this.runTest('Dashboard Analytics Caching', async () => {
      ...
      const response1 = await fetch(
        '/api/dashboard/analytics/paginated?timeRange=30d'
      );
  ...
  private async testContractOptimizations() {
    ...
    await this.runTest('Background Worker Health', async () => {
      const { backgroundContractWorker } = await import(
        '@/lib/workers/background-contract-worker'
      );
  ...
  private async runTest(testName: string, testFunction: () => Promise<any>) {
    ...
    try {
      ...
      console.log(
        `  ❌ Failed: ${testName} - ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private generateReport() {
    ...
    // Dashboard metrics
    const dashboardTests = this.results.filter(r =>
    ...
    if (dashboardTests.length > 0) {
      ...
      dashboardTests.forEach(test => {
        console.log(
          `    ${test.test}: ${test.status === 'passed' ? '✅' : '❌'} (${test.duration}ms)`
        );
    ...
    const authTests = this.results.filter(
      r => r.test.includes('Session') || r.test.includes('Permission')
    );
    ...
    // Contract metrics
    const contractTests = this.results.filter(
      r =>
        r.test.includes('Contract') ||
        r.test.includes('PDF') ||
        r.test.includes('Worker')
    );
    ...
    console.log('\n🚀 Optimization Status:');
    console.log(
      `  Dashboard: ${dashboardTests.filter(t => t.status === 'passed').length}/${dashboardTests.length} optimizations working`
    );
    console.log(
      `  Authentication: ${authTests.filter(t => t.status === 'passed').length}/${authTests.length} optimizations working`
    );
    console.log(
      `  Contracts: ${contractTests.filter(t => t.status === 'passed').length}/${contractTests.length} optimizations working`
    );
  ...
  private monitorUserInteractions() {
    if ('PerformanceObserver' in window) {
      const interactionObserver = new PerformanceObserver(list => {
        for (const entry of list.getEntries()) {
          console.log(
            `🖱️ User Interaction: ${entry.name} - ${entry.duration.toFixed(2)}ms`
          );
        }
      });
