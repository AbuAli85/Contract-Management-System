...
export class BookingSubscriptionManager {
   ...
   */
  subscribeToBookingEvents(bookingId: string, onEvent: BookingEventCallback): () => void {
   ...
   */
  subscribeToUserBookingEvents(userId: string, onEvent: BookingEventCallback): () => void {
   ...
   */
  subscribeToProviderBookingEvents(providerId: string, onEvent: BookingEventCallback): () => void {
    ...
    console.log(`🔄 Subscribing to provider booking events for provider: ${providerId}`)
    ...
    const channel = this.supabase
      ...
      .subscribe((status) => {
        console.log(`📡 Provider subscription status for ${providerId}:`, status)
      })
...
export function subscribeToBookingEvents(bookingId: string, onEvent: BookingEventCallback): () => void {
  return bookingSubscriptionManager.subscribeToBookingEvents(bookingId, onEvent)
}

export function subscribeToUserBookingEvents(userId: string, onEvent: BookingEventCallback): () => void {
  return bookingSubscriptionManager.subscribeToUserBookingEvents(userId, onEvent)
}

export function subscribeToProviderBookingEvents(providerId: string, onEvent: BookingEventCallback): () => void {
  return bookingSubscriptionManager.subscribeToProviderBookingEvents(providerId, onEvent)
}
