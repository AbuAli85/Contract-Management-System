...
export class AuthService {
  ...
  private async syncUserProfile() {
    ...
    try {
      ...
      console.log(
        '[AuthService] Setting isProfileSynced to true despite error to allow app to function'
      );
      ...
      const { data: authListener } = supabase.auth.onAuthStateChange(
        async (event: string, session: Session | null) => {
          ...
          if (event === 'SIGNED_IN') {
            ...
            console.log(
              '[AuthService] Token refreshed, checking if profile sync needed...'
            );
            ...
            console.log(
              '[AuthService] User signed out, resetting profile sync state'
            );
...
export function useAuth() {
  ...
  const signIn = async (email: string, password: string) => {
    if (!supabase || emergencyMode) {
      console.error(
        '🔐 Auth Service: Emergency mode active or no Supabase client'
      );
      return {
        success: false,
        ...
        try {
          ...
          if (!userError && userData) {
            ...
            if (!profileError && profileData) {
              console.log(
                '🔐 Auth Service: Profile status check:',
                profileData.status
              );
          ...
          console.error(
            '🔐 Auth Service: Error checking user status:',
            statusError
          );
