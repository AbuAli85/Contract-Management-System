...
interface UseInfiniteScrollOptions<T> {
  initialData?: T[]
  pageSize?: number
  fetchNextPage: (page: number, pageSize: number) => Promise<{
    data: T[]
    hasMore: boolean
    totalCount?: number
  }>
...
export function useInfiniteContracts({
  ...
  const fetchContractsPage = useCallback(async (page: number, pageSize: number) => {
...
export function useInfiniteDashboardData({
  ...
  const fetchDashboardPage = useCallback(async (page: number, pageSize: number) => {
