...
export class ReportService {
  ...
  static async generateDocumentComplianceReport(
    ...
    try {
      ...
      for (const company of companies || []) {
        ...
        const missingDocs = requiredDocuments.filter(req => !submittedDocs.includes(req))
        const expiredDocs = companyDocs?.filter(d => 
      ...
      complianceData.summary.compliance_rate = 
        (complianceData.summary.compliant_companies / complianceData.summary.total_companies) * 100

      // Calculate by document type statistics
      complianceData.by_document_type = await this.calculateDocumentTypeStats(requiredDocuments)

      return { data: complianceData }
    } catch (error) {
      console.error('Error generating compliance report:', error)
      return { error: error instanceof Error ? error.message : 'Failed to generate compliance report' }
    }
  }
    ...
    try {
      ...
      const reportData: ExpiryAnalysisData = {
        ...
        by_document_type: Array.from(documentTypeStats.values()).map(stat => ({
          ...stat,
          average_validity_period: stat.total_validity_days / stat.total_documents || 0
        })),
        timeline
      }
      ...
      return { error: error instanceof Error ? error.message : 'Failed to generate expiry analysis report' }
    }
  }
  ...
  static async generateActivitySummaryReport(
    ...
    try {
      const startDate = filters.dateRange?.startDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)
      const endDate = filters.dateRange?.endDate || new Date()

      // This would typically come from an audit log table
      // For now, we'll simulate activity data
      const activities = await this.getActivitiesInPeriod(startDate, endDate, filters)

      const summary = {
        total_activities: activities.length,
        new_companies: activities.filter(a => a.type === 'company_created').length,
        new_documents: activities.filter(a => a.type === 'document_uploaded').length,
        document_updates: activities.filter(a => a.type === 'document_updated').length,
        contract_activities: activities.filter(a => a.type.includes('contract')).length
      }

      // Generate daily activity trends
      const dailyActivities = this.generateDailyActivityTrends(activities, startDate, endDate)
      ...
      const topCompanies = Array.from(companyActivityMap.entries())
        .map(([company_name, activity_count]) => ({ company_name, activity_count }))
      ...
      return { error: error instanceof Error ? error.message : 'Failed to generate activity summary report' }
    }
  }
   ...
   */
  static async saveReportConfig(config: ReportConfig): Promise<{ data?: ReportConfig; error?: string }> {
    try {
      ...
      return { error: error instanceof Error ? error.message : 'Failed to save report configuration' }
    }
  }
   ...
   */
  static async getReportConfigs(userId?: string): Promise<{ data?: ReportConfig[]; error?: string }> {
    try {
      let query = supabase
        .from('report_configs')
        .select('*')
      ...
      return { error: error instanceof Error ? error.message : 'Failed to fetch report configurations' }
    }
  }
  ...
  private static calculateDocumentStats(documents: any[]) {
    ...
    stats.compliance_score = documents.length > 0 ? 
      ((stats.active / documents.length) * 100) : 0
  ...
  private static async exportToJSON(data: any, name: string): Promise<ReportGenerationResult> {
  ...
  private static async exportToCSV(data: any, name: string): Promise<ReportGenerationResult> {
  ...
  private static async exportToExcel(data: any, name: string): Promise<ReportGenerationResult> {
  ...
  private static async exportToPDF(data: any, name: string): Promise<ReportGenerationResult> {
