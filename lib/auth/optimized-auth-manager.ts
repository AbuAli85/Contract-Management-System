...
class OptimizedAuthManager {
  ...
  private scheduleSessionRefresh(session: Session) {
    ...
    const refreshTime = expiresAt - (this.config.sessionRefreshBuffer * 60 * 1000)
  ...
  async getCurrentSession(): Promise<Session | null> {
    if (this.sessionCache?.session && this.sessionCache.expiresAt > Date.now()) {
  ...
  getCachedPermissions(): string[] {
    if (this.sessionCache && 
        (Date.now() - this.sessionCache.lastRefresh) < this.config.permissionCacheDuration) {
  ...
  getSessionHealth(): {
    ...
    const needsRefresh = expiresIn < (this.config.sessionRefreshBuffer * 60 * 1000)
