  ...
  return dataRows.map(line => {
    ...
    headers.forEach((header, index) => {
      ...
      if (
        ...
        (entry as Record<string, any>)[header] = value
          ? parseInt(value, 10)
          : null;
      } else {
...
export function analyzePromoterData(promoters: PromoterCSVRow[]) {
  ...
  promoters.forEach(promoter => {
    ...
    if (!hasIdCard && !hasPassport)
      analysis.byDocumentStatus.missingDocuments++;

    // Notification settings analysis
    if (promoter.notify_days_before_id_expiry) {
      analysis.notificationSettings.idExpiry[
        promoter.notify_days_before_id_expiry
      ] =
        (analysis.notificationSettings.idExpiry[
          promoter.notify_days_before_id_expiry
 ...
 * Download CSV file
 */
export function downloadPromoterCSV(
  promoters: Promoter[],
...
export function validatePromoterData(promoter: PromoterCSVRow): {
  ...
  if (
    promoter.status &&
    !['active', 'inactive', 'suspended'].includes(promoter.status)
  ) {
