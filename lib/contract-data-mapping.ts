...
export interface TemplatePlaceholder {
  ...
  defaultValue?: string
  validation?: string
  sourceField?: keyof ContractGeneratorFormData
 ...
 */
export function getTemplatePlaceholders(contractTypeId: string): TemplatePlaceholder[] {
...
export function validateTemplatePlaceholders(
  ...
  placeholders.forEach(placeholder => {
    ...
    if (!templateContent.includes(placeholderPattern)) {
      if (placeholder.required) {
        errors.push(`Required placeholder '${placeholder.key}' not found in template`)
      } else {
        warnings.push(`Optional placeholder '${placeholder.key}' not found in template`)
      }
    }
  })
  ...
  while ((match = templatePlaceholderRegex.exec(templateContent)) !== null) {
    const placeholderKey = match[1].trim()
    if (!knownPlaceholders.has(placeholderKey)) {
      warnings.push(`Unknown placeholder '${placeholderKey}' found in template`)
    }
  }
