...
export async function generateContractPDF(
  ...
  if (contractData.first_party) {
    ...
    yPosition += 5;
    doc.text(
      `Address: ${contractData.first_party.address_en || 'N/A'}`,
      25,
      yPosition
    );
    yPosition += 5;
    doc.text(
      `Contact Person: ${contractData.first_party.contact_person || 'N/A'}`,
      25,
      yPosition
    );
    yPosition += 5;
    doc.text(
      `Contact Email: ${contractData.first_party.contact_email || 'N/A'}`,
      25,
      yPosition
    );
    yPosition += 5;
    doc.text(
      `Contact Phone: ${contractData.first_party.contact_phone || 'N/A'}`,
      25,
      yPosition
    );
    yPosition += 8;
  ...
  if (contractData.second_party) {
    ...
    yPosition += 5;
    doc.text(
      `Contact Person: ${contractData.second_party.contact_person || 'N/A'}`,
      25,
      yPosition
    );
    yPosition += 5;
    doc.text(
      `Contact Email: ${contractData.second_party.contact_email || 'N/A'}`,
      25,
      yPosition
    );
    yPosition += 5;
    doc.text(
      `Contact Phone: ${contractData.second_party.contact_phone || 'N/A'}`,
      25,
      yPosition
    );
    yPosition += 8;
  ...
  yPosition += 5;
  doc.text(
    `Work Location: ${contractData.work_location || 'N/A'}`,
    25,
    yPosition
  );
  yPosition += 8;
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(
    `Contract Type: ${contractData.contract_type || 'N/A'}`,
    25,
    yPosition
  );
  yPosition += 5;
  doc.text(
    `Start Date: ${contractData.contract_start_date ? new Date(contractData.contract_start_date).toLocaleDateString() : 'N/A'}`,
    25,
    yPosition
  );
  yPosition += 5;
  doc.text(
    `End Date: ${contractData.contract_end_date ? new Date(contractData.contract_end_date).toLocaleDateString() : 'N/A'}`,
    25,
    yPosition
  );
  yPosition += 8;
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(
    `Basic Salary: ${contractData.basic_salary || 'N/A'} ${contractData.currency || 'OMR'}`,
    25,
    yPosition
  );
  yPosition += 5;
  doc.text(
    `Allowances: ${contractData.allowances || 'N/A'} ${contractData.currency || 'OMR'}`,
    25,
    yPosition
  );
  yPosition += 8;
  ...
  if (contractData.promoter) {
    ...
    yPosition += 5;
    doc.text(
      `Contact: ${contractData.promoter.mobile_number || 'N/A'}`,
      25,
      yPosition
    );
    yPosition += 5;
  ...
  yPosition += 5;
  doc.text(
    `Approval Status: ${contractData.approval_status || 'Pending'}`,
    25,
    yPosition
  );
  yPosition += 5;
  doc.text(
    `Created: ${contractData.created_at ? new Date(contractData.created_at).toLocaleDateString() : 'N/A'}`,
    25,
    yPosition
  );
  yPosition += 5;
  doc.text(
    `Last Updated: ${contractData.updated_at ? new Date(contractData.updated_at).toLocaleDateString() : 'N/A'}`,
    25,
    yPosition
  );
  yPosition += 8;
  ...
  yPosition += 10;
  doc.text(
    `Promoter Signature: ${contractData.promoter ? '_________________________' : 'N/A'}    Date: ${contractData.promoter ? '_________________' : 'N/A'}`,
    25,
    yPosition
  );
