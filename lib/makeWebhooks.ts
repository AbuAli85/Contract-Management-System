...
export async function dispatchWebhook(type: WebhookType, payload: any): Promise<{ success: boolean; error?: string }> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      console.log(`🔗 Sending ${type} webhook (attempt ${attempt}/${maxAttempts}):`, payload)

      const webhookSecret = process.env.WEBHOOK_SECRET
      const signature = webhookSecret ? 
        Buffer.from(JSON.stringify(payload) + webhookSecret).toString('base64') : 
        undefined
      ...
      if (response.ok) {
        ...
        console.error(`❌ ${type} webhook failed (attempt ${attempt}):`, lastError)
      }
    } catch (error) {
