...
export const canAccessInvoice = (invoice: any, userId: string, userRole: string) => {
  ...
  if (userRole === 'client' && invoice.booking?.client_id === userId) return true;
  
  return false;
};

// Check if user can modify invoice
export const canModifyInvoice = (invoice: any, userId: string, userRole: string) => {
