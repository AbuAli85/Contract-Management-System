...
export class BookingService {
  ...
  async createBooking(booking: BookingInsert): Promise<{ data: Booking | null; error: string | null }> {
    try {
  ...
  async getBookings(filters: BookingFilters = {}): Promise<{ data: BookingDetails[]; error: string | null }> {
    try {
  ...
  // Update booking
  async updateBooking(id: string, updates: BookingUpdate): Promise<{ data: Booking | null; error: string | null }> {
    try {
      // If updating time, check for conflicts
      if (updates.start_time || updates.end_time) {
        const currentBooking = await this.getBookingById(id)
        if (currentBooking.data) {
          const startTime = updates.start_time || currentBooking.data.start_time
          const endTime = updates.end_time || currentBooking.data.end_time
          const resourceId = updates.resource_id || currentBooking.data.resource_id
  ...
  // Cancel booking
  async cancelBooking(id: string, reason?: string): Promise<{ success: boolean; error: string | null }> {
    try {
      const { data, error } = await this.supabase
        .from('bookings')
        .update({
    ...
    try {
      ...
      if (conflicts.length === 0) {
        ...
        if (!nextAvailableSlot && conflicts.length > 0) {
          const lastBookingEnd = new Date(conflicts[conflicts.length - 1].end_time)
          const endOfDayTime = new Date(endOfDay)
          if (endOfDayTime.getTime() - lastBookingEnd.getTime() >= 30 * 60 * 1000) {
  ...
  async getBookingStatistics(
    ...
    try {
      ...
      const utilizationRate = totalBookings > 0 ? (activeBookings + completedBookings) / totalBookings * 100 : 0
      ...
      const popularResources = Object.entries(resourceCounts)
        .map(([resourceId, count]) => {
          ...
          return {
            resource_id: resourceId,
            resource_name: (booking as any)?.booking_resources?.name || 'Unknown',
            booking_count: count
          }
        })
  ...
  // Helper methods
  private async getBookingById(id: string): Promise<{ data: Booking | null; error: string | null }> {
    try {
      const { data, error } = await this.supabase
        .from('bookings')
        .select('*')
  ...
  private async getBookingAttendees(bookingId: string): Promise<{ data: string[]; error: string | null }> {
    try {
      const { data, error } = await this.supabase
  ...
  private async sendBookingNotification(bookingId: string, action: 'created' | 'updated' | 'cancelled'): Promise<void> {
    try {
  ...
  async createRecurringBooking(
    ...
    while (currentDate <= endDate) {
      ...
      if (!excludeDates.includes(dateStr)) {
        ...
        bookingStart.setHours(originalStart.getHours(), originalStart.getMinutes())
