...
export class SimpleNotificationService {
  ...
  // Create notification
  async createNotification(notification: Omit<SimpleNotification, 'id' | 'created_at' | 'updated_at'>): Promise<{ success: boolean; error: string | null }> {
    try {
  ...
  async getUserNotifications(userId: string, limit?: number): Promise<{ data: SimpleNotification[]; error: string | null }> {
    try {
  ...
  async sendContractNotification(
    ...
    try {
      ...
      const message = customMessage || this.getContractNotificationMessage(type)
  ...
  async sendDocumentExpiryNotification(
    ...
    try {
      ...
      const priority = daysUntilExpiry <= 0 ? 'urgent' : 
  ...
  // Helper methods
  private getContractNotificationTitle(type: string, contractId: string): string {
    switch (type) {
      case 'created':
