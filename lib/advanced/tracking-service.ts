...
export class TrackingService {
  ...
  async createTrackingEvent(event: TrackingEventInsert): Promise<{ data: TrackingEvent | null; error: string | null }> {
    try {
  ...
  async getTrackingEvents(filters: TrackingFilters = {}): Promise<{ data: TrackingDetails[]; error: string | null }> {
    try {
  ...
  async getTrackingStatistics(
    ...
    try {
      ...
      const eventTypes = Object.entries(eventTypeCounts).map(([eventType, count]) => ({
      ...
      if (completedEvents.length > 0) {
        ...
        averageCompletionTime = totalTime / completedEvents.length / (1000 * 60 * 60 * 24) // Convert to days
      }
  ...
  async getDeliveryTracking(trackingNumber: string): Promise<{ data: DeliveryTracking | null; error: string | null }> {
    try {
      const { data, error } = await this.supabase
  ...
  private async updateContractTrackingSummary(contractId: string): Promise<void> {
    try {
