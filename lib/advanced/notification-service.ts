...
export class NotificationService {
  ...
  // Create notification
  async createNotification(notification: NotificationInsert): Promise<{ data: NotificationData | null; error: string | null }> {
    try {
  ...
  async getNotifications(filters: NotificationFilters = {}): Promise<{ data: NotificationDetails[]; error: string | null }> {
    try {
  ...
  async getNotificationStats(userId?: string): Promise<{ data: NotificationStats | null; error: string | null }> {
    try {
      let query = this.supabase
        .from('notifications')
        .select('*')
      ...
      const notificationTypes = Object.entries(typeCounts).map(([type, counts]) => ({
        type,
        count: counts.count,
      ...
      const activityByDate = recentNotifications.reduce((acc, notification) => {
  ...
  async sendContractNotification(
    ...
    try {
      ...
      const notifications = recipientIds.map(userId => ({
        ...
        message: customMessage || this.getContractNotificationMessage(type, contract),
        priority: type === 'expired' ? 'urgent' as const : 'medium' as const,
  ...
  async sendDocumentExpiryNotification(
    ...
    try {
      ...
      const notifications = recipientIds.map(userId => ({
        ...
        priority: daysUntilExpiry <= 0 ? 'urgent' as const : 
                 daysUntilExpiry <= 7 ? 'high' as const : 'medium' as const,
      ...
      return { success: false, error: 'Failed to send document expiry notification' }
    }
  }
  ...
  private async sendSMSNotification(notification: Notification): Promise<void> {
    try {
      ...
      if (!user?.phone_number) {
        console.warn('User phone number not found for notification:', notification.id)
        return
      }
