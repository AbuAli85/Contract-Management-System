...
export async function sendToMakeWebhookWithRetry(
  ...
  for (let attempt = 1; attempt <= webhookConfig.retryAttempts; attempt++) {
    console.log(`🔄 Make.com webhook attempt ${attempt}/${webhookConfig.retryAttempts}`)
    
    const result = await sendToMakeWebhook(eventPayload, additionalData, webhookConfig)
  ...
  console.error(`❌ Make.com webhook failed after ${webhookConfig.retryAttempts} attempts:`, lastError)
  return { 
    success: false, 
    error: lastError,
...
export function validateMakeWebhookConfig(): {
  ...
  if (!process.env.MAKE_WEBHOOK_URL) {
    warnings.push('MAKE_WEBHOOK_URL not configured - webhook integration disabled')
  } else {
    try {
  ...
  if (!process.env.MAKE_WEBHOOK_SECRET) {
    warnings.push('MAKE_WEBHOOK_SECRET not configured - webhook security reduced')
  }
 ...
 */
export function updateWebhookStats(success: boolean, responseTime: number, error?: string) {
  ...
  webhookStats.averageResponseTime = 
    (webhookStats.averageResponseTime * (webhookStats.totalSent - 1) + responseTime) / webhookStats.totalSent
}
...
export async function batchSendToMakeWebhook(
  ...
  for (let i = 0; i < events.length; i += batchSize) {
    ...
    console.log(`📦 Processing webhook batch ${Math.floor(i / batchSize) + 1} (${batch.length} events)`)
