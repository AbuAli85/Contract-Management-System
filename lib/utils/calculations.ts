 ...
 */
export function determineGrowthTrend(change: number): 'up' | 'down' | 'neutral' {
 ...
 */
export function calculateComplianceRate(compliantCount: number, totalCount: number): number {
 ...
 */
export function calculateUtilizationRate(activeCount: number, totalCount: number): number {
  if (totalCount === 0 || !totalCount || isNaN(totalCount)) {
    return 0;
  }
  ...
  if (activeCount > totalCount) {
    console.warn(`Data inconsistency: ${activeCount} active workers > ${totalCount} total workforce`);
    return 100;
  }
 ...
 */
export function formatComplianceDisplay(compliantCount: number, totalCount: number): string {
  if (totalCount === 0 || !totalCount || isNaN(totalCount)) {
