...
export function formatDate(dateInput: string | Date, format: 'short' | 'medium' | 'long' = 'medium'): string {
...
export function formatContractStatus(status: string): {
  ...
  const statusMap: Record<string, { label: string; variant: any; color: string }> = {
    ...
    'pending_approval': { label: 'Pending Approval', variant: 'secondary', color: 'yellow' },
