  ...
  const mockClient = {
    auth: {
      ...
      signInWithPassword: async () => ({
        data: null,
        error: {
          message: 'Server-side auth not available in mock mode',
          code: 'SERVER_MOCK_MODE',
        },
      }),
      ...
      signInWithOAuth: async () => ({
        data: null,
        error: {
          message: 'OAuth not available in server mock mode',
...
export async function createClient() {
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      console.log(
        '🔧 Server: Missing environment variables - using mock server client'
      );
      console.log(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error) {
        lastError = error;
        console.error(
          `🔄 Server: Token refresh attempt ${attempt} failed:`,
          error
        );
      ...
      lastError = error;
      console.error(
        `🔄 Server: Token refresh attempt ${attempt} threw error:`,
        error
      );
