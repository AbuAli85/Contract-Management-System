...
export function middleware(request: NextRequest) {
  ...
  if (
    ...
    pathname.includes("/.well-known/") || // Chrome DevTools and other well-known requests
    ...
    if (
      ...
      // Skip rate limiting for Chrome DevTools requests to prevent infinite loops
    ...
    // Log important requests for debugging (skip repetitive auth checks and Chrome DevTools)
    if (process.env.NODE_ENV === "development" && 
        !pathname.includes("/api/auth/check-session") && 
      ...
      // Log auth checks very infrequently to reduce noise
      const shouldLog = Math.random() < 0.01 // Only log 1% of auth checks
      if (shouldLog) {
        console.log(`🔧 Middleware: ${request.method} ${fullPath} - ${requestId} (AUTH CHECK - 1% sample)`)
      }
    }
