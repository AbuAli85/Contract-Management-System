...
serve(async req => {
  ...
  try {
    ...
    console.log(
      `📋 Found ${pendingContracts.length} pending contracts to process`
    );
...
function generateReminderEmailBody(
  ...
  const daysSinceCreation = Math.floor(
    (new Date().getTime() - new Date(contractDetails.created_at).getTime()) /
      (1000 * 60 * 60 * 24)
  );
...
function generateEscalationEmailBody(contract: PendingContract): string {
  const daysSinceCreation = Math.floor(
    (new Date().getTime() - new Date(contract.created_at).getTime()) /
      (1000 * 60 * 60 * 24)
  );
