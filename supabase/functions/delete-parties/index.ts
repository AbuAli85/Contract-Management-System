...
serve(async (req) => {
  ...
  try {
    ...
    if (!partyIds || !Array.isArray(partyIds) || partyIds.length === 0) {
      return new Response(
        JSON.stringify({ error: 'Invalid party IDs provided' }),
        { 
          status: 400, 
    ...
    if (activeContracts && activeContracts.length > 0) {
      const partyIdsWithContracts = [...new Set([
        ...activeContracts.map(c => c.first_party_id),
        ...activeContracts.map(c => c.second_party_id)
      ])]
      
      result.errors.push(`Cannot delete parties with active contracts: ${partyIdsWithContracts.join(', ')}`)
      return new Response(
        JSON.stringify(result),
        { 
          status: 400, 
