...
export function useBookingUpsert(options: UseBookingUpsertOptions = {}) {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['booking', data.booking_number] })
...
export function useBookingUpsertAPI(options: UseBookingUpsertOptions = {}) {
  ...
  return useMutation({
    mutationFn: async (payload: Partial<BookingPayload> & Pick<BookingPayload, 'service_id' | 'provider_company_id' | 'client_id'>): Promise<BookingResult> => {
