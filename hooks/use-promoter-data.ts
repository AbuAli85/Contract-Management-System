...
const api = {
  ...
  updatePromoter: async ({ id, data }: { id: string; data: Partial<Promoter> }): Promise<Promoter> => {
    const response = await fetch(`/api/promoters/${id}`, {
      method: 'PUT',
...
export const usePromoterPerformance = (promoterId: string, period: string = 'month') => {
  ...
  const performance = {
    ...
    averageAttendance: attendanceData ? 
      attendanceData.reduce((acc, day) => acc + day.present, 0) / attendanceData.length : 0,
    totalPoints: tasks ? 
      tasks.filter(t => t.status === 'completed').reduce((acc, t) => acc + t.points, 0) : 0
  }
