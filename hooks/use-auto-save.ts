...
export function useAutoSave<T = any>({
  ...
  if (!watch || !formState) {
    console.warn("💾 Auto-save: watch or formState not provided, auto-save disabled")
    return {
      saveData: async () => {},
  ...
  const loadFromLocalStorage = useCallback(() => {
    try {
      ...
      if (saved) {
        ...
        if (!isExpired) {
          console.log("💾 Auto-save: Loaded from localStorage", { key, timestamp: parsed.timestamp })
          return parsed.data
        } else {
          localStorage.removeItem(key)
          console.log("💾 Auto-save: Removed expired localStorage data", { key })
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (onError) {
        ...
        toast({
          ...
          description: "Your changes were saved locally but failed to sync with the server.",
          variant: "destructive",
        })
      }
    } finally {
