...
export function useBookings(
  ...
  const loadMore = useCallback(async (page: PaginationParams) => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to load more bookings'
      );
    }
  }, []);
...
export function usePaginatedBookings(
  ...
  const loadInitialData = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const result = await fetchBookingsPageWithFilters(
        { from: 0, to: filters.limit! - 1 },
        filters
      );
      setBookings(result.data);
      setCount(result.count);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeBookings(event => {
      if (event.eventType === 'INSERT') {
        ...
        setBookings(prev =>
          prev.filter(booking => booking.id !== event.record.id)
        );
