...
export function useUserProfile() {
  ...
  const checkCircuitBreaker = useCallback(() => {
    ...
    const lastReset = parseInt(
      window.localStorage.getItem('userProfileCircuitReset') || '0'
    );
    const callCount = parseInt(
      window.localStorage.getItem('userProfileCallCount') || '0'
    );
    ...
    if (callCount >= CIRCUIT_BREAKER_THRESHOLD) {
      console.warn(
        '🚨 User profile circuit breaker activated - too many calls'
      );
      return true;
    }
  ...
  const fetchUserProfile = useCallback(async () => {
    ...
    try {
      ...
      if (!targetUserId) {
        if (isLocalhost) {
          console.log(
            '⚠️ No authenticated user, using default admin user for localhost development'
          );
      ...
      const isProductionHardcodedId =
        targetUserId === '3f5dea42-c4bd-44bd-bcb9-0ac81e3c8170' && !isLocalhost;
      if (isProductionHardcodedId) {
        console.error(
          '🚨 BLOCKED: Attempted to use hardcoded user ID in production!'
        );
        setLoading(false);
      ...
      const enhancedProfile: EnhancedUserProfile = {
        ...
        getInitials: () =>
          getInitials(baseProfile.full_name || baseProfile.display_name || ''),
      ...
      try {
        ...
        if (data.success && data.profile) {
          ...
          const updatedProfile: EnhancedUserProfile = {
            ...
            getInitials: () =>
              getInitials(
                data.profile.full_name || data.profile.display_name || ''
              ),
