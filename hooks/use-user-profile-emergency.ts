...
export function useUserProfileEmergency() {
  console.log('🚨 EMERGENCY useUserProfile hook loaded - Infinite loop prevention active')
  const { user } = useAuth()
  ...
  useEffect(() => {
    console.log('🚨 Emergency useEffect running - creating fallback profile only')
    ...
    if (user && user.id) {
      ...
      setError(null)
      console.log('✅ Emergency profile created from auth data:', fallbackProfile)
    } else {
      setProfile(null)
  ...
  const fetchUserProfile = () => {
    console.log('🚨 Emergency mode: Profile fetching disabled to prevent infinite loops')
    return Promise.resolve()
  }

  const updateUserProfile = () => {
    console.log('🚨 Emergency mode: Profile updates disabled to prevent infinite loops')
    return Promise.reject(new Error('Profile updates disabled in emergency mode'))
  }
