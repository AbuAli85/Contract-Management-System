...
import { CalendarDays, Clock, User, DollarSign, MapPin, CheckCircle, AlertCircle } from 'lucide-react'
...
export function BookingFormWithWebhook({ service, onBookingCreated }: BookingFormWithWebhookProps) {
  const { toast } = useToast()
  const [loading, setLoading] = useState(false)
  const [webhookStatus, setWebhookStatus] = useState<'idle' | 'triggering' | 'success' | 'failed'>('idle')
    ...
    try {
      ...
      setTimeout(() => {
        ...
        toast({
          ...
          description: "Email confirmations sent to both client and provider. Calendar events created.",
          duration: 3000
        })
      }, 2000)
  ...
  const testWebhook = async () => {
    try {
      ...
      const response = await fetch('/api/bookings/webhook/test?event=booking.created')
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{service.currency} {service.base_price}</span>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Fill out the form below. You'll receive automatic email confirmations and calendar invites.
          </CardDescription>
          ...
          {webhookStatus !== 'idle' && (
            <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
              ...
              <span className="text-sm font-medium">{getWebhookStatusText()}</span>
            </div>
          )}
        </CardHeader>
        <CardContent>
