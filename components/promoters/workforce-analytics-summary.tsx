...
export function WorkforceAnalyticsSummary({
  ...
  const workforceMetrics = useMemo(() => {
    ...
    const critical = promoters.filter(
      p => p.overallStatus === 'critical'
    ).length;
    const warning = promoters.filter(p => p.overallStatus === 'warning').length;
    const inactive = promoters.filter(
      p => p.overallStatus === 'inactive'
    ).length;

    // Assignment metrics
    const assigned = promoters.filter(
      p => p.assignmentStatus === 'assigned'
    ).length;
    const unassigned = promoters.filter(
      p => p.assignmentStatus === 'unassigned'
    ).length;

    // Document health
    const documentsExpired = promoters.filter(
      p =>
        p.idDocument.status === 'expired' ||
        p.passportDocument.status === 'expired'
    ).length;
    const documentsExpiring = promoters.filter(
      p =>
        p.idDocument.status === 'expiring' ||
        p.passportDocument.status === 'expiring'
    ).length;
    const documentsHealthy = promoters.filter(
      p =>
    ...
    const countries = new Set(promoters.map(p => p.nationality).filter(Boolean))
      .size;

    // Company distribution
    const companies = new Set(promoters.map(p => p.employer_id).filter(Boolean))
      .size;

    // Job title diversity
    const jobTitles = new Set(promoters.map(p => p.job_title).filter(Boolean))
      .size;

    // Compliance metrics
    const complianceRate =
      total > 0 ? Math.round((documentsHealthy / total) * 100) : 0;
          ...
          <div className='flex items-center gap-4'>
            ...
            <div>
              ...
              <p className='text-slate-600 dark:text-slate-400'>
                Complete insights across{' '}
                {workforceMetrics.total.toLocaleString()} workforce members
              </p>
            </div>
          </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        ...
        <Card className='bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 border-green-200 dark:border-green-800 shadow-lg'>
          <CardContent className='p-6'>
            ...
            <Progress
              value={workforceMetrics.activeRate}
              className='mt-3 h-2'
            />
          </CardContent>
        </Card>
        ...
        <Card className='bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 border-purple-200 dark:border-purple-800 shadow-lg'>
          <CardContent className='p-6'>
            ...
            <Progress
              value={workforceMetrics.complianceRate}
              className='mt-3 h-2 [&>div]:bg-purple-500'
            />
          </CardContent>
        </Card>
        ...
        <Card className='bg-gradient-to-br from-orange-50 to-orange-100 dark:from-orange-900/20 dark:to-orange-800/20 border-orange-200 dark:border-orange-800 shadow-lg'>
          <CardContent className='p-6'>
            ...
            <Progress
              value={workforceMetrics.assignmentRate}
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
        ...
        <Card className='shadow-lg border-0'>
          ...
          <CardContent className='space-y-4'>
            {[
              {
                label: 'Active',
                value: workforceMetrics.active,
                status: 'active',
              },
              {
                label: 'Critical',
                value: workforceMetrics.critical,
                status: 'critical',
              },
              {
                label: 'Warning',
                value: workforceMetrics.warning,
                status: 'warning',
              },
              {
                label: 'Inactive',
                value: workforceMetrics.inactive,
                status: 'inactive',
              },
            ].map(item => (
              <div
        ...
        <Card className='shadow-lg border-0'>
          <CardHeader>
            ...
            <CardDescription>
              Document compliance status overview
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-4'>
            <div className='space-y-3'>
              <div className='flex items-center justify-between'>
                ...
                <div className='flex items-center gap-2'>
                  <span className='font-medium text-green-600'>
                    {workforceMetrics.documentsHealthy}
                  </span>
              ...
              <div className='flex items-center justify-between'>
                ...
                <span className='font-medium text-yellow-600'>
                  {workforceMetrics.documentsExpiring}
                </span>
              </div>
              ...
              <div className='flex items-center justify-between'>
                ...
                <span className='font-medium text-red-600'>
                  {workforceMetrics.documentsExpired}
                </span>
              </div>
            </div>
        ...
        <Card className='shadow-lg border-0'>
          ...
          <CardContent className='space-y-4'>
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div className='text-center'>
                <div className='text-2xl font-bold text-orange-600'>
                  {workforceMetrics.recentlyAdded}
                </div>
            ...
            <div className='pt-2 border-t'>
              ...
              <Progress
                value={workforceMetrics.assignmentRate}
                className='h-2 mt-1 [&>div]:bg-purple-500'
              />
            </div>
          </CardContent>
        </Card>
