          ...
          <div className='bg-red-50 border border-red-200 rounded-lg p-4'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <div className='text-sm text-red-600'>Documents Expired</div>
              </div>
          ...
          <div className='bg-green-50 border border-green-200 rounded-lg p-4'>
            <div className='flex items-center justify-between'>
              <div>
                ...
                <div className='text-sm text-green-600'>Documents Healthy</div>
              </div>
                  ...
                  <div className='flex h-4 bg-gray-100 rounded-full overflow-hidden'>
                    ...
                    {item.expired > 0 && (
                      <div
                        ...
                        style={{
                          width: `${(item.expired / Math.max(maxValue, 1)) * 100}%`,
                        }}
                      />
                    )}
                    {/* Expiring Soon */}
                    {item.expiringSoon > 0 && (
                      <div
                        ...
                        style={{
                          width: `${(item.expiringSoon / Math.max(maxValue, 1)) * 100}%`,
                        }}
                      />
                    )}
                    {/* Expiring Later */}
                    {item.expiringLater > 0 && (
                      <div
                        ...
                        style={{
                          width: `${(item.expiringLater / Math.max(maxValue, 1)) * 100}%`,
                        }}
                      />
                    )}
                  </div>
                    ...
                    <div className='absolute top-full left-0 mt-1 text-xs text-muted-foreground'>
                      ...
                      {item.expiringSoon > 0 && (
                        <span className='text-orange-600'>
                          Soon: {item.expiringSoon}{' '}
                        </span>
                      )}
                      {item.expiringLater > 0 && (
                        <span className='text-yellow-600'>
                          Later: {item.expiringLater}
                        </span>
                      )}
                    </div>
                  )}
            ...
            <div className='flex items-start gap-3'>
              ...
              <div>
                ...
                <ul className='text-sm text-amber-700 space-y-1'>
                  {currentStats.expiredCount > 0 && (
                    <li>
                      • {currentStats.expiredCount} promoters have expired
                      documents that need immediate renewal
                    </li>
                  )}
                  {currentStats.expiringSoonCount > 0 && (
                    <li>
                      • {currentStats.expiringSoonCount} promoters have
