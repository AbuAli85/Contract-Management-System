...
export function PromoterIntelligenceHubLayout({
  ...
  const getDocumentHealth = () => {
    ...
    if (promoterData.id_card_expiry_date) {
      const daysUntilExpiry = differenceInDays(new Date(promoterData.id_card_expiry_date), new Date());
    ...
    if (promoterData.passport_expiry_date) {
      const daysUntilExpiry = differenceInDays(new Date(promoterData.passport_expiry_date), new Date());
      if (daysUntilExpiry < 0) {
        urgentAlerts.push({
          id: 'passport-expired',
    ...
    const validDocs = urgentAlerts.length === 0 ? 2 : urgentAlerts.filter(a => a.severity !== 'critical').length;
    const compliancePercentage = Math.round((validDocs / totalDocs) * 100);

    return {
      status: urgentAlerts.length === 0 ? 'compliant' : 
              urgentAlerts.some(a => a.severity === 'critical') ? 'critical' : 'warning',
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-4 space-y-6">
        ...
        {/* Document Health Summary */}
        <Card className={`border-2 ${getHealthStatusColor(documentHealth.status)}`}>
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-lg font-bold">{documentHealth.compliancePercentage}%</span>
              </div>
              <Progress value={documentHealth.compliancePercentage} className="h-2" />
            </div>

            {/* Urgent Alerts */}
            {documentHealth.urgentAlerts.length > 0 ? (
              <div className="space-y-2">
                <h4 className="text-xs font-semibold text-gray-700">Urgent Alerts</h4>
