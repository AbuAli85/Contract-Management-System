...
export function DataCompletenessDashboard({
  ...
  const metrics = useMemo<CompletenessMetrics>(() => {
    ...
    const incomplete = {
      email: promoters.filter(
        p => !p.contactEmail || p.contactEmail.trim() === ''
      ),
      phone: promoters.filter(
        p => !p.contactPhone || p.contactPhone.trim() === ''
      ),
      idCard: promoters.filter(
        p => !p.id_card_expiry_date || p.idDocument.status === 'missing'
      ),
      passport: promoters.filter(
        p => !p.passport_expiry_date || p.passportDocument.status === 'missing'
      ),
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='text-right'>
            <div
              className={cn(
                'text-4xl font-bold',
                getStatusColor(metrics.overall)
              )}
            >
              {metrics.overall}%
            </div>
            ...
            <div className='flex items-center gap-3 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800'>
              ...
              <div>
                <div className='text-2xl font-bold text-amber-700 dark:text-amber-300'>
                  {Math.round(
                    ((100 - metrics.overall) / 100) * metrics.totalPromoters
                  )}
                </div>
            ...
            <div className='mt-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg'>
              ...
              <ul className='space-y-1 text-sm text-amber-800 dark:text-amber-200'>
                {metrics.byField.email < 90 && (
                  <li>
                    • Collect missing email addresses from{' '}
                    {metrics.incomplete.email.length} promoters
                  </li>
                )}
                {metrics.byField.phone < 80 && (
                  <li>
                    • Update phone numbers for {metrics.incomplete.phone.length}{' '}
                    promoters
                  </li>
                )}
                {metrics.byField.idCard < 90 && (
                  <li>
                    • Request ID card documents from{' '}
                    {metrics.incomplete.idCard.length} promoters
                  </li>
                )}
                {metrics.byField.passport < 70 && (
                  <li>
                    • Request passport documents from{' '}
                    {metrics.incomplete.passport.length} promoters
                  </li>
                )}
                {metrics.byField.assignment < 50 && (
                  <li>
                    • Assign {metrics.incomplete.assignment.length} promoters to
                    companies
                  </li>
                )}
              </ul>
