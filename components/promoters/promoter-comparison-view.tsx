...
export function PromoterComparisonView({
  ...
  const [selectedCategory, setSelectedCategory] = useState<
    'all' | ComparisonMetric['category']
  >('all');
  const [comparisonType, setComparisonType] = useState<'team' | 'top' | 'both'>(
    'both'
  );
  ...
  const filteredMetrics = comparisonMetrics.filter(
  ...
  const overallRanking = useMemo(() => {
    ...
    const topCount = comparisonMetrics.filter(
      m =>
        getPerformanceLevel(
          m.promoterValue,
          m.teamAverage,
    ...
    const aboveAvgCount = comparisonMetrics.filter(
      m =>
        getPerformanceLevel(
          m.promoterValue,
          m.teamAverage,
    ...
    <div className='space-y-6'>
            ...
            <div className='flex items-center gap-4'>
              ...
              <div>
                ...
                <p className='text-sm text-gray-500 mt-1'>
                  {promoterName} ranks in the {overallRanking.percentile}th
                  percentile
                </p>
              </div>
            </div>
      ...
      <div className='grid grid-cols-1 gap-4'>
        {filteredMetrics.map(metric => {
          ...
          const teamDiff = calculateDifference(
            metric.promoterValue,
            metric.teamAverage,
            metric.higherIsBetter
          );
          const topDiff = calculateDifference(
            metric.promoterValue,
            metric.topPerformer,
            metric.higherIsBetter
          );
            ...
            <Card key={metric.id}>
              <CardContent className='p-4'>
                ...
                <div className='relative h-12 bg-gray-100 rounded-lg mb-4 overflow-hidden'>
                  ...
                  {(comparisonType === 'team' || comparisonType === 'both') && (
                    <div
                      ...
                      style={{
                        left: `${(metric.teamAverage / Math.max(metric.topPerformer, metric.promoterValue, metric.teamAverage)) * 100}%`,
                      }}
                    >
                      <div className='absolute -top-6 left-1/2 transform -translate-x-1/2 text-xs font-medium text-blue-600 whitespace-nowrap'>
                        Team: {metric.teamAverage}
                        {metric.unit}
                      </div>
                    </div>
                  )}
                  ...
                  {(comparisonType === 'top' || comparisonType === 'both') && (
                    <div
                      ...
                      style={{
                        left: `${(metric.topPerformer / Math.max(metric.topPerformer, metric.promoterValue, metric.teamAverage)) * 100}%`,
                      }}
                    >
                      <div className='absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-xs font-medium text-purple-600 whitespace-nowrap'>
                        Top: {metric.topPerformer}
                        {metric.unit}
                      </div>
                    </div>
                  )}
                  ...
                  <div
                    ...
                    style={{
                      width: `${(metric.promoterValue / Math.max(metric.topPerformer, metric.promoterValue, metric.teamAverage)) * 100}%`,
                    }}
                  >
                ...
                <div className='grid grid-cols-2 gap-4'>
                  {(comparisonType === 'team' || comparisonType === 'both') && (
                    <div
                      className={`flex items-center gap-2 p-2 rounded-lg ${teamDiff.isPositive ? 'bg-green-50' : 'bg-red-50'}`}
                    >
                      <div
                        className={`${teamDiff.isPositive ? 'text-green-600' : 'text-red-600'}`}
                      >
                        {teamDiff.icon}
                      </div>
                      <div>
                        ...
                        <p
                          className={`text-sm font-semibold ${teamDiff.isPositive ? 'text-green-600' : 'text-red-600'}`}
                        >
                          {teamDiff.isPositive ? '+' : ''}
                          {teamDiff.percentage}%
                        </p>
                  ...
                  {(comparisonType === 'top' || comparisonType === 'both') && (
                    <div
                      className={`flex items-center gap-2 p-2 rounded-lg ${topDiff.isPositive ? 'bg-green-50' : 'bg-red-50'}`}
                    >
                      <div
                        className={`${topDiff.isPositive ? 'text-green-600' : 'text-red-600'}`}
                      >
                        {topDiff.icon}
                      </div>
                      <div>
                        ...
                        <p
                          className={`text-sm font-semibold ${topDiff.isPositive ? 'text-green-600' : 'text-red-600'}`}
                        >
                          {topDiff.isPositive ? '+' : ''}
                          {topDiff.percentage}%
                        </p>
              ...
              >
                ...
                <div>
                  ...
                  <p className='text-sm text-green-700 mt-1'>
                    Performing at top tier level ({m.promoterValue}
                    {m.unit}). Continue current strategies.
                  </p>
                </div>
              </div>
            ))}
              ...
              >
                ...
                <div>
                  ...
                  <p className='text-sm text-orange-700 mt-1'>
                    Below team average ({m.promoterValue}
                    {m.unit} vs {m.teamAverage}
                    {m.unit}). Consider additional training or support.
                  </p>
                </div>
              </div>
            ))}
