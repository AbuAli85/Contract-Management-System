...
import { AlertCircle, FileText, Mail, MessageSquare, Loader2, CheckCircle2 } from 'lucide-react';
...
export function BulkDocumentRequestDialog({
  ...
  const [deadlineOption, setDeadlineOption] = useState<DeadlineOption>('14days');
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        description: (
          <div className="space-y-2">
            <p className="font-semibold">
              Successfully sent to {data.results.successCount} of {data.results.totalCount} promoters
            </p>
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'Failed to send document requests',
        variant: 'destructive',
      });
    } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Request documents from {selectedPromoterIds.length} selected promoter{selectedPromoterIds.length !== 1 ? 's' : ''}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={deadlineOption} onValueChange={(value) => setDeadlineOption(value as DeadlineOption)}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                >
                  ...
                  Send SMS Notification {priority === 'urgent' && '(Recommended)'}
                </label>
              </div>
            </div>
          </div>
          ...
          {priority === 'urgent' && (
            <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
              ...
              <div className="text-sm text-amber-800 dark:text-amber-200">
                <strong>Urgent Priority:</strong> This will send immediate notifications to all selected promoters. 
                Consider enabling SMS for critical requests.
              </div>
            </div>
          )}
        </div>
