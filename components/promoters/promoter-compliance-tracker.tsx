...
interface Document {
  id: string;
  type: 'id_card' | 'passport' | 'work_permit' | 'health_certificate' | 'criminal_record' | 'contract' | 'training_certificate' | 'insurance';
  name: string;
...
export function PromoterComplianceTracker({
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'critical' | 'expiring' | 'missing'>('all');
    ...
    const docs: Document[] = [
      {
        ...
        status: getDocumentStatus(promoterData.id_card_expiry_date, promoterData.id_card_url),
        isRequired: true,
        priority: 'critical',
        category: 'identity',
      ...
      },
      {
        ...
        status: getDocumentStatus(promoterData.passport_expiry_date, promoterData.passport_url),
        isRequired: false,
        priority: 'high',
        category: 'identity',
  ...
  const getDocumentStatus = (expiryDate?: string, url?: string): Document['status'] => {
  ...
  const getStatusIcon = (status: Document['status']) => {
    switch (status) {
      ...
      case 'missing': return <FileText className="h-5 w-5 text-gray-400" />;
      case 'pending_review': return <Clock className="h-5 w-5 text-blue-500" />;
    }
  };
  ...
  const getCategoryIcon = (category: Document['category']) => {
    switch (category) {
      ...
      case 'legal': return <FileCheck className="h-4 w-4" />;
      case 'medical': return <AlertCircle className="h-4 w-4" />;
  ...
  const complianceScore = useMemo(() => {
    ...
    const validDocs = requiredDocs.filter(d => d.status === 'valid' || d.status === 'pending_review');
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className={`border-2 ${complianceScore >= 90 ? 'border-green-200 bg-green-50' : complianceScore >= 70 ? 'border-yellow-200 bg-yellow-50' : 'border-red-200 bg-red-50'}`}>
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <div className={`p-4 rounded-full ${complianceScore >= 90 ? 'bg-green-100' : complianceScore >= 70 ? 'bg-yellow-100' : 'bg-red-100'}`}>
              ...
              <div>
                ...
                <Badge className={`mt-1 ${complianceScore >= 90 ? 'bg-green-500' : complianceScore >= 70 ? 'bg-yellow-500' : 'bg-red-500'}`}>
                  {complianceScore >= 90 ? 'Fully Compliant' : complianceScore >= 70 ? 'Partially Compliant' : 'Non-Compliant'}
                </Badge>
              </div>
            </div>
            <div className="grid grid-cols-3 gap-4 text-center">
              ...
              <div>
                <div className="text-2xl font-bold text-red-600">
                  {documents.filter(d => d.status === 'expired' || d.status === 'missing').length}
                </div>
              ...
              <AlertDescription>
                <ul className="list-disc list-inside space-y-1">
                  {criticalAlerts.map(doc => (
                    <li key={doc.id}>
                      <span className="font-medium">{doc.name}</span> - {doc.status.replace('_', ' ')}
      ...
      <div className="flex flex-wrap gap-2">
        {(['all', 'critical', 'expiring', 'missing'] as const).map((filter) => (
          ...
          >
            ...
            {filter === 'critical' && `Critical (${documents.filter(d => d.priority === 'critical').length})`}
            {filter === 'expiring' && `Expiring (${documents.filter(d => d.status === 'expiring_soon' || d.status === 'expired').length})`}
            {filter === 'missing' && `Missing (${documents.filter(d => d.status === 'missing').length})`}
          </Button>
        ))}
      </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <Badge className={`text-xs ${getPriorityColor(doc.priority)} bg-transparent border`}>
                        {doc.priority}
                      </Badge>
              ...
              {doc.expiryDate && (
                <div className="mb-3">
                  <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <span className="font-medium">{format(new Date(doc.expiryDate), 'MMM dd, yyyy')}</span>
                  </div>
                  {doc.status !== 'expired' && (
                    <div className="flex items-center gap-2 text-xs text-gray-500">
                      <Clock className="h-3 w-3" />
                      {differenceInDays(new Date(doc.expiryDate), new Date())} days remaining
                    </div>
                  )}
                </div>
              )}
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {complianceRequirements.map((req) => {
            const reqDocs = documents.filter(d => req.documents.includes(d.type));
            const compliantDocs = reqDocs.filter(d => d.status === 'valid' || d.status === 'pending_review');
            ...
            return (
              <div key={req.id} className="p-4 border rounded-lg">
                ...
                <div className="flex items-center justify-between text-xs text-gray-500">
                  <span>{compliantDocs.length} of {reqDocs.length} documents compliant</span>
                  <span className="flex items-center gap-1">
                    <FileClock className="h-3 w-3" />
                    {reqDocs.filter(d => d.status === 'expiring_soon').length} expiring soon
                  </span>
                </div>
              </div>
            );
