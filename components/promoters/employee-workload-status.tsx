...
export function EmployeeWorkloadStatus({
  ...
  const workloadMetrics = useMemo(() => {
    ...
    const workloadPercentage = Math.min(
      100,
      (activeContracts.length / maxActiveContracts) * 100
    );
    ...
    const averageDuration =
      contractCount > 0 ? Math.round(totalDays / contractCount) : 0;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Current assignment status and capacity
        </CardDescription>
      </CardHeader>
        ...
        <div>
          ...
          <div className='flex items-center justify-between mt-1'>
            <span className='text-xs text-muted-foreground'>
              {workloadMetrics.activeContracts} active assignment
              {workloadMetrics.activeContracts !== 1 ? 's' : ''}
            </span>
            <span className='text-xs text-muted-foreground'>
        ...
        <div className='grid grid-cols-3 gap-2 pt-2 border-t'>
          <div className='text-center'>
            <div className='text-lg font-bold text-blue-600'>
              {workloadMetrics.activeContracts}
            </div>
            <div className='text-xs text-muted-foreground'>Active</div>
          </div>
          <div className='text-center'>
            <div className='text-lg font-bold text-orange-600'>
              {workloadMetrics.pendingContracts}
            </div>
            <div className='text-xs text-muted-foreground'>Pending</div>
          </div>
          <div className='text-center'>
            <div className='text-lg font-bold text-green-600'>
              {workloadMetrics.completedContracts}
            </div>
          ...
          <div className='pt-2 border-t'>
            <div className='flex items-center justify-between text-sm'>
              ...
              <span className='font-semibold'>
                {workloadMetrics.averageDuration} days
              </span>
            </div>
          </div>
        )}
          ...
          <div className='p-2 bg-red-50 border border-red-200 rounded-lg'>
            <div className='flex items-center gap-2 text-sm text-red-700'>
              ...
              <span className='font-medium'>
                High workload detected. Consider redistributing assignments.
              </span>
            </div>
          </div>
        )}

        {workloadMetrics.workloadStatus === 'low' &&
          workloadMetrics.activeContracts === 0 && (
            <div className='p-2 bg-blue-50 border border-blue-200 rounded-lg'>
              <div className='flex items-center gap-2 text-sm text-blue-700'>
                <TrendingUp className='h-4 w-4' />
                <span className='font-medium'>
                  Employee has capacity for new assignments.
                </span>
              </div>
            </div>
          )}
