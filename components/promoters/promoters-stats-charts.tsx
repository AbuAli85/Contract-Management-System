 ...
 */
export function PromotersStatsCharts({
  metrics,
  promoters,
  hasFiltersApplied = false,
}: PromotersStatsChartsProps) {
  ...
  const totalExpiring = expiryTimeline.reduce(
  ...
  const statusDistribution = [
    {
      status: 'Assigned',
      count: assigned,
      percentage: Math.round((assigned / metrics.total) * 100) || 0,
      color: 'bg-green-500',
      description: 'Active with employer',
    },
    {
      status: 'Available',
      count: available,
      percentage: Math.round((available / metrics.total) * 100) || 0,
      color: 'bg-blue-500',
    ...
    },
    {
      ...
      percentage: Math.round((metrics.critical / metrics.total) * 100) || 0,
      color: 'bg-red-500',
    ...
    },
    {
      ...
      percentage: Math.round((metrics.expiring / metrics.total) * 100) || 0,
      color: 'bg-amber-500',
        ...
        <CardHeader>
          <div className='flex items-center justify-between'>
            ...
            <Badge
              ...
              className={
                metrics.complianceRate >= 90 ? 'bg-green-500' : 'bg-amber-500'
              }
            >
          ...
          <div className='space-y-4'>
            ...
            <div className='space-y-2'>
              <div className='flex items-center justify-between text-sm'>
                ...
                <span className='text-muted-foreground'>
                  {metrics.total - metrics.critical - metrics.expiring}/
                  {metrics.total} compliant
                </span>
              </div>
            ...
            {(metrics.critical > 0 || metrics.expiring > 0) && (
              <div className='mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md'>
                ...
                <p className='text-xs text-blue-700'>
                  {metrics.critical > 0 &&
                    `${metrics.critical} critical documents need immediate renewal. `}
                  {metrics.expiring > 0 &&
                    `${metrics.expiring} documents expiring soon.`}
                </p>
              </div>
            )}
          </div>
