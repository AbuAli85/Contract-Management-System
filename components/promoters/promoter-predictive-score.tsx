...
export function PromoterPredictiveScore({
  ...
  const analysis = useMemo((): PredictiveAnalysis => {
    ...
    const contractWeight = 0.20;
    totalScore += contractScore * contractWeight;
    totalWeight += contractWeight;
    factors.push({
      label: 'Contract Activity',
      impact: activeContracts >= 2 ? 'positive' : activeContracts >= 1 ? 'neutral' : 'negative',
    ...
    const daysSinceActive = lastActive ? 
      Math.floor((Date.now() - new Date(lastActive).getTime()) / (1000 * 60 * 60 * 24)) : 7;
    ...
    factors.push({
      ...
      impact: daysSinceActive <= 3 ? 'positive' : daysSinceActive <= 7 ? 'neutral' : 'negative',
    ...
    let prediction = '';
    if (finalScore >= 85) {
      prediction = 'Excellent trajectory. Likely to exceed targets and maintain high performance.';
    } else if (finalScore >= 70) {
      prediction = 'Stable performance expected. Monitor for optimization opportunities.';
    } else if (finalScore >= 60) {
      prediction = 'Performance at risk. Intervention recommended to prevent decline.';
    } else {
  ...
  const getRiskBadge = () => {
    switch (analysis.riskLevel) {
      case 'low':
        return <Badge className="bg-green-500"><CheckCircle className="h-3 w-3 mr-1" />Low Risk</Badge>;
      case 'medium':
        return <Badge className="bg-blue-500"><Activity className="h-3 w-3 mr-1" />Medium Risk</Badge>;
      case 'high':
        return <Badge className="bg-orange-500"><AlertTriangle className="h-3 w-3 mr-1" />High Risk</Badge>;
      case 'critical':
        return <Badge className="bg-red-500"><AlertTriangle className="h-3 w-3 mr-1" />Critical Risk</Badge>;
    }
  };
      ...
      <CardContent className="p-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold text-purple-900">Predictive Performance Score</h3>
          </div>
          {getRiskBadge()}
        </div>
        ...
        <div className="flex items-center justify-center mb-4">
          <div className="relative">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <div className="text-3xl font-bold text-purple-900">{analysis.score}</div>
        ...
        <div className="flex items-center justify-center gap-2 mb-4">
          ...
          <span className={`text-sm font-medium ${
            analysis.trend === 'improving' ? 'text-green-700' :
            analysis.trend === 'declining' ? 'text-red-700' :
            'text-blue-700'
          }`}>
            {analysis.trend === 'improving' ? 'Improving' :
             analysis.trend === 'declining' ? 'Declining' :
             'Stable'}
          </span>
        </div>
        ...
        <div className="p-3 bg-white rounded-lg border border-purple-200 mb-4">
          <p className="text-xs font-medium text-purple-900 mb-1">30-Day Forecast</p>
          <p className="text-xs text-gray-700">{analysis.prediction}</p>
        </div>

        {/* Contributing Factors */}
        <div className="space-y-2">
          <p className="text-xs font-semibold text-purple-900">Contributing Factors</p>
          {analysis.factors.map((factor, index) => (
            <div key={index} className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-700 flex-1">{factor.label}</span>
        ...
        <div className="mt-4 pt-3 border-t border-purple-200">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-purple-700">Prediction Confidence</span>
            <span className="text-xs font-semibold text-purple-900">{analysis.confidence}%</span>
          </div>
