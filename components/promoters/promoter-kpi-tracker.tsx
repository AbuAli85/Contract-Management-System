...
export function PromoterKPITracker({
    ...
    return [
      {
        ...
        targetDate: new Date(today.getFullYear(), today.getMonth() + 1, 0).toISOString(),
        priority: 'high',
        status: 'on-track',
        milestones: [
          { id: 'm1', name: 'First Quarter', targetValue: 1250, targetDate: addDays(new Date(), -15).toISOString(), isAchieved: true, achievedDate: addDays(new Date(), -14).toISOString() },
          { id: 'm2', name: 'Mid-Month', targetValue: 2500, targetDate: addDays(new Date(), -5).toISOString(), isAchieved: true, achievedDate: addDays(new Date(), -3).toISOString() },
          { id: 'm3', name: 'Three Quarters', targetValue: 3750, targetDate: addDays(new Date(), 5).toISOString(), isAchieved: false },
          { id: 'm4', name: 'Month End', targetValue: 5000, targetDate: addDays(new Date(), 15).toISOString(), isAchieved: false }
        ],
        history: [
      ...
      },
      {
        ...
        status: 'at-risk',
        milestones: [
          { id: 'm1', name: 'Q1 Target', targetValue: 85, targetDate: new Date(today.getFullYear(), 2, 31).toISOString(), isAchieved: true },
          { id: 'm2', name: 'Q2 Target', targetValue: 90, targetDate: new Date(today.getFullYear(), 5, 30).toISOString(), isAchieved: false },
          { id: 'm3', name: 'Q3 Target', targetValue: 92, targetDate: new Date(today.getFullYear(), 8, 30).toISOString(), isAchieved: false },
          { id: 'm4', name: 'Year End', targetValue: 95, targetDate: new Date(today.getFullYear(), 11, 31).toISOString(), isAchieved: false }
        ],
        history: [
          { date: new Date(today.getFullYear(), 0, 1).toISOString(), value: 82 },
          { date: new Date(today.getFullYear(), 1, 1).toISOString(), value: 84 },
          { date: new Date(today.getFullYear(), 2, 1).toISOString(), value: 86 },
          { date: new Date(today.getFullYear(), 3, 1).toISOString(), value: 87 },
      ...
      },
      {
        ...
        status: 'achieved',
        milestones: [
          { id: 'm1', name: 'Week 1', targetValue: 85, targetDate: addDays(new Date(), -21).toISOString(), isAchieved: true },
          { id: 'm2', name: 'Week 2', targetValue: 88, targetDate: addDays(new Date(), -14).toISOString(), isAchieved: true },
          { id: 'm3', name: 'Week 3', targetValue: 90, targetDate: addDays(new Date(), -7).toISOString(), isAchieved: true },
        ],
        history: [
      ...
      },
      {
        ...
        targetValue: 5,
        unit: ' courses',
        startDate: new Date(today.getFullYear(), Math.floor(today.getMonth() / 3) * 3, 1).toISOString(),
        targetDate: new Date(today.getFullYear(), Math.floor(today.getMonth() / 3) * 3 + 3, 0).toISOString(),
        priority: 'medium',
        status: 'on-track',
        milestones: [
          { id: 'm1', name: 'Month 1', targetValue: 2, targetDate: addDays(new Date(), -30).toISOString(), isAchieved: true },
          { id: 'm2', name: 'Month 2', targetValue: 3, targetDate: addDays(new Date(), -10).toISOString(), isAchieved: true },
          { id: 'm3', name: 'Month 3', targetValue: 5, targetDate: addDays(new Date(), 20).toISOString(), isAchieved: false },
        ],
        history: [
  ...
  const getStatusIcon = (status: KPI['status']) => {
    switch (status) {
      case 'achieved': return <Trophy className="h-4 w-4" />;
  ...
  const calculateVelocity = (kpi: KPI) => {
    ...
    const daysDiff = differenceInDays(new Date(recent[1].date), new Date(recent[0].date));
  ...
  const summary = useMemo(() => {
    return {
      ...
      avgProgress: Math.round(kpis.reduce((sum, k) => sum + calculateProgress(k), 0) / kpis.length)
    };
  }, [kpis]);
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="text-center p-4 bg-purple-50 rounded-lg">
              <div className="text-2xl font-bold text-purple-600">{summary.avgProgress}%</div>
      ...
      <div className="flex flex-wrap gap-2">
        {(['all', 'performance', 'quality', 'efficiency', 'growth', 'custom'] as const).map((cat) => (
          <Button
            key={cat}
          ...
          >
            {cat === 'all' ? 'All KPIs' : cat.charAt(0).toUpperCase() + cat.slice(1)}
          </Button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredKPIs.map((kpi) => {
          ...
          const achievedMilestones = kpi.milestones.filter(m => m.isAchieved).length;
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between text-xs text-gray-500">
                    ...
                    <span>{kpi.targetValue - kpi.currentValue}{kpi.unit} remaining</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="p-2 bg-gray-50 rounded">
                    ...
                    <div className="font-medium">{format(new Date(kpi.targetDate), 'MMM dd, yyyy')}</div>
                    <div className={`${daysRemaining < 0 ? 'text-red-600' : daysRemaining < 7 ? 'text-yellow-600' : 'text-gray-500'}`}>
                  ...
                  <div className="p-2 bg-gray-50 rounded">
                    ...
                    <div className="font-medium">{velocity.toFixed(1)}{kpi.unit}/day</div>
                    {estimatedCompletion && (
                      <div className={`${
                        estimatedCompletion > new Date(kpi.targetDate) ? 'text-red-600' : 'text-green-600'
                      }`}>
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-gray-500">
                              {milestone.targetValue}{kpi.unit} • {format(new Date(milestone.targetDate), 'MMM dd')}
                            </div>
                          </div>
                          {milestone.isAchieved && milestone.achievedDate && (
                            <Badge variant="outline" className="text-xs bg-green-100 text-green-700">
                              {format(new Date(milestone.achievedDate), 'MMM dd')}
                            </Badge>
                          )}
                        </div>
                      ))}
