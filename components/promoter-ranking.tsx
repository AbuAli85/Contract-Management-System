...
import { Trophy, Star, Award, TrendingUp, Target, MessageSquare, Plus, Edit, Trash2, Crown, Medal, Zap, Heart, Shield, Users, BarChart3 } from 'lucide-react'
...
export function PromoterRanking({ promoterId, isAdmin }: PromoterRankingProps) {
  ...
  const handleScoreSubmit = async () => {
    try {
      ...
      const body = editingItem ? { id: editingItem.id, ...scoreForm } : scoreForm
      ...
      if (response.ok) {
        ...
        toast.success(editingItem ? 'Score updated successfully' : 'Score added successfully')
      }
    } catch (error) {
  ...
  const handleBadgeSubmit = async () => {
    try {
      ...
      const method = editingItem ? 'PUT' : 'POST'
      const body = editingItem ? { id: editingItem.id, ...badgeForm } : badgeForm
      ...
      if (response.ok) {
        ...
        toast.success(editingItem ? 'Badge updated successfully' : 'Badge added successfully')
      }
    } catch (error) {
  ...
  const handleFeedbackSubmit = async () => {
    try {
      ...
      const method = editingItem ? 'PUT' : 'POST'
      const body = editingItem ? { id: editingItem.id, ...feedbackForm } : feedbackForm
      ...
      if (response.ok) {
        ...
        if (editingItem) {
          setFeedback(feedback.map(f => f.id === editingItem.id ? newFeedback : f))
        } else {
        ...
        toast.success(editingItem ? 'Feedback updated successfully' : 'Feedback added successfully')
      }
    } catch (error) {
  ...
  const feedbackData = [
    { name: '5 Stars', value: feedback.filter(f => f.rating === 5).length, color: '#10b981' },
    { name: '4 Stars', value: feedback.filter(f => f.rating === 4).length, color: '#3b82f6' },
    { name: '3 Stars', value: feedback.filter(f => f.rating === 3).length, color: '#f59e0b' },
    { name: '2 Stars', value: feedback.filter(f => f.rating === 2).length, color: '#ef4444' },
    { name: '1 Star', value: feedback.filter(f => f.rating === 1).length, color: '#dc2626' }
  ]
    ...
    return (
      <Card>
        <CardContent className="p-6">
          <p className="text-muted-foreground text-center">Ranking management is restricted to administrators.</p>
        </CardContent>
      </Card>
    )
  }
        ...
        <TabsContent value="scores" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Dialog open={isScoreDialogOpen} onOpenChange={setIsScoreDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{editingItem ? 'Edit Score' : 'Add Score'}</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="score-type">Score Type</Label>
                    <Select value={scoreForm.score_type} onValueChange={(value) => setScoreForm({ ...scoreForm, score_type: value })}>
                      <SelectTrigger>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setScoreForm({ ...scoreForm, score_value: parseInt(e.target.value) || 0 })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setScoreForm({ ...scoreForm, max_score: parseInt(e.target.value) || 100 })}
                      />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setScoreForm({ ...scoreForm, period_start: e.target.value })}
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setScoreForm({ ...scoreForm, period_end: e.target.value })}
                      />
                    </div>
                  </div>
                </div>
                  ...
                  {scores.map((score) => (
                    <div key={score.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium capitalize">{score.score_type.replace('_', ' ')}</p>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="text-right">
                          <p className={`font-semibold ${getScoreColor(score.score_value)}`}>
                            {score.score_value}/{score.max_score || 100}
                          </p>
                          <Progress value={(score.score_value / (score.max_score || 100)) * 100} className="w-20" />
                        </div>
        ...
        <TabsContent value="badges" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Dialog open={isBadgeDialogOpen} onOpenChange={setIsBadgeDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{editingItem ? 'Edit Badge' : 'Award Badge'}</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="badge-type">Badge Type</Label>
                    <Select value={badgeForm.badge_type} onValueChange={(value) => setBadgeForm({ ...badgeForm, badge_type: value })}>
                      <SelectTrigger>
                        <SelectValue placeholder="Select badge type" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="attendance_streak">Attendance Streak</SelectItem>
                        <SelectItem value="performance_excellence">Performance Excellence</SelectItem>
                        <SelectItem value="team_player">Team Player</SelectItem>
                        <SelectItem value="quality_champion">Quality Champion</SelectItem>
              ...
              <Card key={badge.id} className="relative">
                <CardContent className="p-6">
                  ...
                  {badge.badge_description && (
                    <p className="text-sm text-muted-foreground mb-3">{badge.badge_description}</p>
                  )}
        ...
        <TabsContent value="feedback" className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <Dialog open={isFeedbackDialogOpen} onOpenChange={setIsFeedbackDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{editingItem ? 'Edit Feedback' : 'Add Feedback'}</DialogTitle>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                    ...
                    <Select value={feedbackForm.feedback_type} onValueChange={(value) => setFeedbackForm({ ...feedbackForm, feedback_type: value })}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="manager_feedback">Manager Feedback</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  <div>
                    ...
                    <div className="flex gap-1 mt-2">
                      {[1, 2, 3, 4, 5].map((star) => (
                        ...
                        >
                          <Star
                            className={`h-6 w-6 ${star <= feedbackForm.rating ? 'text-yellow-400 fill-current' : 'text-gray-300'}`}
                          />
                        </button>
                      ))}
                    </div>
