...
export function AdvancedDashboard() {
  ...
  const [selectedView, setSelectedView] = useState<'overview' | 'analytics' | 'activity'>('overview')
  ...
  const getStatusColor = (status: string, priority?: string) => {
    if (priority === 'high' || status === 'urgent') return 'text-red-600 bg-red-50'
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          ...
          <p className="text-muted-foreground text-lg">Loading advanced dashboard...</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <Card className="relative overflow-hidden">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatCurrency(metrics?.contracts.value || 0)}</div>
              <p className="text-xs text-muted-foreground">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
          <Card className="relative overflow-hidden">
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatPercentage(metrics?.documents.compliance_rate || 0)}</div>
              <p className="text-xs text-muted-foreground">
                Document compliance score
              </p>
              <div className="mt-2">
                <Progress value={metrics?.documents.compliance_rate || 0} className="h-1" />
              </div>
            </CardContent>
          </Card>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          <Card className="relative overflow-hidden">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">{metrics?.notifications.urgent}</div>
              <p className="text-xs text-muted-foreground">
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline">{metrics?.activity.active_users}</Badge>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="font-medium">{metrics?.documents.active}</span>
                  </div>
                  <Progress value={92} className="h-2" />
                  
                  <div className="flex justify-between items-center">
                    <span className="text-sm">Expiring Soon</span>
                    <span className="font-medium text-orange-600">{metrics?.documents.expiring}</span>
                  </div>
                  <Progress value={15} className="h-2 bg-orange-100" />
                  
                  <div className="flex justify-between items-center">
                    <span className="text-sm">Expired</span>
                    <span className="font-medium text-red-600">{metrics?.documents.expired}</span>
                  </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="text-center">
                    <div className="text-3xl font-bold text-blue-600">{formatPercentage(metrics?.bookings.utilization || 0)}</div>
                    <div className="text-sm text-muted-foreground">Resource Utilization</div>
                  </div>
                  
                  <Progress value={metrics?.bookings.utilization || 0} className="h-3" />
                  
                  <div className="grid grid-cols-2 gap-4 text-center">
                    <div>
                      <div className="text-xl font-semibold">{metrics?.bookings.today}</div>
                      <div className="text-xs text-muted-foreground">Today</div>
                    </div>
                    <div>
                      <div className="text-xl font-semibold">{metrics?.bookings.upcoming}</div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    ...
                    <Badge variant="destructive">{metrics?.notifications.unread}</Badge>
                  </div>
                  
                  <div className="flex justify-between items-center">
                    <span className="text-sm">Urgent</span>
                    <Badge variant="destructive">{metrics?.notifications.urgent}</Badge>
                  </div>
                  
                  <div className="flex justify-between items-center">
                    <span className="text-sm">Total</span>
                    <Badge variant="outline">{metrics?.notifications.total}</Badge>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-4 border rounded-lg">
                      <div className="text-2xl font-bold text-green-600">{metrics?.contracts.active}</div>
                      <div className="text-sm text-muted-foreground">Active</div>
                    </div>
                    <div className="text-center p-4 border rounded-lg">
                      <div className="text-2xl font-bold text-yellow-600">{metrics?.contracts.pending}</div>
                      <div className="text-sm text-muted-foreground">Pending</div>
                    </div>
                  </div>
                  <div className="text-center p-4 border rounded-lg">
                    <div className="text-2xl font-bold text-blue-600">{metrics?.contracts.completed}</div>
        ...
        <TabsContent value="activity" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Real-time activity feed across all system modules</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <AnimatePresence>
                  {recentActivity.map((activity, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${getStatusColor(activity.status, activity.priority)}`}>
