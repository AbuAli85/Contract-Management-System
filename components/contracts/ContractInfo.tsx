          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-start gap-3 p-3 bg-blue-50 rounded-lg">
              <Avatar className="h-10 w-10">
                <AvatarImage src={mockActivityData.addedBy.avatar} alt={mockActivityData.addedBy.name} />
                <AvatarFallback>{mockActivityData.addedBy.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
              </Avatar>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <p className="font-semibold text-sm">{mockActivityData.addedBy.name}</p>
                  <Badge variant="secondary" className="text-xs">{mockActivityData.addedBy.role}</Badge>
                </div>
                <p className="text-sm text-gray-600">{mockActivityData.addedBy.email}</p>
            ...
            <div className="flex items-start gap-3 p-3 bg-green-50 rounded-lg">
              <Avatar className="h-10 w-10">
                <AvatarImage src={mockActivityData.lastUpdatedBy.avatar} alt={mockActivityData.lastUpdatedBy.name} />
                <AvatarFallback>{mockActivityData.lastUpdatedBy.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
              </Avatar>
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <p className="font-semibold text-sm">{mockActivityData.lastUpdatedBy.name}</p>
                  <Badge variant="secondary" className="text-xs">{mockActivityData.lastUpdatedBy.role}</Badge>
                </div>
                <p className="text-sm text-gray-600">{mockActivityData.lastUpdatedBy.email}</p>
                <p className="text-xs text-gray-500 mt-1">
                ...
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      {notification.user && (
                        <p className="text-xs text-gray-500 mt-1">
                          By: <span className="font-medium">{notification.user}</span>
                        </p>
                      )}
                    </div>
