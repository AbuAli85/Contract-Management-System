...
export function ContractInfoEnhanced({
  contract,
  contractId,
  useRealData = false,
}: ContractInfoEnhancedProps) {
  const {
    data: activityData,
    loading,
    error,
    refetch,
  } = useContractActivity(useRealData ? contractId : '');

  // Use real data if available and requested, otherwise use mock data
  const displayData =
    useRealData && activityData
          ...
          <CardContent className='space-y-4'>
            ...
            {displayData.addedBy && (
              <div className='flex items-start gap-3 p-3 bg-blue-50 rounded-lg'>
                <Avatar className='h-10 w-10'>
                  <AvatarImage
                    src={displayData.addedBy.avatar_url}
                    alt={displayData.addedBy.full_name}
                  />
                  <AvatarFallback>
                    {getUserInitials(displayData.addedBy.full_name)}
                  </AvatarFallback>
                </Avatar>
                <div className='flex-1'>
                  <div className='flex items-center gap-2'>
                    <p className='font-semibold text-sm'>
                      {displayData.addedBy.full_name}
                    </p>
            ...
            {displayData.lastUpdatedBy && (
              <div className='flex items-start gap-3 p-3 bg-green-50 rounded-lg'>
                <Avatar className='h-10 w-10'>
                  <AvatarImage
                    src={displayData.lastUpdatedBy.avatar_url}
                    alt={displayData.lastUpdatedBy.full_name}
                  />
                  <AvatarFallback>
                    {getUserInitials(displayData.lastUpdatedBy.full_name)}
                  </AvatarFallback>
                </Avatar>
                <div className='flex-1'>
                  <div className='flex items-center gap-2'>
                    <p className='font-semibold text-sm'>
                      {displayData.lastUpdatedBy.full_name}
                    </p>
                  ...
                  <p className='text-sm text-gray-600'>
                    {displayData.lastUpdatedBy.email}
                  </p>
                  <p className='text-xs text-gray-500 mt-1'>
