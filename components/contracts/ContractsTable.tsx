        ...
        <TableCell className='w-12'>
          <Checkbox
            ...
            onCheckedChange={checked =>
              onSelect(contract.id, checked as boolean)
            }
          />
        </TableCell>
...
const ContractsTable = React.memo(({ className }: ContractsTableProps) => {
  ...
  const filteredAndSortedContracts = useMemo(() => {
    ...
    // Sort contracts
    filtered.sort((a, b) => {
      let aValue: string | number = a[sortField as keyof Contract] as
        | string
        | number;
      let bValue: string | number = b[sortField as keyof Contract] as
        | string
        | number;
    ...
    (selected: boolean) => {
      if (selected) {
        setSelectedContracts(
          new Set(filteredAndSortedContracts.map(c => c.id))
        );
      } else {
