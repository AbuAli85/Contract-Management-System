...
export type ContractStatusValue = 'draft' | 'pending_review' | 'active' | 'expired' | 'terminated' | 'suspended' | 'archived' | 'unknown'
...
export function UnifiedStatusBadge({ 
  ...
  const statusConfig = type === 'document' 
    ? getDocumentStatus(status as DocumentStatus)
  ...
  if (type === 'document') {
    const documentBadge = (
      <div className={cn("flex items-center space-x-2 rounded-md border p-2", statusConfig.color, className)}>
