...
export function SimpleVirtualTable<T>({
  ...
  const handleRowSelect = useCallback((item: T, selected: boolean) => {
  ...
  return (
    <div className={cn("border rounded-lg overflow-hidden", className)}>
      ...
      <div className="bg-muted/50 border-b">
        <div className="flex">
          {onRowSelect && (
            <div className="w-12 flex items-center justify-center p-3 border-r">
              <input
                type="checkbox"
          ...
          {columnWidths.map((column) => (
            <div
              key={column.key}
              className="flex items-center p-3 font-medium text-sm"
...
export function OptimizedContractsTable({
  ...
  const handleRowSelect = useCallback((contractId: string, selected: boolean) => {
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      render: (contract: Contract) => (
        <div className="flex flex-col">
          <span className="text-sm">{new Date(contract.contract_start_date).toLocaleDateString()}</span>
          <span className="text-xs text-muted-foreground">to {new Date(contract.contract_end_date).toLocaleDateString()}</span>
        </div>
      )
    },
    {
