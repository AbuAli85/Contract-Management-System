...
export function AttendanceReportsAnalytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const response = await fetch(
        `/api/employee/attendance/analytics?month=${selectedMonth}&range=${dateRange}`
      );
  ...
  const exportToExcel = async () => {
    try {
      const response = await fetch(
        `/api/employee/attendance/export?month=${selectedMonth}&format=excel`
      );
    ...
    <div className='space-y-6'>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Attendance pattern over the selected period
          </CardDescription>
        </CardHeader>
        <CardContent>
