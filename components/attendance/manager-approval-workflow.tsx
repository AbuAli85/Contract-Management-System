...
export function ManagerApprovalWorkflow() {
  ...
  const fetchPendingAttendance = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/employer/attendance/pending?filter=${filter}`);
  ...
  return (
    <div className="space-y-6">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Review and approve employee attendance records</CardDescription>
            </div>
              ...
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">
                    <Checkbox
                      checked={selectedRecords.size === attendance.length && attendance.length > 0}
                    ...
                    <TableRow key={record.id}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="font-medium text-sm">{record.employee?.full_name || 'Unknown'}</div>
                      ...
                      <TableCell>
                        <Badge
                          ...
                          className={cn(
                            'text-xs',
                            record.approval_status === 'approved' && 'bg-green-100 text-green-800 border-green-300',
                            record.approval_status === 'pending' && 'bg-yellow-100 text-yellow-800 border-yellow-300',
                            record.approval_status === 'rejected' && 'bg-red-100 text-red-800 border-red-300'
                          )}
                        >
      ...
      {/* Approval Dialog */}
      <Dialog open={approvalDialog.open} onOpenChange={(open) => setApprovalDialog({ open, action: null })}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {approvalDialog.action === 'approve' ? 'Approve Attendance' : 'Reject Attendance'}
            </DialogTitle>
            <DialogDescription>
