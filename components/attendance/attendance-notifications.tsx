...
export function AttendanceNotifications() {
  ...
  const fetchSettings = async () => {
    try {
      const response = await fetch(
        '/api/employee/attendance/notifications/settings'
      );
      const data = await response.json();
  ...
  const markAsRead = async (id: string) => {
    try {
      const response = await fetch(
        `/api/employee/attendance/notifications/${id}/read`,
        {
          method: 'POST',
        }
      );
  ...
  const markAllAsRead = async () => {
    try {
      const response = await fetch(
        '/api/employee/attendance/notifications/read-all',
        {
          method: 'POST',
        }
      );
        ...
        <CardHeader>
          <div className='flex items-center justify-between'>
            <div>
              ...
              <CardDescription>
                Stay updated with attendance alerts and reminders
              </CardDescription>
            </div>
        ...
        <CardContent>
          {notifications.length === 0 ? (
            <div className='text-center py-12 text-gray-500'>
              ...
              <p className='text-sm'>
                You'll be notified about attendance updates here
              </p>
            </div>
          ) : (
          ...
          <div className='space-y-4'>
            ...
            <div className='flex items-center justify-between'>
              <div className='space-y-0.5'>
                ...
                <p className='text-sm text-gray-500'>
                  Notify when attendance is approved/rejected
                </p>
              </div>
              <Switch
            ...
            <div className='flex items-center justify-between'>
              <div className='space-y-0.5'>
                ...
                <p className='text-sm text-gray-500'>
                  Warn when approaching late check-in time
                </p>
              </div>
              <Switch
