  ...
  return (
    <Card className="max-w-md">
      ...
      <CardContent className="space-y-2 text-sm">
        ...
        <p><strong>Auth User Metadata Role:</strong> {user?.user_metadata?.role || 'null'}</p>
        <p><strong>Profile Role:</strong> {profile?.role || 'null'}</p>
        <p><strong>Current Role (hook):</strong> {currentRole}</p>
        <p><strong>Role Display:</strong> {roleInfo.displayText}</p>
        <p><strong>Profile Full Name:</strong> {profile?.full_name || 'null'}</p>
        <p><strong>Profile Display Name:</strong> {profile?.display_name || 'null'}</p>
