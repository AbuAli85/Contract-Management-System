...
import { runDashboardDiagnostics, testDashboardDataPoints, quickDashboardHealthCheck } from '@/lib/dashboard-diagnostics'
...
export function DashboardDiagnosticsPanel({ className = "" }: DashboardDiagnosticsPanelProps) {
  const [isRunning, setIsRunning] = useState(false)
  const [results, setResults] = useState<DashboardDiagnosticSummary | null>(null)
  ...
  const quickCheck = async () => {
    ...
    try {
      ...
      setResults({
        ...
        summary: isHealthy ? 'Dashboard is working correctly' : 'Dashboard has issues'
      })
    } catch (error) {
  ...
  return (
    <Card className={`border-orange-200 bg-orange-50/50 ${className}`}>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-orange-800">Dashboard Diagnostics</CardTitle>
          </div>
          <Badge variant={results?.failedTests === 0 ? "default" : "destructive"}>
            {results ? `${results.passedTests}/${results.totalTests}` : 'Not Run'}
          </Badge>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {/* Results Summary */}
        {results && (
          <Alert className={results.failedTests === 0 ? "border-green-200 bg-green-50" : "border-red-200 bg-red-50"}>
            {results.failedTests === 0 ? (
              <CheckCircle className="h-4 w-4 text-green-600" />
            ) : (
              <XCircle className="h-4 w-4 text-red-600" />
            )}
            <AlertTitle className={results.failedTests === 0 ? "text-green-800" : "text-red-800"}>
              {results.failedTests === 0 ? 'All Tests Passed' : 'Some Tests Failed'}
            </AlertTitle>
            <AlertDescription className={results.failedTests === 0 ? "text-green-700" : "text-red-700"}>
              {results.summary}
            </AlertDescription>
          </Alert>
        )}
          ...
          <div className="space-y-3">
            ...
            {dataPoints.stats && (
              <div className="p-3 bg-white rounded-lg border">
                ...
                <div className="text-sm text-slate-600">
                  <div>Total Contracts: {dataPoints.stats.totalContracts || 0}</div>
                  <div>Total Promoters: {dataPoints.stats.totalPromoters || 0}</div>
                  <div>Total Parties: {dataPoints.stats.totalParties || 0}</div>
                  <div>Pending Approvals: {dataPoints.stats.pendingApprovals || 0}</div>
                </div>
              </div>
            )}
            ...
            {dataPoints.notifications && (
              <div className="p-3 bg-white rounded-lg border">
                ...
                <div className="text-sm text-slate-600">
                  Count: {Array.isArray(dataPoints.notifications) ? dataPoints.notifications.length : 'N/A'}
                </div>
              </div>
            )}
            ...
            {dataPoints.activities && (
              <div className="p-3 bg-white rounded-lg border">
                ...
                <div className="text-sm text-slate-600">
                  Count: {Array.isArray(dataPoints.activities) ? dataPoints.activities.length : 'N/A'}
                </div>
              </div>
            )}
          </div>
