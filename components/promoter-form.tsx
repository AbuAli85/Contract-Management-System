...
export default function PromoterForm(props: PromoterFormProps) {
  ...
  const handleDocumentUpload = async (file: File, documentType: 'id_document' | 'passport_document') => {
    if (!file) return
  ...
  const handleDocumentRemove = (documentType: 'id_document' | 'passport_document') => {
    setUploadedDocuments(prev => ({
      ...prev,
      [documentType]: null
    }))
  ...
  const fetchCompanies = async () => {
    ...
    try {
      ...
      const companyNames = data?.map(party => party.name_en || party.name_ar).filter(Boolean) || []
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant={uploadedDocuments.id_document ? "default" : "secondary"}>
                      {uploadedDocuments.id_document ? "Uploaded" : "Not Uploaded"}
                    </Badge>
                  </div>
                    ...
                    <div className="border rounded-lg p-4 bg-gray-50">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <p className="font-medium text-sm">{uploadedDocuments.id_document.name}</p>
                            <p className="text-xs text-gray-500">
                              {(uploadedDocuments.id_document.size / 1024 / 1024).toFixed(2)} MB
                            </p>
                          </div>
                        </div>
                        <div className="flex space-x-2">
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            onClick={() => window.open(uploadedDocuments.id_document.url, '_blank')}
                          >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <Label className="text-base font-medium">Passport Document</Label>
                    <Badge variant={uploadedDocuments.passport_document ? "default" : "secondary"}>
                      {uploadedDocuments.passport_document ? "Uploaded" : "Not Uploaded"}
                    </Badge>
                  </div>
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                      ...
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const file = e.target.files?.[0]
                            if (file) handleDocumentUpload(file, 'passport_document')
                          }}
                          className="hidden"
                        ...
                        <Button
                          type="button"
                          variant="outline"
                          onClick={() => document.getElementById('passport-document-upload')?.click()}
                          disabled={uploading}
                          className="mt-2"
                        >
                          {uploading ? (
                    ...
                    <div className="border rounded-lg p-4 bg-gray-50">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          <FileText className="h-8 w-8 text-blue-500" />
                          <div>
                            <p className="font-medium text-sm">{uploadedDocuments.passport_document.name}</p>
                            <p className="text-xs text-gray-500">
                              {(uploadedDocuments.passport_document.size / 1024 / 1024).toFixed(2)} MB
                            </p>
                          </div>
                        </div>
                        <div className="flex space-x-2">
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            onClick={() => window.open(uploadedDocuments.passport_document.url, '_blank')}
                          >
                            <Eye className="h-4 w-4" />
                          </Button>
                          <Button
                            ...
                            variant="outline"
                            size="sm"
                            onClick={() => handleDocumentRemove('passport_document')}
                          >
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="graduation_year">Graduation Year (YYYY)</Label>
                    <Input
                      id="graduation_year"
                      type="number"
