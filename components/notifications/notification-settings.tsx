        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label>Refresh Interval: {settings.autoRefreshInterval} minutes</Label>
              <Slider
                value={[settings.autoRefreshInterval]}
                onValueChange={(value: number[]) => updateSetting('autoRefreshInterval', value[0])}
                max={60}
                min={1}
                step={1}
                className="w-full"
            ...
            {Object.entries(settings.categories).map(([category, enabled]) => (
              <div key={category} className="flex items-center justify-between">
                ...
                <Switch
                  id={category}
                  checked={enabled}
                  onCheckedChange={(checked) => updateSetting(`categories.${category}`, checked)}
                />
              </div>
            ))}
          </div>
            ...
            {Object.entries(settings.priorities).map(([priority, enabled]) => (
              <div key={priority} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  {priority === 'medium' && <AlertTriangle className="h-4 w-4 text-yellow-500" />}
                  {priority === 'low' && <Clock className="h-4 w-4 text-blue-500" />}
                ...
                <Switch
                  id={priority}
                  checked={enabled}
                  onCheckedChange={(checked) => updateSetting(`priorities.${priority}`, checked)}
                />
              </div>
            ))}
          </div>
