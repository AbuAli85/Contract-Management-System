...
export function useEnhancedRBAC(): EnhancedRBACContextType {
  ...
  if (!context) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error('useEnhancedRBAC must be used within an EnhancedRBACProvider')
...
export function EnhancedRBACProvider({ children }: EnhancedRBACProviderProps) {
  ...
  const refreshRoles = useCallback(async () => {
    ...
    try {
      ...
      if (!usersError && usersData?.role) {
        console.log("✅ Enhanced RBAC: Found user by email, role:", usersData.role)
        userData = usersData
        ...
        console.log('📋 Enhanced RBAC: User not found by email, trying auth ID...')
        ...
        if (!authIdError && authIdUser?.role) {
          console.log("✅ Enhanced RBAC: Found user by auth ID, role:", authIdUser.role)
          userData = authIdUser
          ...
          console.log('🔄 Enhanced RBAC: Using email fallback role:', fallbackRole)
      ...
      if (!userData) {
        console.warn('⚠️ Enhanced RBAC: No user data found, using default user role')
  ...
  const checkAnyPermission = useCallback((permissions: PermissionKey[]): boolean => {
    if (!userRole) return false
    return hasAnyPermission(userRole, permissions)
  }, [userRole])

  const checkAllPermissions = useCallback((permissions: PermissionKey[]): boolean => {
    if (!userRole) return false
    return hasAllPermissions(userRole, permissions)
  }, [userRole])

  const checkRoleLevel = useCallback((requiredRole: EnhancedUserRole): boolean => {
    if (!userRole) return false
...
export function PermissionWrapper({ permission, children, fallback = null }: PermissionWrapperProps) {
...
export function RoleWrapper({ role, children, fallback = null }: RoleWrapperProps) {
...
export function RoleRedirect({ allowedRoles, redirectTo = '/unauthorized', children }: RoleRedirectProps) {
...
export function CompanyWrapper({ requireMembership = true, children, fallback = null }: CompanyWrapperProps) {
