...
import { ProfessionalAuthService, AuthState, SecurityContext } from '@/lib/auth/professional-auth-service'
...
interface ProfessionalAuthContextType extends AuthState {
  ...
  signInWithMFA: (token: string, backupCode?: string) => Promise<{ success: boolean; error?: string }>
  
  signInWithBiometric: () => Promise<{ success: boolean; error?: string }>
  
  signInWithSSO: (provider: string) => Promise<{ success: boolean; error?: string }>
  
  // MFA Management
  enableMFA: () => Promise<{ success: boolean; qrCode?: string; backupCodes?: string[]; error?: string }>
  
  verifyMFASetup: (token: string) => Promise<{ success: boolean; error?: string }>
  
  disableMFA: (password: string) => Promise<{ success: boolean; error?: string }>
...
export function ProfessionalAuthProvider({ 
  ...
  const signIn = useCallback(async (
    ...
    const result = await authService.signInWithCredentials(email, password, options)
  ...
  const signInWithMFA = useCallback(async (token: string, backupCode?: string) => {
  ...
  const enableMFA = useCallback(async () => {
    ...
    if (result.success) {
      ...
      toast({
        ...
        description: result.error || "Unable to set up two-factor authentication.",
        variant: "destructive"
      })
    }
  ...
  const verifyMFASetup = useCallback(async (token: string) => {
  ...
  const disableMFA = useCallback(async (password: string) => {
    ...
    if (result.success) {
      ...
      toast({
        ...
        description: result.error || "Unable to disable two-factor authentication.",
        variant: "destructive"
      })
    }
  ...
  const signInWithBiometric = useCallback(async () => {
    ...
    if (result.success) {
      ...
      toast({
        ...
        description: result.error || "Biometric authentication not available or failed.",
        variant: "destructive"
      })
    }
  ...
  const requestAccountDeletion = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description: "Your account will be deleted within 30 days. You can cancel this request until then.",
        variant: "destructive"
      })
      return { success: true }
    } catch (error) {
