...
export function SocialLogin({
  ...
  const handleSocialLogin = async (provider: SupportedProvider) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Redirecting to ${PROVIDER_CONFIGS[provider]?.name || provider}...`
        );
        onSuccess?.();
      } else {
        const errorMessage =
      ...
      <Card className='mx-auto w-full max-w-md'>
        ...
        <CardContent>
          <Alert>
            ...
            <AlertDescription>
              Social login providers need to be configured in your Supabase
              project settings.
            </AlertDescription>
          </Alert>
        </CardContent>
    ...
    <Card className='mx-auto w-full max-w-md'>
      ...
      <CardContent className='space-y-4'>
        ...
        <Alert>
          ...
          <AlertDescription>
            Your account is now connected to {config?.name || provider}. You can
            use this method to sign in in the future.
          </AlertDescription>
        </Alert>
