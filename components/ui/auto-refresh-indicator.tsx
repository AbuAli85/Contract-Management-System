...
export function AutoRefreshIndicator({ className, showIcon = true }: AutoRefreshIndicatorProps) {
  ...
  return (
    ...
    >
      ...
      {formCount > 1 && (
        <span className="text-muted-foreground">({formCount} forms active)</span>
      )}
    </Badge>
  )
}
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      <div className="flex items-center gap-1">
        <Zap className={cn(
          "h-4 w-4",
          isFormActive ? "text-muted-foreground" : "text-green-500"
        )} />
        <span className={cn(
          "text-sm",
          isFormActive ? "text-muted-foreground" : "text-green-600"
        )}>
          {isFormActive ? "Paused" : "Active"}
        </span>
      </div>
      {isFormActive && (
        <AutoRefreshIndicator showIcon={false} />
      )}
    </div>
  )
} 