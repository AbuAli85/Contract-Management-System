...
export function PromoterCVResume({
  ...
  const handleExperienceSubmit = async () => {
    try {
      ...
      const method = editingItem ? 'PUT' : 'POST';
      const body = editingItem
        ? { id: editingItem.id, ...experienceForm }
        : experienceForm;
      ...
      if (response.ok) {
        ...
        if (editingItem) {
          setExperience(
            experience.map(e => (e.id === editingItem.id ? newExperience : e))
          );
        } else {
    ...
    try {
      ...
      const body = editingItem
        ? {
            id: editingItem.id,
            ...educationForm,
            year: parseInt(educationForm.year) || null,
          }
      ...
      if (response.ok) {
        ...
        if (editingItem) {
          setEducation(
            education.map(e => (e.id === editingItem.id ? newEducation : e))
          );
        } else {
  ...
  const handleDocumentSubmit = async () => {
    try {
      ...
      const method = editingItem ? 'PUT' : 'POST';
      const body = editingItem
        ? { id: editingItem.id, ...documentForm }
        : documentForm;
      ...
      if (response.ok) {
        ...
        if (editingItem) {
          setDocuments(
            documents.map(d => (d.id === editingItem.id ? newDocument : d))
          );
        } else {
