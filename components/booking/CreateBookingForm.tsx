  ...
  return (
    <Card className='w-full max-w-2xl mx-auto'>
      <CardHeader>
        ...
        <CardDescription>
          Book "{serviceDetails.name}" - {serviceDetails.duration_minutes}{' '}
          minutes
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className='space-y-6'>
          ...
          <div className='grid grid-cols-2 gap-4 p-4 bg-muted/50 rounded-lg'>
            <div className='flex items-center gap-2'>
              ...
              <span className='text-sm'>
                {serviceDetails.duration_minutes} minutes
              </span>
            </div>
            <div className='flex items-center gap-2'>
              <DollarSign className='h-4 w-4 text-muted-foreground' />
              <span className='text-sm'>
                {serviceDetails.price_base} {serviceDetails.price_currency}
              </span>
            </div>
            <div className='flex items-center gap-2'>
              <Users className='h-4 w-4 text-muted-foreground' />
              <span className='text-sm'>
                Max: {serviceDetails.max_participants || 'Unlimited'}
              </span>
            </div>
            <div className='flex items-center gap-2'>
              <AlertCircle className='h-4 w-4 text-muted-foreground' />
              <span className='text-sm'>
                {serviceDetails.requires_approval
                  ? 'Approval Required'
                  : 'Auto-confirmed'}
              </span>
            </div>
          ...
          <div className='grid grid-cols-2 gap-4'>
            ...
            <div className='space-y-2'>
              ...
              <Input
                ...
                min={getMinTime()}
                className={
                  validationErrors.scheduledTime ? 'border-red-500' : ''
                }
              />
          ...
          <div className='space-y-2'>
            ...
            <Select
              ...
              onValueChange={value =>
                handleInputChange('participantCount', parseInt(value))
              }
            >
              <SelectTrigger>
                <SelectValue />
