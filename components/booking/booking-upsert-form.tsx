...
export function BookingUpsertForm({
  initialData,
  onSuccess,
  useAPI = false,
  ...
  const form = useForm<BookingFormData>({
    ...
    defaultValues: {
      ...
      scheduled_start: new Date(Date.now() + 4 * 24 * 60 * 60 * 1000)
        .toISOString()
        .slice(0, 16),
    ...
    <Card className='w-full max-w-2xl mx-auto'>
      <CardHeader>
        ...
        <div className='flex gap-2'>
          ...
          <Badge variant='outline'>Conflict Target: booking_number</Badge>
        </div>
      </CardHeader>
      ...
      <CardContent className='space-y-6'>
        <Alert>
          <AlertDescription>
            This form demonstrates Supabase upsert with{' '}
            <code>booking_number</code> as the conflict target. If a booking
            with the same number exists, it will be updated; otherwise, a new
            one will be created.
          </AlertDescription>
        </Alert>
