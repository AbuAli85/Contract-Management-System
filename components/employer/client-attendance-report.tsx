...
export function ClientAttendanceReport() {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      if (data.team) {
        setEmployees(data.team.map((emp: any) => ({
          id: emp.id,
          full_name: emp.employee?.full_name || emp.promoter?.name_en || 'Unknown',
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/employer/team/attendance/report?${params}`);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Generate professional attendance reports for clients</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Select employee and date range for the report</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Select 
                value={reportType} 
                onValueChange={(value) => handleReportTypeChange(value as 'daily' | 'weekly' | 'monthly' | 'custom')}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <p className="font-semibold">{reportData.employee.employee_code || 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Job Title</p>
                <p className="font-semibold">{reportData.employee.job_title || 'N/A'}</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">Department</p>
                <p className="font-semibold">{reportData.employee.department || 'N/A'}</p>
              </div>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-4">
              <div className="p-4 bg-blue-50 dark:bg-blue-900/10 rounded-lg">
                ...
                <p className="text-2xl font-bold">{reportData.summary.total_days}</p>
              </div>
              <div className="p-4 bg-green-50 dark:bg-green-900/10 rounded-lg">
                <p className="text-sm text-gray-500">Present Days</p>
                <p className="text-2xl font-bold">{reportData.summary.present_days}</p>
              </div>
              <div className="p-4 bg-yellow-50 dark:bg-yellow-900/10 rounded-lg">
                <p className="text-sm text-gray-500">Total Hours</p>
                <p className="text-2xl font-bold">{reportData.summary.total_hours.toFixed(1)}h</p>
              </div>
              <div className="p-4 bg-purple-50 dark:bg-purple-900/10 rounded-lg">
                <p className="text-sm text-gray-500">Avg Hours/Day</p>
                <p className="text-2xl font-bold">{reportData.summary.average_hours.toFixed(1)}h</p>
              </div>
            </div>
