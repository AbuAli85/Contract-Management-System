...
interface HRMetrics {
  ...
  attendance: {
    ...
    dailyAttendance: Array<{ date: string; present: number; absent: number; late: number }>;
  };
  documents: {
    total: number;
    expired: number;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {metrics.employees.active} active • {metrics.employees.newHires} new this {period}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {metrics.attendance.present} present • {metrics.attendance.absent} absent
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              {metrics.documents.compliant} compliant • {metrics.documents.expired} expired
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              {metrics.tasks.completed} completed • {metrics.tasks.overdue} overdue
            </p>
          </CardContent>
        </Card>
      </div>
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <RechartsPieChart>
                    <Pie
                      data={Object.entries(metrics.employees.byDepartment).map(([name, value]) => ({
                        name,
                        value,
                      }))}
                      cx="50%"
                      ...
                      dataKey="value"
                    >
                      {Object.entries(metrics.employees.byDepartment).map((_, index) => (
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={250}>
                  <RechartsBarChart data={[
                    ...
                    { name: 'Terminated', value: metrics.employees.terminated },
                  ]}>
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="name" />
                    <YAxis />
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={250}>
                  <RechartsPieChart>
                    <Pie
                      data={Object.entries(metrics.employees.byEmploymentType).map(([name, value]) => ({
                        name,
                        value,
                      }))}
                      cx="50%"
                      ...
                      dataKey="value"
                    >
                      {Object.entries(metrics.employees.byEmploymentType).map((_, index) => (
                        <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                      ))}
                    </Pie>
                    <Tooltip />
        ...
        <TabsContent value="attendance" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{metrics.attendance.leave}</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <RechartsPieChart>
                    <Pie
                      data={[
                        { name: 'Compliant', value: metrics.documents.compliant },
                        { name: 'Expiring Soon', value: metrics.documents.expiringSoon },
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <RechartsBarChart
                    data={Object.entries(metrics.documents.byType).map(([name, value]) => ({
                      name,
                      value,
                    }))}
                  >
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <RechartsPieChart>
                    <Pie
                      data={Object.entries(metrics.tasks.byPriority).map(([name, value]) => ({
                        name,
                        value,
                      }))}
                      cx="50%"
                      ...
                      dataKey="value"
                    >
                      {Object.entries(metrics.tasks.byPriority).map((_, index) => (
                        <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />
                      ))}
                    </Pie>
                    <Tooltip />
