  ...
  const { data: payrollData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        `/api/hr/payroll/runs?month=${selectedMonth}`
      );
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>
              {payrollRuns
                .reduce((sum, run) => sum + (run.total_amount || 0), 0)
                .toFixed(2)}{' '}
              OMR
            </div>
              ...
              <TableBody>
                {payrollRuns.map(run => (
                  <TableRow key={run.id}>
                    ...
                    <TableCell>
                      {run.total_basic_salary?.toFixed(2)} OMR
                    </TableCell>
                    <TableCell>
                      {run.total_allowances?.toFixed(2)} OMR
                    </TableCell>
                    <TableCell>
                      {run.total_deductions?.toFixed(2)} OMR
                    </TableCell>
                    <TableCell>
