...
import { 
  Calendar, Clock, Star, TrendingUp, DollarSign, Users, 
  CheckCircle, AlertCircle, Plus, BarChart3, Settings,
  MapPin, Phone, Mail, Eye
} from 'lucide-react'
    ...
    try {
      ...
      if (servicesData) {
        setServices(servicesData.map(service => ({
          ...
          rating: service.service_reviews?.reduce((sum, r) => sum + r.rating, 0) / (service.service_reviews?.length || 1) || 0,
                    ...
                    {upcomingBookings.map((booking) => (
                      <div key={booking.id} className="flex items-center space-x-4 p-3 border rounded-lg">
                        <Avatar>
                          ...
                          <AvatarFallback>{booking.client_name.slice(0, 2)}</AvatarFallback>
                        </Avatar>
                  ...
                  <div className="space-y-4">
                    {recentReviews.map((review) => (
                      <div key={review.id} className="p-3 border rounded-lg">
                        ...
                        <p className="text-sm text-gray-500 mb-1">{review.service_name}</p>
        ...
        <TabsContent value="bookings" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manage your booking requests and appointments</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {services.map((service) => (
                  <div key={service.id} className="border rounded-lg p-4">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-medium">{formatCurrency(service.price_base)}</span>
                      <div className="flex items-center">
                        <Star className="h-3 w-3 text-yellow-400 fill-current mr-1" />
                        <span className="text-xs">{service.rating.toFixed(1)} ({service.reviews_count})</span>
                      </div>
                    </div>
                    <p className="text-xs text-gray-400 mb-3">{service.bookings_count} bookings</p>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        <Avatar className="h-8 w-8">
                          <AvatarFallback>{review.client_name.slice(0, 2)}</AvatarFallback>
                        </Avatar>
                        <div>
                          <p className="font-medium">{review.client_name}</p>
