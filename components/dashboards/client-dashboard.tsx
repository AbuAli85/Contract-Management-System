...
import { Calendar, Clock, Star, Heart, TrendingUp, MapPin, Phone, Mail, CreditCard } from 'lucide-react'
    ...
    try {
      ...
      if (upcomingData) {
        setUpcomingBookings(upcomingData.map(booking => ({
          ...
          provider_name: booking.provider_services?.provider?.full_name || 'Unknown Provider',
      ...
      if (favoritesData) {
        setFavoriteServices(favoritesData.map(fav => ({
          id: fav.id,
          service_name: fav.provider_services?.name || 'Unknown Service',
          provider_name: fav.provider_services?.provider?.full_name || 'Unknown Provider',
          ...
          <CardContent>
            {upcomingBookings.length === 0 ? (
              <div className="text-center py-8">
                ...
                <Link href="/services">
                  <Button variant="outline" className="mt-2">Browse Services</Button>
                </Link>
              </div>
            ) : (
                ...
                {upcomingBookings.map((booking) => (
                  <div key={booking.id} className="flex items-center space-x-4 p-3 border rounded-lg">
                    <Avatar>
                      ...
                      <AvatarFallback>{booking.provider_name.slice(0, 2)}</AvatarFallback>
                    </Avatar>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium truncate">{booking.service_name}</p>
                      <p className="text-sm text-gray-500">{booking.provider_name}</p>
                      <p className="text-sm text-gray-500">{formatDate(booking.scheduled_start)}</p>
                    </div>
          ...
          <CardContent>
            {favoriteServices.length === 0 ? (
              <div className="text-center py-8">
                ...
                <Link href="/services">
                  <Button variant="outline" className="mt-2">Explore Services</Button>
                </Link>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {favoriteServices.map((service) => (
                  <div key={service.id} className="border rounded-lg p-3">
                    ...
                    <p className="text-sm text-gray-500 truncate">{service.provider_name}</p>
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-sm font-medium">{formatCurrency(service.price_base)}</span>
                ...
                <Link href="/favorites">
                  <Button variant="outline" className="col-span-full">View All Favorites</Button>
                </Link>
              </div>
            )}
          </CardContent>
          ...
          <CardContent>
            {recentReviews.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-400">Share your experience with completed services</p>
              </div>
            ) : (
              <div className="space-y-4">
