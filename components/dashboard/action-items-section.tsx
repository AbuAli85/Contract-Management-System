...
export function ActionItemsSection({ locale = 'en', maxItems = 5 }: ActionItemsSectionProps) {
  const {
    data: actionData,
    isLoading,
    error,
      ...
      <Card className="border-0 shadow-lg bg-gradient-to-br from-green-50 to-emerald-50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {locale === 'ar' ? 'جميع الإجراءات محدثة' : 'All Actions Up to Date'}
          </CardTitle>
          <CardDescription>
            {locale === 'ar'
        ...
        <div className="flex items-center justify-between">
            ...
            <CardDescription className="mt-1">
              {actionData?.highPriority > 0 && (
                <span className="text-red-600 font-medium">
                  {actionData.highPriority} {locale === 'ar' ? 'عاجل' : 'urgent'}
                  {actionData.highPriority > 1 ? (locale === 'ar' ? '' : 's') : ''}
                </span>
              )}
              {actionData?.highPriority > 0 && actionItems.length > actionData.highPriority && (
          ...
          {actionItems.length > maxItems && (
            <Badge variant="outline" className="text-xs">
              +{actionItems.length - maxItems} {locale === 'ar' ? 'أكثر' : 'more'}
            </Badge>
          )}
        </div>
      </CardHeader>
