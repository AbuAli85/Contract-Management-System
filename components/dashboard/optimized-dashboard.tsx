...
export default function OptimizedDashboard() {
  ...
  const { batchGeneratePDFs, getWorkerStatus, cleanupTempFiles } =
  ...
  const fetchOverview = useCallback(async () => {
    try {
      const response = await fetch(
        `/api/dashboard/analytics/paginated?timeRange=${timeRange}`
      );
      if (response.ok) {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      await Promise.all([fetchOverview(), refreshContracts()]);
    } finally {
            ...
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold'>
                  {overview.totalContracts.toLocaleString()}
                </p>
              </div>
            ...
            <div className='flex items-center justify-between'>
              <div>
                ...
                <p className='text-2xl font-bold text-yellow-600'>
                  {overview.pendingApprovals.toLocaleString()}
                </p>
              </div>
        ...
        <div className='flex items-center justify-between'>
          <div className='flex-1'>
            ...
            <div className='text-sm text-muted-foreground space-y-1'>
              ...
              {contract.promoter && (
                <p>
                  <strong>Promoter:</strong> {contract.promoter.name_en}
                </p>
              )}
              <p>
                <strong>Created:</strong> {formatDate(contract.created_at)}
              </p>
            </div>
          </div>
          ...
          <div className='text-right'>
            <p className='text-lg font-semibold'>
              {formatCurrency(
                parseFloat(contract.basic_salary) || 0,
                contract.currency
              )}
            </p>
          </div>
