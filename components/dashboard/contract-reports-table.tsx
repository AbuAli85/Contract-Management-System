...
export default function ContractReportsTable() {
  ...
  useEffect(() => {
    ...
    const contractsBaseChannel = supabase
      ...
      .on(
        ...
        p => handleTableChange(p, 'contracts')
      )
      .subscribe();
    const promotersChannel = supabase
      ...
      .on(
        ...
        p => handleTableChange(p, 'promoters')
      )
      .subscribe();
  ...
  const filteredData = useMemo(() => {
    ...
    return contracts.filter(item => {
      ...
      const matchesSearch =
        (item.contract_id?.toLowerCase() || '').includes(
          searchTerm.toLowerCase()
        ) ||
        (item.promoter_name?.toLowerCase() || '').includes(
          searchTerm.toLowerCase()
        ) ||
        (item.employer_name?.toLowerCase() || '').includes(
          searchTerm.toLowerCase()
        ) ||
        (item.client_name?.toLowerCase() || '').includes(
          searchTerm.toLowerCase()
        );
      const matchesStatus =
      ...
      <CardHeader>
        <div className='flex flex-col items-start justify-between gap-4 md:flex-row md:items-center'>
          <div>
            ...
            <CardDescription>
              Detailed list of contracts. / قائمة مفصلة بالعقود.
            </CardDescription>
          </div>
