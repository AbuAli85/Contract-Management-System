  ...
  const fetchExpiringContracts = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/contracts/expiring?days=${daysThreshold}`
      );
      const data = await response.json();

      if (data.success) {
        const contractsWithDays = (data.contracts || []).map(
          (contract: any) => ({
            ...contract,
      ...
      {contracts.length === 0 ? (
        <div className='flex flex-col items-center justify-center h-full text-center p-4'>
          ...
          <p className='text-xs text-muted-foreground mt-1'>
            No expiring contracts in the next{' '}
            {props.config.filters?.daysThreshold || 30} days
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className='flex items-center gap-2 text-xs text-muted-foreground'>
                ...
                <span className='ml-auto font-medium'>
                  {contract.daysUntilExpiry} day
                  {contract.daysUntilExpiry !== 1 ? 's' : ''}
                </span>
              </div>
            </Link>
          ))}
