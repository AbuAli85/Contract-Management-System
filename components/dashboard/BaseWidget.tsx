...
export function BaseWidget({
  ...
  return (
    <Card
      className={`h-full flex flex-col ${editMode ? 'ring-2 ring-orange-300' : ''}`}
    >
      {showHeader && (
      ...
      <CardContent
        className={`flex-1 ${showHeader ? 'pt-2' : 'pt-6'} overflow-auto`}
      >
        {error ? (
      ...
      {showFooter && footerActions && (
        <div className='px-6 pb-4 border-t pt-2 mt-2'>{footerActions}</div>
      )}
    </Card>
  );
}
