...
export function WidgetLibrary({ onAddWidget, usedWidgets, onClose }: WidgetLibraryProps) {
  ...
  const categories = useMemo(() => {
    const cats = new Set(Object.values(WIDGET_DEFINITIONS).map((w) => w.category));
                ...
                return (
                  <div
                    ...
                    className={`p-4 border rounded-lg ${
                      isUsed ? 'bg-muted opacity-60' : 'bg-card hover:shadow-md transition-shadow'
                    }`}
                  >
