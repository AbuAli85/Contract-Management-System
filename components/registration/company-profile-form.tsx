...
export function CompanyProfileForm({ 
  ...
  React.useEffect(() => {
    ...
    const requiredFields = ['company_name', 'commercial_registration', 'contact_person', 'email', 'phone', 'address']
  ...
  const handleLogoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      setSubmitMessage({ type: 'error', text: 'Logo file size must be less than 2MB' })
      return
    }
  ...
  const onSubmit = async (data: CompanyProfile) => {
    ...
    try {
      ...
      if (logoFile) {
        const uploadResult = await CompanyService.uploadCompanyLogo(logoFile, userId)
      ...
      <Card>
        <CardHeader className="text-center">
          <CardTitle className="flex items-center justify-center gap-2 text-2xl">
            ...
            {role === 'client' ? 'Client Company' : 'Service Provider'} Registration
          </CardTitle>
          <CardDescription>
            {role === 'client' 
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="commercial_registration" className="flex items-center gap-2">
                  ...
                  {errors.commercial_registration && (
                    <p className="text-sm text-red-500 mt-1">{errors.commercial_registration.message}</p>
                  )}
                </div>

                <div>
                  <Label htmlFor="cr_expiry_date" className="flex items-center gap-2">
                  ...
                  {errors.cr_expiry_date && (
                    <p className="text-sm text-red-500 mt-1">{errors.cr_expiry_date.message}</p>
                  )}
                </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {PROVIDER_SERVICES.map((service) => (
                    <Badge key={service} variant="outline" className="cursor-pointer hover:bg-primary hover:text-primary-foreground">
                      {service}
                    </Badge>
                  ))}
                </div>
            ...
            {/* Submit Message */}
            {submitMessage && (
              <Alert className={`${submitMessage.type === 'error' ? 'border-red-500 bg-red-50' : 'border-green-500 bg-green-50'}`}>
