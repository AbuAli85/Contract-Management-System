...
import { TrendingUp, TrendingDown, Calendar, FileText, Download, Plus, BarChart3, Activity, Target, Clock, CheckCircle, XCircle, AlertCircle } from 'lucide-react'
...
export function PromoterReports({ promoterId, isAdmin }: PromoterReportsProps) {
  ...
  const [performanceMetrics, setPerformanceMetrics] = useState<PromoterPerformanceMetric[]>([])
  ...
  const fetchPerformanceMetrics = async () => {
    try {
      const response = await fetch(`/api/promoters/${promoterId}/performance-metrics`)
      if (response.ok) {
        const data = await response.json()
    ...
    return (
      <Card>
        <CardContent className="p-6">
          <p className="text-muted-foreground text-center">Reports are restricted to administrators.</p>
        </CardContent>
      </Card>
    )
  }
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {format(new Date(metric.period_start), 'MMM d')} - {format(new Date(metric.period_end), 'MMM d, yyyy')}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      {getStatusBadge(metric.value >= (metric.target_value || 80) ? 'excellent' : 'average')}
                    </div>
                  </div>
                ))}
            ...
            <Dialog open={isGenerateReportDialogOpen} onOpenChange={setIsGenerateReportDialogOpen}>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Create a new performance or analytics report for this promoter.
                  </DialogDescription>
                </DialogHeader>
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Select value={reportForm.report_type} onValueChange={(value) => setReportForm({ ...reportForm, report_type: value })}>
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                      ...
                      {report.report_data && (
                        <div className="text-sm text-muted-foreground">
                          {report.report_data.attendance_rate && (
                            <p>Attendance Rate: {report.report_data.attendance_rate.toFixed(1)}%</p>
                          )}
                          {report.report_data.summary && (
                            <p>Total Days: {report.report_data.summary.total_days}</p>
                          )}
                        </div>
                      )}
                    </div>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={[
                    ...
                    <YAxis />
                    <Tooltip />
                    <Bar dataKey="promoter" fill="#3b82f6" name="This Promoter" />
                ...
                <div className="flex items-start gap-3 p-4 bg-green-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-700">
                      Attendance rate is 7% above team average. Consider recognizing this achievement.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-4 bg-blue-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-700">
                      Response time could be improved by 5% to match top performers.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-4 bg-yellow-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-700">
                      Late arrivals increased by 2% this month. Consider discussing punctuality.
                    </p>
                  </div>
                </div>
              </div>
