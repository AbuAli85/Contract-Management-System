...
export function SessionTimeout({
  ...
  const progressPercentage = ((timeoutMinutes * 60 - timeRemaining) / (timeoutMinutes * 60)) * 100
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      <Card className={`w-80 transition-all duration-300 ${showWarning ? 'border-orange-200 bg-orange-50' : 'border-gray-200'}`}>
        <CardHeader className="pb-2">
          <CardTitle className="flex items-center gap-2 text-sm">
            <Clock className="h-4 w-4" />
            Session Timeout
            {showWarning && <AlertTriangle className="h-4 w-4 text-orange-500" />}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-xs text-gray-600">
              ...
              <span className={showWarning ? 'text-orange-600 font-medium' : ''}>
