...
export default function SharafDGDeploymentForm({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: partiesData, error: partiesError } = await supabase
        .from('parties')
        .select(
          'id, name_en, name_ar, crn, type, logo_url, status, signatory_name_en, signatory_name_ar, designation_id'
        )
        .eq('status', 'Active')
        .order('name_en');
      ...
      const { data: designationsData, error: designationsError } =
        await supabase
          .from('designations')
  ...
  const filteredPromoters = allPromoters.filter(p => {
    ...
    const matchesSearch = promoterSearchTerm
      ? p.name_en?.toLowerCase().includes(promoterSearchTerm.toLowerCase()) ||
        ...
        p.id_card_number
          ?.toLowerCase()
          .includes(promoterSearchTerm.toLowerCase())
      : true;
  ...
  const validateForm = (): boolean => {
    ...
    if (selectedPromoter) {
      ...
      if (!hasValidIdCard)
        errors.push('Promoter ID Card Image (valid, not placeholder)');
      if (!hasValidPassport)
        errors.push('Promoter Passport Image (valid, not placeholder)');
      if (!selectedPromoter.id_card_number)
        errors.push('Promoter ID Card Number');
      if (!selectedPromoter.passport_number)
        errors.push('Promoter Passport Number');
    }

    // Validate parties have required data
    if (selectedClient && !selectedClient.name_ar)
      errors.push('Client Arabic Name');
    if (selectedClient && !selectedClient.crn) errors.push('Client CRN');
    if (selectedEmployer && !selectedEmployer.name_ar)
      errors.push('Employer Arabic Name');
    if (selectedEmployer && !selectedEmployer.crn) errors.push('Employer CRN');
    if (selectedSupplier && !selectedSupplier.name_ar)
      errors.push('Supplier/Brand Arabic Name');
    ...
    try {
      ...
      const contractData = {
        ...
        title:
          ...
          `Sharaf DG Deployment - ${selectedPromoter?.name_en}`,
        description:
          formData.special_terms ||
          `${formData.job_title} at ${formData.work_location}`,
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const webhookData = {
        ...
        second_party_signatory_name_en:
          selectedEmployer?.signatory_name_en || '',
        second_party_signatory_name_ar:
      ...
      if (
        ...
        safeWebhookData.has_id_card_image =
          !!safeWebhookData.promoter_id_card_url;
      }
      ...
      if (
        ...
        safeWebhookData.has_passport_image =
          !!safeWebhookData.promoter_passport_url;
      }

      // Add flags for Make.com to conditionally replace images
      safeWebhookData.skip_id_card_replacement =
        !safeWebhookData.has_id_card_image;
      safeWebhookData.skip_passport_replacement =
        !safeWebhookData.has_passport_image;
      ...
      if (!safeWebhookData.first_party_logo) {
        safeWebhookData.first_party_logo =
          safeWebhookData.first_party_logo_url || placeholderImages.logo;
      }
      if (!safeWebhookData.second_party_logo) {
        safeWebhookData.second_party_logo =
          safeWebhookData.second_party_logo_url || placeholderImages.logo;
      }
      ...
      toast({
        ...
        description:
          'Your deployment letter is being generated via Make.com. This will take about 30 seconds.',
      });
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const { data: contract, error } = await supabase
          .from('contracts')
          .select(
            'id, contract_number, status, pdf_url, google_doc_url, terms, notes'
          )
        ...
        if (contract?.pdf_url) {
          ...
          toast({
            title: 'PDF Ready!',
            description:
              'Your deployment letter has been generated successfully.',
          });
          return;
        }
        ...
        if (contract?.notes) {
          try {
            ...
            if (urlMatch) {
              ...
              const driveMatch = contract.notes.match(
        ...
        if (pollCount >= maxPolls) {
          ...
          toast({
            ...
            description:
              'The PDF generation is taking longer than expected. Please check Make.com or try generating again.',
            variant: 'destructive',
          });
        }
      } catch (error) {
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <CardDescription className='mt-2'>
                Generate professional bilingual deployment letters for Sharaf DG
                with automated PDF creation
              </CardDescription>
            </div>
            {lastSaved && (
      ...
      <form onSubmit={handleSubmit} className='space-y-6'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Select employer first, then choose promoter from that employer's
              list (must have ID card and passport images)
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-4'>
            ...
            {!formData.second_party_id && (
              <Alert className='border-amber-200 bg-amber-50'>
                ...
                <AlertDescription className='text-amber-900'>
                  Please select <strong>Second Party (Employer)</strong> first
                  to see promoters from that employer.
                </AlertDescription>
              </Alert>
            )}
            ...
            {selectedPromoter &&
              (() => {
                ...
                const allValid =
                  hasValidIdCard && hasValidPassport && hasPassportNumber;
                      ...
                      <div className='space-y-2 text-sm'>
                        <div className='grid grid-cols-2 gap-2'>
                          ...
                          <div className='text-right'>
                            <strong>الاسم:</strong> {selectedPromoter.name_ar}
                          </div>
                        </div>
                        ...
                        <div>
                          <strong>Email:</strong>{' '}
                          {selectedPromoter.email || 'Not provided'}
                        </div>
                        <div>
                          <strong>Mobile:</strong>{' '}
                          {selectedPromoter.mobile_number || 'Not provided'}
                        </div>
                            ...
                            <AlertDescription>
                              <div className='space-y-1'>
                                {!allValid && (
                                  <div>
                                    This promoter has{' '}
                                    {!hasValidIdCard && !hasValidPassport
                                      ? 'missing or placeholder images'
                                      : !hasValidIdCard
                                        ? 'missing or placeholder ID card'
                                        : 'missing or placeholder passport'}
                                    . Please upload valid images before
                                    proceeding.
                                  </div>
                                )}
                                {!selectedPromoter.passport_number && (
                                  <div className='font-semibold'>
                                    ⚠️ Passport Number is required for Sharaf DG
                                    deployment letters.
                                  </div>
                                )}
                              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              First Party (Sharaf DG) is fixed for this form. Select employer
              and supplier/brand.
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-4'>
            ...
            <div className='space-y-2'>
              ...
              {selectedClient ? (
                <div className='border rounded-md p-3 bg-muted/50 flex items-center justify-between'>
                  <div className='flex items-center gap-3'>
                    ...
                    <div className='flex flex-col'>
                      ...
                      <span className='text-sm text-muted-foreground'>
                        {selectedClient.name_ar} • CRN:{' '}
                        {selectedClient.crn || 'N/A'}
                      </span>
                    </div>
                  </div>
                ...
                <Alert className='border-amber-200 bg-amber-50'>
                  <AlertTriangle className='h-4 w-4 text-amber-600' />
                  <AlertDescription className='text-amber-900'>
                    Sharaf DG client not found in database. Please add "Sharaf
                    DG" as a Client type party.
                  </AlertDescription>
                </Alert>
              )}
              <p className='text-xs text-muted-foreground'>
                This form is specifically for Sharaf DG deployment letters. The
                client is automatically set to Sharaf DG.
              </p>
            </div>
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      <div className='text-sm space-y-1'>
                        ...
                        <div className='text-xs text-muted-foreground'>
                          Signatory:{' '}
                          {selectedClient.signatory_name_en || 'Not provided'}
                          {selectedClientDesignation.en &&
                            ` (${selectedClientDesignation.en})`}
                        </div>
                        {selectedClient.signatory_name_ar && (
                          <div className='text-right text-xs text-muted-foreground'>
                            {selectedClient.signatory_name_ar}
                            {selectedClientDesignation.ar &&
                              ` (${selectedClientDesignation.ar})`}
                          </div>
                        )}
                      </div>
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      <div className='text-sm space-y-1'>
                        ...
                        <div className='text-xs text-muted-foreground'>
                          Signatory:{' '}
                          {selectedEmployer.signatory_name_en || 'Not provided'}
                          {selectedEmployerDesignation.en &&
                            ` (${selectedEmployerDesignation.en})`}
                        </div>
                        {selectedEmployer.signatory_name_ar && (
                          <div className='text-right text-xs text-muted-foreground'>
                            {selectedEmployer.signatory_name_ar}
                            {selectedEmployerDesignation.ar &&
                              ` (${selectedEmployerDesignation.ar})`}
                          </div>
                        )}
                      </div>
                    ...
                    <AlertDescription>
                      <div className='text-sm space-y-1'>
                        ...
                        <div className='text-xs text-blue-900/70'>
                          Signatory:{' '}
                          {selectedSupplier.signatory_name_en || 'Not provided'}
                          {selectedSupplierDesignation.en &&
                            ` (${selectedSupplierDesignation.en})`}
                        </div>
                        {selectedSupplier.signatory_name_ar && (
                          <div className='text-right text-xs text-blue-900/70'>
                            {selectedSupplier.signatory_name_ar}
                            {selectedSupplierDesignation.ar &&
                              ` (${selectedSupplierDesignation.ar})`}
                          </div>
                        )}
                      </div>
        ...
        <Card id='pdf-section' className='border-green-200 bg-green-50/50'>
          <CardHeader>
            ...
            <CardDescription className='text-green-800'>
              Contract Number: <strong>{formData.contract_number}</strong> has
              been saved. Now generate the deployment letter PDF.
            </CardDescription>
          </CardHeader>
          <CardContent className='space-y-4'>
            {pdfStatus === 'idle' && (
              <div className='space-y-3'>
                <Alert className='border-blue-200 bg-blue-50'>
                  ...
                  <AlertDescription className='text-blue-900'>
                    <div className='space-y-2'>
                      <div className='font-semibold'>
                        Ready to generate PDF deployment letter
                      </div>
                ...
                <AlertDescription>
                  <div className='space-y-3'>
                    ...
                    <div className='text-sm text-green-800'>
                      Your bilingual PDF has been generated with employee images
                      embedded.
                    </div>
              ...
              <Alert variant='destructive'>
                ...
                <AlertDescription>
                  <div className='space-y-2'>
                    ...
                    <div className='text-sm'>
                      Please check that all required data is present and try
                      again. Ensure promoter has valid ID card and passport
                      images.
                    </div>
                    <Button
                      onClick={handleGeneratePDF}
      ...
      <Card className='border-blue-200 bg-blue-50/50'>
        ...
        <CardContent>
          <div className='text-sm text-blue-900/80 space-y-2'>
            <p>
              This form creates official deployment letters for promoters
              assigned to Sharaf DG locations.
            </p>
