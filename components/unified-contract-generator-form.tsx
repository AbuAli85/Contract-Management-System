        ...
        this.props.fallback || (
          <div className='p-8 text-center'>
            ...
            <p className='text-gray-600 mb-4'>
              The contract form encountered an error.
            </p>
            <button
...
function UnifiedContractGeneratorForm({
  ...
  const watchedSecondPartyId = useWatch({
    control: form.control,
  ...
  const promoterOptions = useMemo(() => {
    ...
    if (watchedSecondPartyId) {
      filteredPromoters = (promoters as Promoter[]).filter((p: Promoter) => {
        ...
        return (
          p.employer_id?.toString() === watchedSecondPartyId ||
  ...
  useEffect(() => {
    if (watchedPromoterId && promoters && Array.isArray(promoters)) {
      const selectedPromoter = (promoters as Promoter[]).find(
        (p: Promoter) => p.id.toString() === watchedPromoterId
      );
      if (selectedPromoter) {
  ...
  useEffect(() => {
    if (
      watchedContractType &&
      watchedContractType !== form.getValues('contract_type')
    ) {
      // Get template configuration based on contract type
      const selectedTemplate = CONTRACT_TYPES.find(
        type => type.value === watchedContractType
      );
      ...
      if (selectedTemplate) {
        ...
        switch (watchedContractType) {
          case 'unlimited-contract':
            ...
            if (!currentValues.probation_period_months)
              form.setValue('probation_period_months', 3);
            if (!currentValues.notice_period_days)
              form.setValue('notice_period_days', 30);
            if (!currentValues.working_hours_per_week)
              form.setValue('working_hours_per_week', 40);
            break;
          ...
          case 'oman-fixed-term-makecom':
            ...
            if (
              !currentValues.contract_end_date &&
              currentValues.contract_start_date
            ) {
            ...
            if (!currentValues.probation_period_months)
              form.setValue('probation_period_months', 2);
            if (!currentValues.notice_period_days)
              form.setValue('notice_period_days', 14);
            if (!currentValues.working_hours_per_week)
              form.setValue('working_hours_per_week', 40);
            break;
          ...
          case 'oman-part-time-makecom':
            ...
            if (!currentValues.working_hours_per_week)
              form.setValue('working_hours_per_week', 20);
            if (!currentValues.probation_period_months)
              form.setValue('probation_period_months', 1);
            if (!currentValues.notice_period_days)
              form.setValue('notice_period_days', 14);
            break;
          ...
          case 'probationary':
            ...
            if (!currentValues.notice_period_days)
              form.setValue('notice_period_days', 7);
            if (!currentValues.working_hours_per_week)
              form.setValue('working_hours_per_week', 40);
            break;
          ...
          case 'training-contract':
            ...
            if (!currentValues.notice_period_days)
              form.setValue('notice_period_days', 7);
            if (!currentValues.working_hours_per_week)
              form.setValue('working_hours_per_week', 35);
          ...
          case 'project-based':
            ...
            if (!currentValues.notice_period_days)
              form.setValue('notice_period_days', 7);
            if (!currentValues.working_hours_per_week)
              form.setValue('working_hours_per_week', 30);
            form.setValue('probation_period_months', 0);
            break;
          ...
          case 'temporary':
            ...
            form.setValue('probation_period_months', 0);
            if (!currentValues.notice_period_days)
              form.setValue('notice_period_days', 3);
            if (!currentValues.working_hours_per_week)
              form.setValue('working_hours_per_week', 25);
            break;
          ...
          case 'director':
            ...
            if (!currentValues.probation_period_months)
              form.setValue('probation_period_months', 6);
            if (!currentValues.notice_period_days)
              form.setValue('notice_period_days', 60);
            if (!currentValues.working_hours_per_week)
              form.setValue('working_hours_per_week', 45);
            if (!currentValues.basic_salary)
              form.setValue('basic_salary', 2000); // Higher base salary
            break;

          case 'remote-work':
            // Remote work contract - flexible arrangements
            if (!currentValues.work_location)
              form.setValue('work_location', 'remote');
            if (!currentValues.working_hours_per_week)
              form.setValue('working_hours_per_week', 40);
            if (!currentValues.probation_period_months)
              form.setValue('probation_period_months', 2);
        ...
        toast.success(`Template applied: ${selectedTemplate.label}`, {
          description:
            'Form fields have been pre-filled based on the selected contract type.',
          duration: 3000,
        });
      }
    }
  ...
  if (hasNonAuthErrors) {
    return (
      <div className='p-8 text-center'>
        ...
        <p className='text-gray-600 mb-4'>
          Unable to load required data for the contract form.
        </p>
        <button
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Choose the type of employment contract. Template settings
              ...
              >
                <div className='flex items-center gap-2 mb-2'>
                  ...
                  <h4 className='text-sm font-medium text-blue-800'>
                    Template Applied:{' '}
                    {
                      CONTRACT_TYPES.find(t => t.value === watchedContractType)
                        ?.label
                    }
                  </h4>
                </div>
                <p className='text-xs text-blue-600 mb-2'>
                  The following defaults have been applied based on your
                  selection:
                </p>
                <div className='grid grid-cols-2 gap-2 text-xs text-blue-700'>
                  ...
                  {(watchedContractType === 'limited-contract' ||
                    watchedContractType === 'oman-fixed-term-makecom') && (
                    <>
                  ...
                  {(watchedContractType === 'part-time-contract' ||
                    watchedContractType === 'oman-part-time-makecom') && (
                    <>
                  ...
                  {(watchedContractType === 'internship' ||
                    watchedContractType === 'training-contract') && (
                    <>
                  ...
                  {(watchedContractType === 'consulting' ||
                    watchedContractType === 'freelance' ||
                    watchedContractType === 'project-based') && (
                    <>
                  ...
                  {(watchedContractType === 'seasonal' ||
                    watchedContractType === 'temporary') && (
                    <>
                  ...
                  {(watchedContractType === 'executive' ||
                    watchedContractType === 'management' ||
                    watchedContractType === 'director') && (
                    <>
        ...
        <div className='mb-6'>
          <div className='mb-2 flex items-center justify-between'>
            ...
            <span className='text-sm text-gray-500'>
              {isNaN(formProgress) ? 0 : formProgress}%
            </span>
          </div>
          <Progress
            value={isNaN(formProgress) ? 0 : formProgress}
            className='h-2'
          />
        </div>
                ...
                <div className='flex-1'>
                  ...
                  <div className='mt-1 flex gap-4 text-sm text-blue-700'>
                    ...
                    {contractInsights.totalCompensation > 0 &&
                      !isNaN(contractInsights.totalCompensation) && (
                        <span>
                          Total: {contractInsights.totalCompensation} OMR
                        </span>
                      )}
                    <Badge
                      variant={
                        contractInsights.isShortTerm ? 'secondary' : 'default'
                      }
                    >
                    ...
                    <FormItem>
                      ...
                      {watchedSecondPartyId && promoterOptions.length === 0 && (
                        <p className='text-sm text-amber-600 mt-2'>
                          ...
                          Please select a different employer or add promoters to
                          this employer.
                        </p>
                      )}
                      {watchedSecondPartyId && promoterOptions.length > 0 && (
                        <p className='text-sm text-green-600 mt-2'>
                          ✅ {promoterOptions.length} promoter(s) available for
                          this employer
                        </p>
                      )}
                    </FormItem>
            ...
            <Alert className='border-blue-200 bg-blue-50'>
              <AlertDescription>
                <div className='text-xs space-y-1'>
                  ...
                  <div>
                    Submit Pending: {submitMutation.isPending ? '🔄' : '✅'}
                  </div>
...
function UnifiedContractGeneratorFormSafe(
  props: UnifiedContractGeneratorFormProps
) {
  return (
    <ErrorBoundary>
