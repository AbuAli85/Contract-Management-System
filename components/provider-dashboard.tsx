...
import { Calendar, DollarSign, Users, TrendingUp, Clock, Star, MapPin, Phone, Mail } from 'lucide-react'
...
export function ProviderDashboard({ user }: ProviderDashboardProps) {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        const totalRevenue = bookingsData?.reduce((sum, b) => sum + (b.quoted_price || 0), 0) || 0
        ...
        setStats({
          ...
          active_services: servicesData?.filter(s => s.status === 'active').length || 0,
  ...
  useEffect(() => {
    console.log('🔔 Setting up real-time bookings subscription for provider:', user.id)
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatCurrency(stats.this_month_revenue)}</div>
              <p className="text-xs text-muted-foreground">
