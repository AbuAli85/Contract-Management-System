  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const statusCounts = CONTRACT_STATUSES.reduce(
        (acc, status) => {
          acc[status.value] = contracts.filter(
      ...
      const contractsWithValue = contracts.filter(
        c => c.contract_value && c.contract_value > 0
      );
      const totalValue = contractsWithValue.reduce(
        (sum, c) => sum + (c.contract_value || 0),
        0
      );
      const averageValue =
        contractsWithValue.length > 0
          ? totalValue / contractsWithValue.length
          : 0;
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Progress
              value={
                (analytics.activeContracts / analytics.totalContracts) * 100
              }
              className='mt-2'
            />
          </CardContent>
      ...
      <div className='grid gap-4 md:grid-cols-2'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Current status breakdown of all contracts
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className='h-64'>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Monthly contract creation over time
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className='h-64'>
