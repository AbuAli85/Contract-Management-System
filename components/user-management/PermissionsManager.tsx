...
export default function PermissionsManager({ 
  ...
  const handleRoleChange = (newRole: string) => {
    ...
    setSelectedPermissions(new Set(ROLE_PERMISSIONS[newRole as keyof typeof ROLE_PERMISSIONS] || []));
  };
  ...
  const isCategoryPartiallySelected = (category: string) => {
    const categoryPermissions = permissionsByCategory[category].map(p => p.id);
    const selectedCount = categoryPermissions.filter(permissionId => selectedPermissions.has(permissionId)).length;
