...
const navigationItems: NavigationItem[] = [
  ...
  },
  {
    ...
    subItems: [
      ...
      { name: 'Draft Contracts', href: '/contracts?status=draft', icon: FileText },
      { name: 'Active Contracts', href: '/contracts?status=active', icon: FileText },
      { name: 'Expired Contracts', href: '/contracts?status=expired', icon: FileText }
    ]
  },
  {
    name: 'Documents',
  ...
  },
  {
    ...
    subItems: [
      ...
      { name: 'Delivery Tracking', href: '/tracking/deliveries', icon: Package },
  ...
  },
  {
    ...
    subItems: [
      ...
      { name: 'Starred', href: '/notifications?filter=starred', icon: Bookmark },
...
export function AdvancedNavigation({ className, onItemClick }: AdvancedNavigationProps) {
  ...
  const NavigationLink = ({ item, isSubItem = false }: { item: NavigationItem; isSubItem?: boolean }) => {
      ...
      <div className="p-4 border-b bg-gradient-to-r from-blue-50 to-indigo-50">
        <div className="text-center">
          <div className="flex items-center justify-center gap-1 mb-2">
            ...
            <span className="text-sm font-semibold text-blue-900">🚀 NEW FEATURES</span>
          </div>
              ...
              {item.subItems && isActiveRoute(item.href) && (
                ...
                >
                  {item.subItems.map((subItem) => (
                    <NavigationLink key={subItem.name} item={subItem} isSubItem />
                  ))}
                </motion.div>
              )}
            </div>
