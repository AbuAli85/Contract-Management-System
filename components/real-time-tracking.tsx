...
export function RealTimeTracking({ bookingId, userRole, userId }: RealTimeTrackingProps) {
  ...
  useEffect(() => {
    const loadTrackingData = async () => {
      try {
        ...
        const { data: trackingCurrentData, error: trackingError } = await supabase
          .from('trackings')
          .select('*')
  ...
  useEffect(() => {
    console.log('🔔 Setting up real-time tracking subscription for booking:', bookingId)
      ...
      .on('postgres_changes', 
        ...
        (payload) => {
          ...
          setTrackingHistory(current => {
            ...
            if (exists) {
              return current.map(t => t.id === updatedTracking.id ? updatedTracking : t)
            } else {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'service_started': return 'bg-green-100 text-green-800'
      case 'service_completed': return 'bg-green-100 text-green-800'
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  {trackingData.notes && (
                    <p className="text-sm text-gray-600 mt-1">{trackingData.notes}</p>
                  )}
                </div>
              </div>
              ...
              {trackingData.location_address && (
                <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                  ...
                  <span className="text-sm">{trackingData.location_address}</span>
                </div>
              )}

              {/* Estimated/Actual Arrival */}
              {(trackingData.estimated_arrival || trackingData.actual_arrival) && (
                <div className="grid grid-cols-2 gap-4 text-sm">
                  {trackingData.estimated_arrival && (
                    <div>
                      <span className="font-medium">Estimated Arrival:</span>
                      <p className="text-gray-600">{formatDateTime(trackingData.estimated_arrival)}</p>
                    </div>
                  )}
                  {trackingData.actual_arrival && (
                    <div>
                      <span className="font-medium">Actual Arrival:</span>
                      <p className="text-gray-600">{formatDateTime(trackingData.actual_arrival)}</p>
                    </div>
                  )}
                </div>
              )}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="text-xs text-gray-500 bg-blue-50 p-3 rounded-lg">
              💡 <strong>Tip:</strong> Status updates trigger automatic notifications to the client 
              and team via email, Slack, and real-time dashboard updates.
            </div>
          </CardContent>
        </Card>
      )}
