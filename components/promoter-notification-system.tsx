...
export function PromoterNotificationSystem({ promoters }: PromoterNotificationSystemProps) {
  ...
  const [statusFilter, setStatusFilter] = useState<'all' | 'pending' | 'sent' | 'delivered' | 'failed'>('all');
  ...
  const unassignedPromoters = useMemo(() => {
    return promoters.filter(promoter => promoter.assignmentStatus === 'unassigned');
  }, [promoters]);
  ...
  const filteredNotifications = useMemo(() => {
    return notifications.filter(notification => {
      const matchesSearch = !searchTerm || 
        notification.promoterName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const handleSendNotification = useCallback(async () => {
    if (selectedPromoters.size === 0) {
      toast({
        ...
        description: 'Please select at least one promoter to send notifications.',
      });
      return;
    }
    ...
    try {
      ...
      const selectedPromoterData = promoters.filter(p => selectedPromoters.has(p.id));
      
      const newNotifications: Notification[] = selectedPromoterData.map(promoter => {
        let message = customMessage || template?.message || 'Notification message';
  ...
  }, [selectedPromoters, templates, selectedTemplate, customMessage, deliveryMethod, priority, promoters, toast]);

  // Handle bulk actions
  const handleBulkAction = useCallback((action: 'select_all' | 'select_expiring' | 'select_unassigned' | 'clear') => {
    switch (action) {
      case 'select_all':
        setSelectedPromoters(new Set(promoters.map(p => p.id)));
        break;
      case 'select_expiring':
        setSelectedPromoters(new Set(promotersNeedingAttention.map(p => p.id)));
        break;
  ...
  const handleCreateTemplate = useCallback((templateData: Omit<NotificationTemplate, 'id' | 'createdAt'>) => {
                  ...
                  <div className='space-y-4'>
                    <div className='flex items-center justify-between p-4 border rounded-lg'>
                      <div>
                        ...
                        <p className='text-sm text-muted-foreground'>
                          Automatically notify promoters when documents are expiring
                        </p>
                      </div>
                      <Button variant='outline' size='sm'>
                        <Settings className='mr-2 h-4 w-4' />
                    ...
                    <div className='flex items-center justify-between p-4 border rounded-lg'>
                      <div>
                        ...
                        <p className='text-sm text-muted-foreground'>
                          Notify promoters when they are assigned to new companies
                        </p>
                      </div>
                      <Button variant='outline' size='sm'>
                        <Settings className='mr-2 h-4 w-4' />
  ...
  return (
    <div className='flex items-center justify-between p-4 border rounded-lg'>
      <div className='flex items-center gap-4'>
        <div className={cn('rounded-full p-2', statusColors[notification.status])}>
    ...
    <Card>
      ...
      <CardContent className='pt-0'>
        <div className='space-y-3'>
          ...
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-2'>
              <div className={cn('w-2 h-2 rounded-full', template.isActive ? 'bg-green-500' : 'bg-slate-300')} />
    ...
    <form onSubmit={handleSubmit} className='space-y-4'>
      ...
      <div className='space-y-2'>
        ...
        <p className='text-xs text-muted-foreground'>
          Available variables: {`{{promoter_name}}, {{company_name}}, {{document_type}}, {{days_remaining}}, {{expiry_date}}`}
        </p>
      </div>
