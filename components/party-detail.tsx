...
export default function PartyDetail({ partyId }: PartyDetailProps) {
  ...
  useEffect(() => {
    if (user) {
      ...
      supabase
        ...
        .eq('party_id', partyIdStr)
        .then(({ data }) =>
          setActivities((data as unknown as PartyActivity[]) || [])
        );
      supabase
        .from('parties')
        .select('owner_id')
  ...
  const addNote = async () => {
    ...
    const supabase = createClient();
    await supabase.from('party_notes').insert({
  ...
  const addTag = async () => {
    ...
    await supabase
      .from('party_tags')
      .insert({ party_id: partyId.toString(), tag: tagInput });
    setTagInput('');
    supabase
      .from('party_tags')
      .select('*')
