  ...
  createClient: vi.fn(() => ({
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
          single: vi.fn(() =>
            Promise.resolve({ data: mockContract, error: null })
          ),
        })),
      })),
      insert: vi.fn(() => ({
...
describe('Contracts Module - Data Mapping Validation', () => {
  describe('validateAndMapContractData', () => {
    it('should validate and map contract data successfully', () => {
      ...
      expect(result.mappedFields).toHaveProperty(
        'job_title',
        'Software Developer'
      );
      expect(result.mappedFields).toHaveProperty(
    ...
    it('should apply transformations correctly', () => {
      ...
      expect(result.mappedFields.contract_start_date).toMatch(
        /\d{2}\/\d{2}\/\d{4}/
      );
      expect(result.mappedFields.contract_end_date).toMatch(
        /\d{2}\/\d{2}\/\d{4}/
      );
    ...
    it('should generate contract number automatically', () => {
      ...
      expect(result.mappedFields.contract_number).toMatch(
        /^PAC-\d{8}-[A-Z0-9]{4}$/
      );
    });
  });
  ...
  describe('STANDARD_TEMPLATE_PLACEHOLDERS', () => {
    ...
    it('should have proper validation rules', () => {
      const jobTitlePlaceholder = STANDARD_TEMPLATE_PLACEHOLDERS.find(
        p => p.key === 'job_title'
      );
...
describe('Contracts Module - Export Error Handling', () => {
  describe('ExportErrorCode enum', () => {
    it('should contain all expected error codes', () => {
      ...
      expect(ExportErrorCode.MISSING_REQUIRED_FIELDS).toBe(
        'MISSING_REQUIRED_FIELDS'
      );
      expect(ExportErrorCode.TEMPLATE_NOT_FOUND).toBe('TEMPLATE_NOT_FOUND');
      expect(ExportErrorCode.PDF_GENERATION_FAILED).toBe(
    ...
    it('should generate PDF when requested', async () => {
      ...
      global.fetch = vi.fn(() =>
        Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve({ pdfUrl: 'https://test.com/contract.pdf' }),
        })
      ) as any;
  ...
  describe('Signature validation', () => {
    it('should validate signer types', () => {
      ...
      validSignerTypes.forEach(type => {
        expect(['first_party', 'second_party', 'promoter', 'admin']).toContain(
          type
        );
      });
    });
...
describe('Contracts Module - Integration Tests', () => {
  describe('End-to-end contract workflow', () => {
    it('should handle complete contract lifecycle', async () => {
      ...
      const generationResult = await contractService.generateContract(
        mockFormData,
        {
          validateDataMapping: true,
          generatePdf: true,
      ...
      expect(
        listResult.data?.contracts.some(
          c => c.id === generationResult.details!.contractId
        )
      ).toBe(true);
    });
  });
  ...
  describe('Error handling scenarios', () => {
    ...
    it('should handle validation errors with detailed feedback', async () => {
      ...
      expect(result.validation?.missingRequiredFields.length).toBeGreaterThan(
        0
      );
    });
  });
