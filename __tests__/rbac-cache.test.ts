...
describe('RBAC Permission Cache', () => {
  ...
  describe('Cache API Methods', () => {
    ...
    it('should have invalidateUsers method for multiple users', async () => {
      ...
      // Test that it doesn't throw
      await expect(
        cache.invalidateUsers(['user1', 'user2'])
  ...
  describe('Cache Statistics', () => {
    ...
    it('should track cache size correctly', async () => {
      ...
      const mockPermissions = [{ permissions: { name: 'user:read:own' } }];
      ...
      mockSupabase.from.mockReturnValue({
        select: jest.fn(() => ({
          eq: jest.fn(() => ({
            is: jest.fn(() => ({
              single: jest.fn(() =>
                Promise.resolve({ data: mockRoleAssignments, error: null })
              ),
            })),
          })),
        })),
      ...
      mockSupabase.from.mockReturnValue({
        select: jest.fn(() => ({
          in: jest.fn(() =>
            Promise.resolve({ data: mockPermissions, error: null })
          ),
        })),
      });
      ...
      mockSupabase.from.mockReturnValue({
        select: jest.fn(() => ({
          eq: jest.fn(() => ({
            is: jest.fn(() => ({
              single: jest.fn(() =>
                Promise.resolve({ data: null, error: 'Database error' })
              ),
            })),
          })),
        })),
