...
import { AuditLogger, PermissionUsageData, RoleChangeData } from '../lib/rbac/audit'
...
jest.mock('../lib/supabase/server', () => ({
  createClient: jest.fn(() => ({
    from: jest.fn(() => ({
      insert: jest.fn(() => ({
        select: jest.fn(() => ({
          single: jest.fn(() => Promise.resolve({ data: { id: 'audit123' }, error: null }))
        }))
      })),
      select: jest.fn(() => ({
        eq: jest.fn(() => ({
...
describe('RBAC Audit Logging', () => {
  ...
  describe('Audit Log Retrieval', () => {
    ...
    it('should retrieve audit logs by event type', async () => {
      ...
      const logs = await auditLogger.getAuditLogsByType('PERMISSION_CHECK', 10, 0)
        ...
        headers: {
          get: jest.fn((header: string) => {
            switch (header) {
              ...
              case 'x-real-ip': return '192.168.1.2'
              case 'cf-connecting-ip': return '192.168.1.3'
              default: return null
            }
          })
        }
