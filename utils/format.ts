...
export const calculateDuration = (
  ...
  try {
    ...
    if (diffDays < 7) {
      result = shortFormat ? `${diffDays}d` : `${diffDays} day${diffDays !== 1 ? 's' : ''}`
    } else if (diffDays < 30 && includeWeeks) {
      const weeks = Math.floor(diffDays / 7)
      const remainingDays = diffDays % 7
      if (shortFormat) {
        result = remainingDays > 0 ? `${weeks}w ${remainingDays}d` : `${weeks}w`
      } else {
        result = remainingDays > 0 ? `${weeks} week${weeks !== 1 ? 's' : ''}, ${remainingDays} day${remainingDays !== 1 ? 's' : ''}` : `${weeks} week${weeks !== 1 ? 's' : ''}`
      }
...
export const formatCurrencyLegacy = (amount?: number, currency?: string | null): string => {
...
export const calculateDurationLegacy = (startDate?: string | null, endDate?: string | null): string => {
