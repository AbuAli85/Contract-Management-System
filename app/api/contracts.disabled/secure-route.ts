      ...
      if (userProfile?.role === 'admin') {
        processedContracts = await Promise.all(
          contracts?.map(contract =>
            databaseSecurity.decryptSensitiveFields(
              contract,
...
export const POST = withSecurity(
  async (request: NextRequest) => {
    try {
      ...
      const validations = {
        job_title: validateInput(body.job_title, {
          ...VALIDATION_PRESETS.name,
          maxLength: 200,
        }),
        work_location: validateInput(body.work_location, {
          type: 'string',
          maxLength: 200,
          noHTML: true,
        }),
        contract_value: validateInput(body.contract_value, {
          type: 'number',
          min: 0,
        }),
      };
      ...
      const validationErrors = Object.entries(validations)
        ...
        .map(
          ([field, validation]) => `${field}: ${validation.errors.join(', ')}`
        );
      ...
      // Log the creation
      await auditLogger.logContractAction(
        'created',
        user?.id || '',
        newContract.id,
        null,
