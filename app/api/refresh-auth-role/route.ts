...
export async function POST(request: NextRequest) {
  try {
    console.log('=== REFRESH AUTH ROLE START ===')
    ...
    console.log('=== REFRESH AUTH ROLE COMPLETE ===')
    ...
    return NextResponse.json({
      ...
      authRefresh: {
        required: true,
        instructions: 'Auth provider role refreshed. Please refresh the page to see updated role.'
      },
      summary: {
        finalRole: currentRole,
        roleSource: roleSource,
        message: `Auth role refreshed. Current role: ${currentRole} (from ${roleSource})`
      }
    })

  } catch (error) {
    console.error('=== REFRESH AUTH ROLE ERROR ===')
    console.error('Unexpected error:', error)
    
    return NextResponse.json({
      success: false,
      error: 'Refresh auth role failed',
