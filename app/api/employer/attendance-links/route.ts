...
export const POST = withRBAC(
  ...
  async (request: NextRequest) => {
    try {
        ...
        return NextResponse.json(
          {
            ...
            details: 'Please select an active company in your profile settings',
          },
          { status: 400 }
        );
      }
        ...
        return NextResponse.json(
          {
            ...
            details: `Your role (${userProfile.role}) does not have permission to create attendance links. Required: admin, employer, or manager`,
          },
          { status: 403 }
        );
      }
      ...
      const { data: link, error: createError } = await (
  ...
  async (request: NextRequest) => {
    try {
      ...
      if (!activeCompanyId) {
        ...
        const { data: userCompanies } = await (
          ...
          return NextResponse.json(
            {
              error: 'No active company found',
              message:
                'Please set your active company in profile settings or contact support.',
