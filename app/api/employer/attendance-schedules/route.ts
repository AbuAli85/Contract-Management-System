...
export const POST = withRBAC('attendance:create:all', async (
  ...
  try {
    ...
    if (!office_location_id && (!target_latitude || !target_longitude)) {
      return NextResponse.json(
        { error: 'Either office location or custom coordinates must be provided' },
        { status: 400 }
      );
    }
    ...
    const nextGenerationDate = checkInDateTime > now 
      ? new Date().toISOString().split('T')[0]
      : new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0];

    // Create the schedule
    const { data: schedule, error: createError } = await (supabaseAdmin.from('attendance_link_schedules') as any)
      .insert({
        ...
        send_check_in_link,
        send_check_out_link,
        notification_method: Array.isArray(notification_method) ? notification_method : [notification_method],
        send_before_minutes,
        send_to_all_employees,
        specific_employee_ids: assignment_type === 'selected' && specific_employee_ids.length > 0 ? specific_employee_ids : null,
        employee_group_ids: (assignment_type === 'groups' || assignment_type === 'location_based') && employee_group_ids.length > 0 ? employee_group_ids : null,
        assignment_type: assignment_type || (send_to_all_employees ? 'all' : 'selected'),
    ...
    if (createError) {
      ...
      return NextResponse.json(
        { error: 'Failed to create attendance schedule', details: createError.message },
        { status: 500 }
      );
    }
