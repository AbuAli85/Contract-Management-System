    ...
    try {
      ...
      if (!ids.length || !action || !['approve', 'reject'].includes(action)) {
        return NextResponse.json(
          {
            error:
              'Invalid request. Provide attendance_id(s) and action (approve/reject)',
          },
          { status: 400 }
        );
      }
      ...
      const { data: attendanceRecords, error: attendanceError } = await (
      ...
      for (const attendance of attendanceRecords) {
        ...
        const isSameCompany =
          profile?.active_company_id === employerEmployee?.company_id;

        if (!isEmployer && !isAdmin && !isSameCompany) {
          return NextResponse.json(
            {
              error:
                'You do not have permission to approve one or more attendance records',
            },
            { status: 403 }
          );
        }
