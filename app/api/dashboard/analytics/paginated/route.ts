        ...
        supabase
          ...
          .then(({ data, count }) => {
            const overview = {
              ...
              pendingApprovals: data?.filter(c => c.status === 'pending_approval').length || 0,
              completedContracts: data?.filter(c => c.status === 'completed').length || 0,
              totalRevenue: data?.reduce((sum, c) => sum + (parseFloat(c.basic_salary) || 0), 0) || 0
            }
      ...
      queries.push(
        trendsQuery.then(({ data }) => {
          if (data) {
            ...
            responseData.trends = Array.from(groupedData.entries()).map(([date, metrics]) => ({
              date,
              ...metrics
            }))
          }
      ...
      queries.push(
        supabase
          ...
          .then(({ data }) => {
            ...
            responseData.breakdown.byStatus = Array.from(statusCounts.entries()).map(([status, count]) => ({
              status,
              count
            }))
          })
      ...
      queries.push(
        supabase
          ...
          .then(({ data }) => {
            ...
            responseData.breakdown.byType = Array.from(typeCounts.entries()).map(([type, count]) => ({
              type,
              count
            }))
          })
        ...
        supabase
          ...
          .then(({ data }) => {
            ...
            data?.forEach((contract: any) => {
              ...
              promoterCounts.set(promoterName, (promoterCounts.get(promoterName) || 0) + 1)
            })
            
            responseData.breakdown.byPromoter = Array.from(promoterCounts.entries())
