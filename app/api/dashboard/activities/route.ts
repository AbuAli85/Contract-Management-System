...
export async function GET(request: NextRequest) {
  try {
    ...
    if (userError) {
      console.warn('Activities API: User error (continuing anyway):', userError)
    }
    if (!user) {
    ...
    if (contractsActivity.data) {
      contractsActivity.data.forEach(contract => {
        activities.push({
          ...
          description: contract.status === 'pending' ? 'Pending approval' : 
                      contract.status === 'active' ? 'Activated' : 
    ...
    if (promotersActivity.data) {
      promotersActivity.data.forEach(promoter => {
        activities.push({
          ...
          description: promoter.status === 'active' ? 'Registered and active' : 
                      promoter.status === 'inactive' ? 'Set to inactive' : 
                      'Status updated',
          status: promoter.status,
          timestamp: promoter.updated_at,
          entity: promoter.name_en || promoter.name_ar || 'Unknown Promoter',
          action: promoter.created_at === promoter.updated_at ? 'registered' : 'updated',
          icon: 'Users',
