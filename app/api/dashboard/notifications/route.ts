  ...
  try {
    ...
    pendingContracts?.forEach((contract: any) => {
      ...
      notifications.push({
        ...
        entity: contract.parties?.[0]?.name_en || contract.parties?.[0]?.name_ar || 'Unknown Party',
        metadata: {
    ...
    const { data: expiringContracts } = await supabase
      ...
      .lte('end_date', new Date(now.getTime() + 60 * 24 * 60 * 60 * 1000).toISOString()) // Next 60 days
    ...
    expiringContracts?.forEach((contract: any) => {
      ...
      notifications.push({
        ...
        type: daysUntilEnd <= 7 ? 'error' : daysUntilEnd <= 30 ? 'warning' : 'info',
        category: 'contract',
        priority: daysUntilEnd <= 7 ? 'high' : daysUntilEnd <= 30 ? 'medium' : 'low',
    ...
    const { data: expiringIds } = await supabase
      ...
      .lte('visa_expiry_date', new Date(now.getTime() + 120 * 24 * 60 * 60 * 1000).toISOString()) // Next 120 days
    ...
    expiringIds?.forEach((promoter: any) => {
      ...
      notifications.push({
        ...
        type: daysUntilExpiry <= 7 ? 'error' : daysUntilExpiry <= 30 ? 'warning' : 'info',
        category: 'document',
