...
export async function GET() {
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY && !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    diagnostics.errors.push('❌ Neither NEXT_PUBLIC_SUPABASE_ANON_KEY nor SUPABASE_SERVICE_ROLE_KEY is set');
  }

  if (diagnostics.errors.length > 0) {
    diagnostics.status = 'error';
    diagnostics.message = 'Missing required Supabase environment variables. Please create .env.local file.';
