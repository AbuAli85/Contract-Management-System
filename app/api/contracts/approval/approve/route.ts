...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contract.current_reviewer_id && contract.current_reviewer_id !== user.id && !await hasAdminRole(user.id, supabase)) {
    ...
    if (nextReviewerId && nextReviewerId !== user.id) {
      await sendReviewNotification(nextReviewerId, contractId, nextStatus, supabase)
    }

    // Create audit log
    await createAuditLog(user.id, `contract_${action}`, 'contracts', contractId, {
...
async function getNextReviewer(status: string, supabase: any): Promise<string | null> {
  let roleType = ''
