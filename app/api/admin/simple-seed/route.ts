  ...
  try {
    ...
    if (!parties?.length || !promoters?.length) {
      return NextResponse.json(
        {
          error:
            'No parties or promoters found. Please add some parties and promoters first.',
    ...
    if (!clients.length || !employers.length) {
      return NextResponse.json(
        {
          error:
            'Need both Client and Employer type parties to create contracts',
    ...
    const sampleContract = {
      ...
      contract_end_date: new Date(now.getTime() + 365 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split('T')[0],
    ...
    return NextResponse.json(
      {
        error: 'Failed to create sample contract',
