...
export async function POST(request: NextRequest) {
  try {
    console.log('=== IMMEDIATE ROLE REFRESH START ===')
    ...
    console.log('=== IMMEDIATE ROLE REFRESH COMPLETE ===')
    ...
    return NextResponse.json({
      ...
      clientUpdate: {
        required: true,
        newRole: currentRole,
        instructions: 'Update client-side role state immediately'
      },
      summary: {
        ...
        message: `Role updated: ${currentRole} (from ${roleSource})`
      }
    })

  } catch (error) {
    console.error('=== IMMEDIATE ROLE REFRESH ERROR ===')
    console.error('Unexpected error:', error)
    
    return NextResponse.json({
      success: false,
      error: 'Immediate role refresh failed',
