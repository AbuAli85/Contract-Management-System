...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (validated.parties && typeof validated.parties === 'object') {
      ...
      const partyIds = [firstPartyId, secondPartyId, clientId, employerId].filter(Boolean) as string[];
      ...
      if (partyIds.length > 0) {
        ...
        if (invalidParties.length > 0) {
          ...
          const missingParties = partyIds.filter(id => !foundPartyIds.includes(id));
          if (missingParties.length > 0) {
            throw new McpError('NOT_FOUND', `Parties not found: ${missingParties.join(', ')}`);
          }
          ...
          throw new McpError('FORBIDDEN', `Parties do not belong to your company: ${invalidParties.join(', ')}`);
        }
      }
    }
    ...
    const templateId = (contract.metadata as any)?.template_id || validated.template_id;
