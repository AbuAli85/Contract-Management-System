...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!supabase || typeof supabase.from !== 'function') {
      console.error("❌ Booking Resources API: Using mock client - returning mock data")
    ...
    console.log("🔍 Booking Resources API: Fetching resources from database...")
    ...
    try {
      ...
      if (resourcesError) {
        console.warn("⚠️ Booking Resources API: Error fetching resources:", resourcesError.message)
        resources = []
      } else {
        resources = resourcesData || []
      }
    } catch (error) {
      console.warn("⚠️ Booking Resources API: Resource fetch failed, continuing with empty array")
      resources = []
    }

    console.log(`✅ Booking Resources API: Successfully fetched ${resources?.length || 0} resources`)
