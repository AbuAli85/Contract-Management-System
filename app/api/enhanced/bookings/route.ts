...
export async function GET(request: NextRequest) {
  try {
    ...
    if (role === 'client' || role === 'user') {
      ...
      countQuery = countQuery.or(`client_id.eq.${user.id},provider_id.eq.${user.id}`)
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const hoursInAdvance = (scheduledStart.getTime() - now.getTime()) / (1000 * 60 * 60)
    ...
    const { data: conflicts } = await supabase
      ...
      .or(`and(scheduled_start.lte.${body.scheduled_start},scheduled_end.gt.${body.scheduled_start}),and(scheduled_start.lt.${body.scheduled_end},scheduled_end.gte.${body.scheduled_end})`)
