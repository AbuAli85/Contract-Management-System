...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        totalSteps: 4,
        passedSteps: Object.values(testResults).filter(
          (step: any) => step.status === '✅ Passed'
        ).length,
        failedSteps: Object.values(testResults).filter(
          (step: any) => step.status === '❌ Failed'
        ).length,
        overallStatus: Object.values(testResults).every(
          (step: any) => step.status === '✅ Passed'
        )
          ? '✅ All Tests Passed'
          : '⚠️ Some Tests Failed',
