...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status === "active") {
      ...
      query = query.or(
        "approval_status.in.(legal_review,hr_review,final_approval,signature),status.in.(draft,pending,generated)",
      )
    } else if (status === "completed") {
      // Get contracts that have been completed
      query = query.or("approval_status.in.(active,draft),status.in.(active,completed)")
    } else {
