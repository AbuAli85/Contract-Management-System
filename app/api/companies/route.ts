...
export const GET = withAnyRBAC(['company:read:own', 'company:read:organization', 'company:read:all'], async (request: NextRequest) => {
  try {
    ...
    if (!supabase || typeof supabase.from !== 'function') {
      console.error("❌ Companies API: Using mock client - environment variables may be missing")
    ...
    let companies = []
    try {
      const { data: companiesData, error: companiesError } = await query.limit(50)

      if (companiesError) {
        console.warn("⚠️ Companies API: Error fetching companies:", companiesError.message)
        companies = []
      } else {
        companies = companiesData || []
      }
    } catch (error) {
      console.warn("⚠️ Companies API: Company fetch failed, continuing with empty array")
      companies = []
    }

    console.log(`✅ Companies API: Successfully fetched ${companies?.length || 0} companies`)
