...
export async function GET(
  ...
  try {
    ...
    const calculatedTotals = {
      ...
      total_amount: invoice.subtotal + (invoice.subtotal * invoice.vat_rate) / 100,
      total_paid: invoice.payments?.reduce((sum: number, payment: any) => 
...
export async function DELETE(
  ...
  try {
    ...
    const hasCompletedPayments = invoice.payments?.some((p: any) => p.status === 'completed');
