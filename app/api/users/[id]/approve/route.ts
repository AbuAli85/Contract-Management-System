...
export async function PUT(
  ...
  try {
    ...
    if (!pendingError) {
      ...
      if (userStillPending) {
        console.log("❌ API Approve: PROBLEM - User still appears in pending list!", userStillPending)
      } else {
        console.log("✅ API Approve: User no longer in pending list")
      }
      console.log(`📊 API Approve: Total pending users remaining: ${stillPending.length}`)
    }
    ...
    if (!countError) {
      ...
      console.log(`📊 API Approve: Status distribution - Pending: ${pending}, Active: ${active}, Inactive: ${inactive}`)
    }
    return NextResponse.json({ 
      success: true,
