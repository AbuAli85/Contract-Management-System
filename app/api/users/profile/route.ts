...
export const GET = withRBAC('profile:read:own', async (request: NextRequest) => {
  try {
    ...
    if (profileError) {
      console.log('⚠️ User Profile API: Profile not found, creating default profile')
      ...
      if (createError) {
        console.error('❌ User Profile API: Error creating profile:', createError)
...
export const PUT = withRBAC('profile:update:own', async (request: NextRequest) => {
  try {
    ...
    if (updateError) {
      console.error('❌ User Profile API: Error updating profile:', updateError)
      return NextResponse.json({ 
