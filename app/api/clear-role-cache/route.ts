...
export async function POST(request: NextRequest) {
  try {
    console.log('=== CLEAR ROLE CACHE START ===')
    ...
    console.log('=== CLEAR ROLE CACHE COMPLETE ===')
    ...
    return NextResponse.json({
      ...
      cacheClear: {
        required: true,
        instructions: 'Clear localStorage cache and reload page to see updated role'
      },
      summary: {
        finalRole: currentRole,
        roleSource: roleSource,
        message: `Role cache cleared. Current role: ${currentRole} (from ${roleSource})`
      }
    })

  } catch (error) {
    console.error('=== CLEAR ROLE CACHE ERROR ===')
    console.error('Unexpected error:', error)
    
    return NextResponse.json({
      success: false,
      error: 'Clear role cache failed',
