...
export async function GET(request: NextRequest) {
  try {
    ...
    if (profileError && profileError.code !== 'PGRST116') {
      console.error("❌ Debug User Role: Error fetching profile:", profileError)
    ...
    if (userRecordError && userRecordError.code !== 'PGRST116') {
      console.error("❌ Debug User Role: Error fetching user record:", userRecordError)
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("🔧 Debug User Role: Updating role to:", newRole, "for user:", user.email)
    ...
    if (profileError) {
      console.error("❌ Debug User Role: Error updating profile:", profileError)
      updates.push({ type: 'profile', success: false, error: profileError.message })
    } else {
    ...
    if (authError) {
      console.error("❌ Debug User Role: Error updating auth metadata:", authError)
      updates.push({ type: 'auth_metadata', success: false, error: authError.message })
    } else {
