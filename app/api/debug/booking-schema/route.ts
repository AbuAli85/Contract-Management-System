    ...
    if (!tableError) {
      ...
      try {
        ...
        schemaChecks.unique_constraint_booking_number = {
          exists: false,
          error:
            'Could not verify constraint existence. Run migration to ensure it exists.',
      ...
      try {
        ...
        // Clean up test data
        if (upsertData) {
          await supabase.from('bookings').delete().eq('id', upsertData.id);
        }
      } catch (upsertTestError) {
        schemaChecks.upsert_functionality = {
          works: false,
          error:
            upsertTestError instanceof Error
              ? upsertTestError.message
              : 'Unknown upsert test error',
        };
      }
    }
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        unique_constraint_exists:
          schemaChecks.unique_constraint_booking_number?.exists || false,
