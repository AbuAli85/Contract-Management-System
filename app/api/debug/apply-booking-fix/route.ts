...
export async function POST() {
  try {
    ...
    const migrationPath = path.join(process.cwd(), 'supabase', 'migrations', '20250117_fix_booking_issues.sql')
    ...
    console.log('📋 Loaded migration file, size:', migrationSQL.length, 'chars')
    ...
    const testPayload = {
      ...
      metadata: { test: true, fix_applied: true, timestamp: new Date().toISOString() }
    }
