  ...
  try {
    ...
    if (!config_name || !config_type || !config_data) {
      return NextResponse.json(
        {
          error:
            'Missing required fields: config_name, config_type, config_data',
        },
        { status: 400 }
      );
    }
    ...
    if (userData?.role !== 'admin') {
      return NextResponse.json(
        { error: 'Only admins can manage workflow configuration' },
        { status: 403 }
      );
    }
    ...
    if (existingConfig) {
      return NextResponse.json(
        {
          error:
            'Configuration with this name already exists. Use PUT to update.',
        },
        { status: 409 }
      );
    }
