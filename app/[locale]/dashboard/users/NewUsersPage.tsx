...
export default function NewUsersPage() {
  ...
  const handleApproveUser = useCallback(
    async (userId: string, status: string, role?: string) => {
      try {
        ...
        if (response.ok) {
          ...
          toast({
            ...
            description:
              result.message ||
              `User ${status === 'active' ? 'approved' : status === 'inactive' ? 'deactivated' : 'updated'} successfully.`,
          });
                      ...
                      <tr key={user.id} className='border-b hover:bg-gray-50'>
                        ...
                        <td className='p-4'>
                          <div className='flex items-center space-x-2'>
                            ...
                            <span className='text-sm'>
                              {relativeTime(user.last_login ?? null)}
                            </span>
                          </div>
                        </td>
                        {['admin', 'manager', 'user'].includes(
                                ...
                                user.status === 'pending' && (
                                  <>
                                    ...
                                    <Button
                                      ...
                                      size='sm'
                                      onClick={() =>
                                        handleApproveUser(user.id, 'inactive')
                                      }
                              ...
                              {currentUserRole === 'admin' &&
                                user.status === 'inactive' && (
                                  <Button
                                    ...
                                    size='sm'
                                    onClick={() =>
                                      handleApproveUser(user.id, 'active')
                                    }
