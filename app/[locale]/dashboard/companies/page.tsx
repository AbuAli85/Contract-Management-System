...
export default function CrossCompanyDashboard() {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'employees' | 'attendance'>('name');
  ...
  const getFilteredCompanies = (companyList: CompanyData[]) => {
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'attendance':
          const aRate = a.stats.employees > 0 
            ? (a.stats.checked_in_today / a.stats.employees) * 100 
            : 0;
          const bRate = b.stats.employees > 0 
  ...
  return (
    <div className="container mx-auto py-6 space-y-8">
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
          <div className="bg-white p-4 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Companies</p>
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Checked In</p>
                <p className="text-2xl font-bold text-gray-900">{summary.total_checked_in || '—'}</p>
              </div>
              <div className="ml-4 p-2 bg-gray-50 rounded-lg">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Pending Leaves</p>
                <p className="text-2xl font-bold text-gray-900">{summary.total_pending_leaves || '—'}</p>
              </div>
              <div className="ml-4 p-2 bg-gray-50 rounded-lg">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Pending Expenses</p>
                <p className="text-2xl font-bold text-gray-900">{summary.total_pending_expenses || '—'}</p>
              </div>
              <div className="ml-4 p-2 bg-gray-50 rounded-lg">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-2xl font-bold text-gray-900">{summary.total_contracts || '—'}</p>
              </div>
              <div className="ml-4 p-2 bg-gray-50 rounded-lg">
          ...
          <div className="bg-white p-4 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors">
            <div className="flex items-center justify-between">
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Open Tasks</p>
                <p className="text-2xl font-bold text-gray-900">{summary.total_open_tasks || '—'}</p>
              </div>
              <div className="ml-4 p-2 bg-gray-50 rounded-lg">
      ...
      {Object.keys(grouped).length > 0 ? (
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <Badge className={cn('mt-1 text-xs', roleColors[company.user_role.toLowerCase()] || roleColors.member)}>
                                  {company.user_role}
                                </Badge>
                              </div>
                            </div>
                            {(canEdit || canDelete) && (
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                  <DropdownMenuItem onClick={() => handleSwitchToCompany(company.id)}>
                          ...
                          <div className="grid grid-cols-3 gap-2 mb-3">
                            <div className="text-center p-2 bg-gray-50 rounded">
                              <p className="text-lg font-bold text-gray-900">{company.stats.employees || '—'}</p>
                              <p className="text-xs text-gray-500">Employees</p>
                            </div>
                            <div className="text-center p-2 bg-green-50 rounded">
                              <p className="text-lg font-bold text-green-700">{company.stats.checked_in_today || '—'}</p>
                              <p className="text-xs text-green-600">Present</p>
                            </div>
                            <div className="text-center p-2 bg-orange-50 rounded">
                              <p className="text-lg font-bold text-orange-700">{company.stats.open_tasks || '—'}</p>
                          ...
                          {company.stats.employees > 0 && (
                            <div className="space-y-1">
                              <div className="flex items-center justify-between text-xs">
                                ...
                                <span className="font-medium text-gray-900">{Math.round((company.stats.checked_in_today / company.stats.employees) * 100)}%</span>
                              </div>
                              <Progress 
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b border-gray-200">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tasks</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {filtered.map((company) => {
                        const canEdit = canEditCompany(userHighestRole, company.user_role);
                        const canDelete = canDeleteCompany(userHighestRole, company.user_role);
                        const attendanceRate: number = company.stats.employees > 0
                          ? Math.round((company.stats.checked_in_today / company.stats.employees) * 100)
                          : 0;

                        return (
                          <tr key={company.id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors group">
                            ...
                            <td className="px-4 py-3 text-sm text-gray-600">{company.stats.employees || '—'}</td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-medium text-green-600">{company.stats.checked_in_today || '—'}</span>
                                {company.stats.employees > 0 && (
                                  <span className="text-xs text-gray-400">/ {company.stats.employees}</span>
                                )}
                              </div>
                            </td>
                            <td className="px-4 py-3">
                            ...
                            <td className="px-4 py-3 text-sm text-gray-600">{company.stats.open_tasks || '—'}</td>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline">{getFilteredCompanies(companies).length}</Badge>
          </div>
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      ...
      {companies.length > 0 && searchTerm && getFilteredCompanies(companies).length === 0 && (
