      ...
      <Card>
        ...
        <CardContent>
          {testResults.env ? (
            <div className="space-y-2">
              <p>
                <strong>Status:</strong> {testResults.env.status} {testResults.env.ok ? "✅" : "❌"}
              </p>
              {testResults.env.data?.env && (
                <>
                  <p>
                    <strong>Supabase URL:</strong>{" "}
                    {testResults.env.data.env.hasSupabaseUrl ? "✅ Set" : "❌ Missing"}
                  </p>
                  <p>
                    <strong>Supabase Anon Key:</strong>{" "}
                    {testResults.env.data.env.hasSupabaseAnonKey ? "✅ Set" : "❌ Missing"}
                  </p>
                  <p>
                    <strong>Service Role Key:</strong>{" "}
                    {testResults.env.data.env.hasServiceRoleKey ? "✅ Set" : "❌ Missing"}
                  </p>
                </>
              )}
              {testResults.env.error && (
                <p className="text-red-500">
                  <strong>Error:</strong> {testResults.env.error}
                </p>
              )}
            </div>
          ) : (
      ...
      <Card>
        ...
        <CardContent>
          {testResults.auth ? (
            <div className="space-y-2">
              <p>
                <strong>Status:</strong> {testResults.auth.status}{" "}
                {testResults.auth.ok ? "✅" : "❌"}
              </p>
              {testResults.auth.data?.auth && (
                <>
                  <p>
                    ...
                    {testResults.auth.data.auth.hasSession ? "✅ Yes" : "❌ No"}
                  </p>
                  <p>
                    <strong>Has User:</strong>{" "}
                    {testResults.auth.data.auth.hasUser ? "✅ Yes" : "❌ No"}
                  </p>
                  <p>
                    <strong>User Email:</strong> {testResults.auth.data.auth.userEmail || "None"}
                  </p>
                </>
              )}
              {testResults.auth.error && (
                <p className="text-red-500">
                  <strong>Error:</strong> {testResults.auth.error}
                </p>
              )}
            </div>
          ) : (
              ...
              {testResults.supabase.data?.supabase && (
                <>
                  <p>
                    ...
                    {testResults.supabase.data.supabase.canConnect ? "✅ Yes" : "❌ No"}
                  </p>
                  <p>
                    <strong>Has Data:</strong>{" "}
                    {testResults.supabase.data.supabase.hasData ? "✅ Yes" : "❌ No"}
                  </p>
                </>
              )}
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex gap-2">
              ...
              <Button onClick={() => (window.location.href = "/en/auth/login")}>Go to Login</Button>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            {authLoading && <p className="text-yellow-500">⚠️ Authentication is still loading</p>}
            {!user && !authLoading && (
              <p className="text-blue-500">
                ℹ️ You are not authenticated - this is normal if you haven't logged in
              </p>
            )}
