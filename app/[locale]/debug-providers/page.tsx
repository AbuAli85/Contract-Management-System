...
export default function DebugProvidersPage() {
  ...
  try {
    ...
    const { useEnhancedRBAC } = require('@/components/auth/enhanced-rbac-provider')
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-gray-600">Testing provider setup and context availability</p>
      </div>
        ...
        <CardContent>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span>Status:</span>
              <Badge variant={authTest.status === 'success' ? 'default' : authTest.status === 'error' ? 'destructive' : 'secondary'}>
                {authTest.status}
              </Badge>
            </div>
            ...
            {authTest.status === 'success' && authTest.data && (
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span>Has User:</span>
                  <Badge variant={authTest.data.hasUser ? 'default' : 'secondary'}>
                    {authTest.data.hasUser ? 'Yes' : 'No'}
                  </Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span>Has Session:</span>
                  <Badge variant={authTest.data.hasSession ? 'default' : 'secondary'}>
                    {authTest.data.hasSession ? 'Yes' : 'No'}
                  </Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span>Has Supabase:</span>
                  <Badge variant={authTest.data.hasSupabase ? 'default' : 'secondary'}>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span>Status:</span>
              <Badge variant={rbacTest.status === 'success' ? 'default' : rbacTest.status === 'error' ? 'destructive' : 'secondary'}>
                {rbacTest.status}
              </Badge>
            </div>
            ...
            {rbacTest.status === 'success' && rbacTest.data && (
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span>User Role:</span>
                  <Badge variant={rbacTest.data.userRole ? 'default' : 'secondary'}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={rbacTest.data.isLoading ? 'secondary' : 'default'}>
                    {rbacTest.data.isLoading ? 'Yes' : 'No'}
                  </Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span>Company Member:</span>
                  <Badge variant={rbacTest.data.isCompanyMember ? 'default' : 'secondary'}>
                    ...
                    {rbacTest.error.includes('EnhancedRBACProvider') && (
                      <div className="text-sm">
                        ...
                        <ul className="list-disc list-inside space-y-1 text-xs">
                          <li>EnhancedRBACProvider not added to the provider tree</li>
                          <li>Component is rendered outside the provider</li>
                          <li>Provider import path is incorrect</li>
                          <li>Provider is not properly wrapped in the layout</li>
                        </ul>
                      </div>
                    )}
                  </div>
        ...
        <CardContent>
          <div className="space-y-3">
            ...
            <Alert>
              ...
              <AlertDescription>
                <div className="space-y-2">
                  <p className="font-medium">Next Steps if Enhanced RBAC Fails:</p>
                  <ol className="list-decimal list-inside space-y-1 text-sm">
                    <li>Check if app/providers.tsx includes EnhancedRBACProvider</li>
