...
export default function MinimalRBACTestPage() {
  ...
  useEffect(() => {
    const runTests = async () => {
      ...
      try {
        const { useEnhancedRBAC } = await import('@/components/auth/enhanced-rbac-provider')
        ...
        <div className="border p-4 rounded">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`px-2 py-1 rounded text-sm ${
              providerTests.auth.status === 'success' ? 'bg-green-100 text-green-800' :
              providerTests.auth.status === 'error' ? 'bg-red-100 text-red-800' :
          ...
          {providerTests.auth.data && (
            <div className="mt-2 text-sm">
              <div>Has User: {providerTests.auth.data.hasUser ? 'Yes' : 'No'}</div>
              <div>Has Supabase: {providerTests.auth.data.hasSupabase ? 'Yes' : 'No'}</div>
            </div>
          )}
        </div>

        <div className="border p-4 rounded">
          <h2 className="text-lg font-semibold mb-2">Enhanced RBAC Provider Test</h2>
          <div className="flex items-center gap-2">
            <span>Status:</span>
            <span className={`px-2 py-1 rounded text-sm ${
              providerTests.rbac.status === 'success' ? 'bg-green-100 text-green-800' :
              providerTests.rbac.status === 'error' ? 'bg-red-100 text-red-800' :
              'bg-yellow-100 text-yellow-800'
            }`}>
            ...
            <div className="mt-2 text-red-600 text-sm">
              ...
              {providerTests.rbac.error.includes('EnhancedRBACProvider') && (
                <div className="mt-2 text-xs">
                  ...
                  <p>The EnhancedRBACProvider is not properly set up in the provider tree.</p>
                  <p>Check app/providers.tsx to ensure it includes the EnhancedRBACProvider.</p>
                </div>
              )}
            </div>
          )}
          {providerTests.rbac.data && (
            <div className="mt-2 text-sm">
              ...
              <div>Is Loading: {providerTests.rbac.data.isLoading ? 'Yes' : 'No'}</div>
              <div>Permission Count: {providerTests.rbac.data.permissionCount}</div>
            </div>
          )}
        </div>
