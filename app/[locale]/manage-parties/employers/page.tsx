...
export default function EmployersPage() {
  ...
  const filteredEmployers = useMemo(() => {
    return employers.filter(employer => {
      ...
      const matchesDocument = documentFilter === 'all' ||
        (documentFilter === 'expired' && (employer.cr_status === 'expired' || employer.license_status === 'expired')) ||
        (documentFilter === 'expiring' && (employer.cr_status === 'expiring' || employer.license_status === 'expiring')) ||
        (documentFilter === 'valid' && employer.cr_status === 'valid' && employer.license_status === 'valid');
  ...
  const stats = useMemo(() => {
    return {
      ...
      expiring: employers.filter(e => e.cr_status === 'expiring' || e.license_status === 'expiring').length,
      expired: employers.filter(e => e.cr_status === 'expired' || e.license_status === 'expired').length,
      total_contracts: employers.reduce((sum, e) => sum + (e.contract_count || 0), 0),
    };
  }, [employers]);
  ...
  const toggleEmployerExpansion = useCallback((employerId: string) => {
                      ...
                      <TableRow>
                        <TableCell>
                          <div className='flex items-center gap-2'>
                            <div>
                              <div className='font-medium'>{employer.name_en}</div>
                              {employer.name_ar && (
                          ...
                          {employer.cr_expiry_date ? (
                            <div className='text-sm'>
                              ...
                              <div className={cn(
                                ...
                                employer.cr_status === 'expiring' && 'text-yellow-600',
                                employer.cr_status === 'valid' && 'text-green-600'
                              )}>
                                {employer.cr_status === 'expired' && 'Expired'}
                                {employer.cr_status === 'expiring' && `${employer.days_until_cr_expiry} days left`}
                          ...
                          {employer.license_expiry ? (
                            <div className='text-sm'>
                              ...
                              <div className={cn(
                                ...
                                employer.license_status === 'expiring' && 'text-yellow-600',
                                employer.license_status === 'valid' && 'text-green-600'
                              )}>
                                {employer.license_status === 'expired' && 'Expired'}
                                {employer.license_status === 'expiring' && `${employer.days_until_license_expiry} days left`}
                        ...
                        <TableCell>
                          <Badge variant='outline'>{employer.contract_count || 0}</Badge>
                        </TableCell>
                        <TableCell>
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent>
                              ...
                              <DropdownMenuItem onClick={() => router.push(`/en/manage-parties?id=${employer.id}`)}>
                                <EditIcon className='mr-2 h-4 w-4' />
                                Edit
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => router.push(`/en/manage-parties/${employer.id}`)}>
                            ...
                            <div className='bg-slate-50 dark:bg-slate-900 border-t'>
                              <div className='p-4'>
                                <div className='flex items-center gap-2 mb-4'>
                                  ...
                                  <h3 className='text-lg font-semibold'>Assigned Promoters</h3>
                                  <Badge variant='outline'>
                                    {promotersByEmployer[employer.id]?.length || 0} Total
                                  </Badge>
                                </div>
                                ...
                                {loadingPromoters.has(employer.id) ? (
                                  ...
                                  const employerPromoters = promotersByEmployer[employer.id];
                                  return employerPromoters && employerPromoters.length > 0 ? (
