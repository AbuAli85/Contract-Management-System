...
export default async function PartyDetailPage({
	...
	try {
		...
		const { data: contracts } = await supabase
			.from('contracts')
			.select('id, contract_number, status, created_at, first_party_id, second_party_id')
								...
								<div className='space-y-2 text-sm'>
									{contracts!.map(c => (
										<div key={c.id} className='flex items-center justify-between rounded border p-2'>
											<div>
												<div className='font-medium'>{c.contract_number ?? c.id}</div>
												<div className='text-muted-foreground text-xs'>Status: {c.status ?? 'unknown'}</div>
											</div>
