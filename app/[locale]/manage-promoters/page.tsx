          ...
          {promoters.length === 0 ? (
            ...
            <div className="space-y-4">
              {promoters.slice(0, 10).map((promoter, index) => (
                <div key={promoter.id || index} className="border rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-medium">{promoter.name_en || 'Unknown'}</h3>
                      <p className="text-sm text-muted-foreground">{promoter.name_ar || 'غير معروف'}</p>
                      <p className="text-sm">ID: {promoter.id_card_number || 'N/A'}</p>
                      <p className="text-sm">Status: {promoter.status || 'N/A'}</p>
                      {promoter.parties && (
                        <p className="text-sm">Employer: {promoter.parties.name_en || 'N/A'}</p>
                      )}
                    </div>
                  </div>
                </div>
