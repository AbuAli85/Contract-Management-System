...
export default function DiagnosticPage() {
  ...
  const runTest = async (testName: string, testFn: () => Promise<void>) => {
    ...
    try {
      ...
      setErrors(prev => ({ ...prev, [testName]: error instanceof Error ? error.message : String(error) }))
    }
  }
  ...
  const testDynamicImport = async () => {
    try {
      const module = await import("@/components/unified-contract-generator-form")
      if (!module.default) {
    ...
    <div className="mx-auto max-w-4xl p-6 space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          <Alert>
            <AlertDescription>
              This page tests various components that might cause Server Component rendering errors.
            </AlertDescription>
          </Alert>
