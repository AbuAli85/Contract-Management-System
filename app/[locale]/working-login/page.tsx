...
export default function WorkingLoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      console.log("🔐 Working Login - Supabase URL:", process.env.NEXT_PUBLIC_SUPABASE_URL)
      ...
      if (authError) {
        ...
        if (authError.message.includes("Invalid login credentials")) {
          ...
          setError("❌ Database error. Please run the schema fix script first.")
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <p className="text-sm text-gray-600">Simple authentication without complex logic</p>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
            ...
            <Alert className="border-blue-200 bg-blue-50">
              <AlertDescription className="text-blue-800">
                <div className="space-y-1">
                  <div><strong>Logged in as:</strong> {currentUser.email}</div>
                  <div><strong>User ID:</strong> {currentUser.id}</div>
                  <div><strong>Confirmed:</strong> {currentUser.email_confirmed_at ? "✅ Yes" : "❌ No"}</div>
                  <div><strong>Role:</strong> {currentUser.user_metadata?.role || currentUser.raw_user_meta_data?.role || "Unknown"}</div>
                </div>
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="text-xs text-gray-500 text-center border-t pt-2 space-y-1">
            ...
            <p><strong>Supabase:</strong> reootcngcptfogfozlmz</p>
