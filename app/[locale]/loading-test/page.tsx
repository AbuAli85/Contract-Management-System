export default async function LoadingTestPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params

  return (
    <div className="space-y-6">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-green-600">✅ Loading Test Page</h1>
        <p className="text-muted-foreground">Testing if content renders without loading issues</p>
      </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <ul className="space-y-2 text-sm text-blue-700">
          ...
          <li>• Page should load immediately</li>
        </ul>
      </div>

      <div className="text-center">
        <p className="text-sm text-gray-500">
          If you can see this content, the loading issue is resolved!
        </p>
      </div>
    </div>
  )
