  ...
  const refreshDebugInfo = async () => {
    ...
    try {
      const [
        ...
        fetch('/api/auth/check-session').catch(() => ({ json: () => ({ error: 'Failed to fetch' }) })),
        fetch('/api/test-connection').catch(() => ({ json: () => ({ error: 'Failed to fetch' }) })),
      ...
      const newDebugInfo = {
        ...
        browserState: {
          ...
          localStorage: typeof window !== 'undefined' ? Object.keys(localStorage).filter(key => key.includes('auth')) : [],
          sessionStorage: typeof window !== 'undefined' ? Object.keys(sessionStorage).filter(key => key.includes('auth')) : []
        }
      }
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {debugInfo.browserState.localStorage.length > 0 ? (
                    debugInfo.browserState.localStorage.map((key: string, index: number) => (
                      <Badge key={index} variant="outline">{key}</Badge>
                    ))
                  ) : (
        ...
        <CardContent className="space-y-4">
          <div className="space-y-3">
            <Alert>
              ...
              <AlertDescription>
                <strong>Issue:</strong> Auth cookies present but session not established<br />
            ...
            <Alert>
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                <strong>Issue:</strong> Session exists but user profile not found<br />
                <strong>Solution:</strong> Ensure database tables are created and RLS policies are applied
              </AlertDescription>
            </Alert>
            ...
            <Alert>
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                <strong>Issue:</strong> Middleware redirects not working<br />
            ...
            <Alert>
              <AlertTriangle className="h-4 w-4" />
              <AlertDescription>
                <strong>Issue:</strong> Environment variables missing<br />
                <strong>Solution:</strong> Ensure .env file is properly configured with Supabase credentials
              </AlertDescription>
            </Alert>
          </div>
        </CardContent>
