...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Handle OAuth errors
    if (error) {
      console.error(`🔐 Enhanced Auth Callback [${requestId}]: OAuth error:`, error, errorDescription)
    ...
    if (!code) {
      console.warn(`🔐 Enhanced Auth Callback [${requestId}]: No authorization code provided`)
      ...
      // Redirect to login
      return NextResponse.redirect(`${origin}/en/auth/login?error=no_code&message=No authorization code provided`)
    }

    console.log(`🔐 Enhanced Auth Callback [${requestId}]: Exchanging code for session`)
    ...
    if (exchangeError) {
      console.error(`🔐 Enhanced Auth Callback [${requestId}]: Code exchange error:`, exchangeError)
    ...
    if (!data.session) {
      console.error(`🔐 Enhanced Auth Callback [${requestId}]: No session returned from code exchange`)
    ...
    console.log(`🔐 Enhanced Auth Callback [${requestId}]: Session established for user:`, data.session.user.email)

    // Check if user needs to complete profile setup
    const needsProfileSetup = await checkProfileSetup(data.session.user.id, supabase)
    
    if (needsProfileSetup) {
      console.log(`🔐 Enhanced Auth Callback [${requestId}]: User needs profile setup`)
      return NextResponse.redirect(`${origin}/en/onboarding?user_id=${data.session.user.id}`)
    }
    ...
    if (mfaRequired) {
      console.log(`🔐 Enhanced Auth Callback [${requestId}]: MFA required for user`)
      return NextResponse.redirect(`${origin}/en/auth/mfa-setup?user_id=${data.session.user.id}`)
    }

    // Validate redirect URL for security
    const validatedNext = validateRedirectUrl(next, origin)
    
    console.log(`🔐 Enhanced Auth Callback [${requestId}]: Redirecting to:`, validatedNext)
    ...
    console.error(`🔐 Enhanced Auth Callback [${requestId}]: Unexpected error:`, error)
    ...
    return NextResponse.redirect(`${request.nextUrl.origin}/en/auth/login?error=callback_error&message=Authentication callback failed`)
  }
}

// Check if user needs to complete profile setup
async function checkProfileSetup(userId: string, supabase: any): Promise<boolean> {
  try {
...
async function checkMFARequirement(userId: string, supabase: any): Promise<boolean> {
  try {
    const { data, error } = await supabase
      .from('user_profiles')
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`🔐 Enhanced Auth Callback POST [${requestId}]: Processing OAuth state verification`)
    ...
    if (error) {
      console.error(`🔐 Enhanced Auth Callback POST [${requestId}]: Code exchange error:`, error)
    ...
    console.log(`🔐 Enhanced Auth Callback POST [${requestId}]: OAuth session established for user:`, data.session.user.email)
    ...
    console.error(`🔐 Enhanced Auth Callback POST [${requestId}]: Unexpected error:`, error)
