...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!code) {
      console.warn(`🔧 Auth Callback [${requestId}]: No authorization code provided`)
    ...
    if (exchangeError) {
      console.error(`🔧 Auth Callback [${requestId}]: Code exchange error:`, exchangeError)
      return NextResponse.redirect(`${origin}/en/auth/login?error=exchange_failed&message=${exchangeError.message}`)
    }
    ...
    console.log(`🔧 Auth Callback [${requestId}]: Success, redirecting to:`, next)
    return NextResponse.redirect(`${origin}${next}`)
    
  } catch (error) {
    console.error(`🔧 Auth Callback [${requestId}]: Unexpected error:`, error)
    return NextResponse.redirect(`${request.nextUrl.origin}/en/auth/login?error=unexpected&message=An unexpected error occurred`)
  }
} 