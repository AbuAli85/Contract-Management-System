...
describe('RBAC Database Idempotency', () => {
  ...
  describe('Seed Data Idempotency', () => {
    it('should handle role insertion conflicts gracefully', async () => {
      ...
      const insertRole = async (name: string, category: string, description: string) => {
      ...
      // First insertion
      const result1 = await insertRole('Basic Client', 'client', 'Basic client with limited booking capabilities')
      expect(result1.updated).toBe(true)
      expect(mockDbState.roles.size).toBe(1)

      // Second insertion (should update, not duplicate)
      const result2 = await insertRole('Basic Client', 'client', 'Updated description')
    ...
    it('should handle permission insertion conflicts gracefully', async () => {
      ...
      const insertPermission = async (resource: string, action: string, scope: string, name: string, description: string) => {
        if (mockDbState.permissions.has(name)) {
          ...
          // Insert new
          const newPermission = { id: 'new-uuid', resource, action, scope, name, description }
      ...
      // First insertion
      const result1 = await insertPermission('user', 'view', 'own', 'user:view:own', 'View own user profile')
      expect(result1.updated).toBe(true)
      expect(mockDbState.permissions.size).toBe(1)

      // Second insertion (should update, not duplicate)
      const result2 = await insertPermission('user', 'view', 'own', 'user:view:own', 'Updated description')
      expect(result2.updated).toBe(true)
    ...
    it('should handle role-permission mapping conflicts gracefully', async () => {
      ...
      mockDbState.permissions.set(permissionId, { id: permissionId, name: 'test:permission:all' })
      mockDbState.rolePermissions.set(mappingKey, { roleId, permissionId })

      const insertRolePermission = async (roleId: string, permissionId: string) => {
  ...
  describe('Unique Constraint Validation', () => {
    ...
    it('should enforce unique permission names', () => {
      const permissions = [
        { name: 'user:view:own', resource: 'user', action: 'view', scope: 'own' },
        { name: 'user:view:own', resource: 'user', action: 'view', scope: 'own' } // Duplicate name
      ]
  ...
  describe('Data Integrity', () => {
    it('should maintain referential integrity for role-permission mappings', () => {
      ...
      mockDbState.roles.set(validRoleId, { id: validRoleId, name: 'Valid Role' })
      mockDbState.permissions.set(validPermissionId, { id: validPermissionId, name: 'valid:permission:all' })

      // Valid mapping
      const validMapping = { roleId: validRoleId, permissionId: validPermissionId }
      ...
      const invalidMapping = { roleId: 'non-existent-role', permissionId: validPermissionId }
    ...
    it('should maintain referential integrity for user-role assignments', () => {
      ...
      const invalidAssignment = { userId: validUserId, roleId: 'non-existent-role' }
