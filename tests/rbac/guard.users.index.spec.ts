...
describe('RBAC: /api/users (collection)', () => {
  ...
  test('GET /api/users requires user:read:all permission', async () => {
    ...
    expect(withRBAC).toHaveBeenCalledWith('user:read:all', expect.any(Function));
  });
  ...
  test('POST /api/users requires user:create:all permission', async () => {
    ...
    expect(withRBAC).toHaveBeenCalledWith('user:create:all', expect.any(Function));
  });
  ...
  test('PUT /api/users requires user:update:all permission', async () => {
    ...
    expect(withRBAC).toHaveBeenCalledWith('user:update:all', expect.any(Function));
  });
  ...
  test('DELETE /api/users requires user:delete:all permission', async () => {
    ...
    expect(withRBAC).toHaveBeenCalledWith('user:delete:all', expect.any(Function));
  });
