...
describe('RBAC Permission Cache', () => {
  ...
  describe('In-Memory Caching', () => {
    it('should cache user permissions with TTL', async () => {
      ...
      // Mock database fetch
      const mockFetchFromDb = jest.spyOn(
    ...
    it('should handle database fetch errors gracefully', async () => {
      ...
      mockFetchFromDb.mockRejectedValue(
        new Error('Database connection failed')
      );

      // Should handle error gracefully
      await expect(permissionCache.getUserPermissions(userId)).rejects.toThrow(
  ...
  describe('Cache Invalidation', () => {
    it('should invalidate specific user cache', async () => {
      ...
      await permissionCache.getUserPermissions(userId);
      expect((permissionCache as any).cache.has(`permissions:${userId}`)).toBe(
        true
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle null user ID', async () => {
      ...
      await expect(
        permissionCache.getUserPermissions(null as any)
      ).rejects.toThrow();
    });
    ...
    it('should handle undefined user ID', async () => {
      // Should handle gracefully
      await expect(
        permissionCache.getUserPermissions(undefined as any)
      ).rejects.toThrow();
    });
