...
describe('Promoter Management - Integration Tests', () => {
  ...
  describe('End-to-End Promoter Management Flow', () => {
    ...
    it('should handle promoter creation and validation', async () => {
      ...
      const result = await mockSupabaseClient.from('promoters').insert(validPromoter).select().single();
    ...
    it('should handle promoter update workflow', async () => {
      ...
      const result = await mockSupabaseClient.from('promoters')
        .update(updatedData)
        .eq('id', 1)
        .select()
        .single();
  ...
  describe('CV Data Management Integration', () => {
    it('should fetch and display CV data', async () => {
      const mockCVData = {
        ...
        education: [
          { id: 1, institution: 'University', degree: 'Computer Science', year: '2020' },
        ],
        documents: [
          { id: 1, document_name: 'Resume.pdf', document_type: 'resume' },
        ],
      };
    ...
    it('should handle CV data updates', async () => {
      ...
      const result = await mockSupabaseClient.from('promoter_skills')
        .insert(newSkill)
        .select()
        .single();
  ...
  describe('CSV Import/Export Integration', () => {
    it('should export promoters to CSV format', async () => {
      const mockPromoters = [
        ...
        { id: 2, first_name: 'Jane', last_name: 'Smith', email: 'jane@example.com', phone: '+0987654321' },
      ];
...
Jane,Smith,jane@example.com`;
  ...
  describe('Data Consistency and Integrity', () => {
    it('should maintain referential integrity', async () => {
      ...
      const promoterResult = await mockSupabaseClient.from('promoters')
        .select()
        .eq('id', 1)
        .single();

      const contractsResult = await mockSupabaseClient.from('contracts')
        .select()
        .eq('promoter_id', 1);
    ...
    it('should handle concurrent data modifications', async () => {
      ...
      const promises = Array(3).fill(null).map(() => 
        mockSupabaseClient.from('promoters')
