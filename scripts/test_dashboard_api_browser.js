...
async function testDashboardAPIs() {
  ...
  try {
    ...
    if (statsResponse.ok) {
      ...
      console.log('- Total Promoters:', results.stats.totalPromoters, '(expected: 158)');
      console.log('- Active Promoters:', results.stats.activePromoters, '(expected: 158)');
      console.log('- Total Parties:', results.stats.totalParties, '(expected: 16)');
      console.log('- Total Contracts:', results.stats.totalContracts, '(expected: 0)');
      console.log('- Pending Approvals:', results.stats.pendingApprovals, '(expected: 0)');
      console.log('- Recent Activity:', results.stats.recentActivity, '(expected: 0)');
    } else {
      const errorText = await statsResponse.text();
      results.errors.push(`Stats API failed: ${statsResponse.status} - ${errorText}`);
  ...
  try {
    ...
    if (notificationsResponse.ok) {
      ...
      results.errors.push(`Notifications API failed: ${notificationsResponse.status} - ${errorText}`);
      console.error('❌ Notifications API Failed:', notificationsResponse.status, errorText);
    }
  } catch (error) {
  ...
  try {
    ...
    if (activitiesResponse.ok) {
      ...
      results.errors.push(`Activities API failed: ${activitiesResponse.status} - ${errorText}`);
      console.error('❌ Activities API Failed:', activitiesResponse.status, errorText);
    }
  } catch (error) {
  ...
  console.log('- Notifications API:', results.notifications ? '✅ Working' : '❌ Failed');
  console.log('- Activities API:', results.activities ? '✅ Working' : '❌ Failed');
...
console.log('fetch("/api/dashboard/notifications").then(r => r.json()).then(console.log)');
console.log('fetch("/api/dashboard/activities").then(r => r.json()).then(console.log)');
