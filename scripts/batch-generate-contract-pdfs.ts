...
async function main() {
  ...
  console.log(
    `   Make.com Webhook: ${MAKECOM_WEBHOOK_URL.substring(0, 50)}...`
  );
  ...
  console.log(
    `📄 Step 2: Generating PDFs for ${contracts.length} contracts...\n`
  );
  ...
  for (let i = 0; i < contracts.length; i++) {
    ...
    try {
      ...
      const response = await fetch(
        `${APP_URL}/api/contracts/makecom/generate`,
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        ...
        console.log(
          `   ✅ Success - Status: ${result.data?.makecom?.status || 'triggered'}`
        );
        successCount++;
      } else {
  ...
  console.log(
    '   Check the contracts table in a few minutes to see updated pdf_url values.'
  );
}
