...
class RBACSmokeTester {
    ...
    try {
      ...
      if (hasPermissions) {
        ...
        this.recordTest(
          ...
          false,
          Date.now() - testStart,
          'Permission table empty or inaccessible'
        );
  ...
  private async testPerformanceBaseline(): Promise<void> {
    ...
    try {
      ...
      if (performance.withinThreshold) {
        this.recordTest(testName, true, Date.now() - testStart);
        console.log(
          `   ✅ Performance within threshold (${performance.avgResponseTime}ms)`
        );
      } else {
        this.recordTest(
          testName,
        ...
        console.log(
          `   ❌ Performance degraded: ${performance.avgResponseTime}ms`
        );
      }
    } catch (error) {
      this.recordTest(testName, false, Date.now() - testStart, error.message);
  ...
  private async verifyGuardCoverage(): Promise<{
    criticalPathsGuarded: boolean;
  ...
  private async verifyRolePermissions(): Promise<{
    valid: boolean;
    issueCount: number;
  }> {
  ...
  private async verifyAuditLogging(): Promise<{
    enabled: boolean;
    functional: boolean;
  }> {
