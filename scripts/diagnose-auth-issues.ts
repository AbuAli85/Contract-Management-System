...
async function checkUserRoleAssignments() {
  ...
  try {
    ...
    const rbacAdminUsers =
      userRoles?.filter(ur => ur.roles && (ur.roles as any).name === 'admin') ||
      [];
...
async function checkPermissionCache() {
  ...
  addResult({
    ...
    message: cacheEnabled
      ? 'Permission cache is enabled'
      : 'Permission cache is disabled',
    details: {
    ...
    fix: cacheEnabled
      ? undefined
      : 'Enable cache for better performance: ENABLE_PERMISSION_CACHE=true',
  });
}
...
async function generateReport() {
  ...
  if (failed > 0 || warnings > 0) {
    ...
    console.log(
      '✅ All checks passed! Your authentication system is properly configured.'
    );
    console.log();
  }
...
async function main() {
  ...
  console.log(
    'This tool will check your authentication setup and identify issues.\n'
  );
  console.log('='.repeat(80) + '\n');
