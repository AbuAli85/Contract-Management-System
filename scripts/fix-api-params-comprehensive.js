...
function fixParamsInFile(filePath) {
  try {
    ...
    const functionRegex =
      /export async function (GET|POST|PUT|DELETE|PATCH)\s*\(\s*([^)]*)\s*,\s*\{\s*params\s*\}\s*\)/g;
    ...
    const paramsSemicolonRegex =
      /const\s*\{\s*([^}]+)\s*\}\s*=\s*await params;([^;]+)/g;

    content = content.replace(
      paramsSemicolonRegex,
      (match, destructuring, rest) => {
        modified = true;
    ...
    const paramsIdRegex =
      /const\s*\{\s*id\s*:\s*([^}]+)\s*\}\s*=\s*params\s*;?/g;
    ...
    const paramsAwaitWrongFormatRegex =
      /const\s*\{\s*([^}]+)\s*\}\s*=\s*await params;([^;]+)/g;

    content = content.replace(
      paramsAwaitWrongFormatRegex,
      (match, destructuring, rest) => {
        modified = true;
        return `const { ${destructuring} } = await params;\n  ${rest.trim()}`;
      }
