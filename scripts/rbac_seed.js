/*
  Idempotent RBAC seeder. Run with: node scripts/rbac_seed.js
...
async function upsertRole(supabase, name, category, description = '') {
  const { data, error } = await supabase.rpc('rbac_upsert_role', { p_name: name, p_category: category, p_description: description })
  if (error) throw error
  return data
}

async function upsertPermission(supabase, resource, action, scope, name, description = '') {
...
async function attach(supabase, roleId, permId) {
...
async function seed() {
  ...
  // FIXED: All scopes now use only allowed values: 'own', 'provider', 'organization', 'booking', 'public', 'all'
  const permissionStrings = [
    ...
    'service:search:all','service:view:public','service:review:own','service:manage:own','service:favorite:own',
    ...
    'booking:confirm:provider','booking:start:provider','booking:complete:provider','booking:reschedule:own','booking:dispute:own','booking:dispute:all',
    ...
    'call:initiate:booking','call:receive:booking','call:record:own','call:manage:provider',
    'payment:create:own','payment:read:own','payment:method:own','payment:refund:own','payment:process:provider','payment:payout:provider',
  ...
  await grant('Basic Client', [
    ...
    'service:search:all','service:view:public','service:review:own','service:manage:own','service:favorite:own',
    'booking:create:own','booking:read:own','booking:update:own','booking:cancel:own','booking:reschedule:own',
    'message:read:own','message:manage:own','notification:receive:own','notification:manage:own',
    'payment:create:own','payment:read:own','payment:method:own','payment:refund:own',
  ...
  await grant('Premium Client', Array.from(permIds.keys()).filter(k => [
    ...
    'booking:create:own','booking:read:own','booking:update:own','booking:cancel:own','booking:reschedule:own',
    'message:read:own','message:manage:own','notification:receive:own','notification:manage:own',
    'payment:create:own','payment:read:own','payment:method:own','payment:refund:own','finance:report:own','tax:document:own'
  ].includes(k)))
  ...
  await grant('Individual Provider', [
    ...
    'message:send:booking','call:initiate:booking','call:receive:booking','call:record:own','call:manage:provider',
    'payment:process:provider','payment:payout:provider','finance:report:provider','tax:document:provider'
  ])
