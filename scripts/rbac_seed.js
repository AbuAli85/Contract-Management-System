...
async function upsertRole(supabase, name, category, description = '') {
  const { data, error } = await supabase.rpc('rbac_upsert_role', {
    p_name: name,
    p_category: category,
...
async function upsertPermission(
  ...
  const { data, error } = await supabase.rpc('rbac_upsert_permission', {
    p_resource: resource,
    p_action: action,
    p_scope: scope,
    p_name: name,
...
async function attach(supabase, roleId, permId) {
  const { error } = await supabase.rpc('rbac_attach_permission', {
    p_role_id: roleId,
    p_perm_id: permId,
  });
  if (error) throw error;
...
async function seed() {
  ...
  const permissionStrings = [
    ...
    'service:review:own',
    'service:manage:own',
    'service:favorite:own',
    'booking:create:own',
    'booking:read:own',
    'booking:update:own',
    'booking:cancel:own',
    'booking:read:provider',
    'booking:manage:provider',
    'booking:read:all',
    'booking:confirm:provider',
    'booking:start:provider',
    'booking:complete:provider',
    'booking:reschedule:own',
    'booking:dispute:own',
    'booking:dispute:all',
    'message:send:booking',
    'message:read:own',
    'message:manage:own',
    'message:moderate:all',
    'notification:receive:own',
    'notification:manage:own',
    'call:initiate:booking',
    'call:receive:booking',
    'call:record:own',
    'call:manage:provider',
    'payment:create:own',
    'payment:read:own',
    'payment:method:own',
    'payment:refund:own',
  ...
  await grant(
    'Premium Client',
    Array.from(permIds.keys()).filter(k =>
      [
        'user:read:own',
        'user:update:own',
        'user:delete:own',
  ...
  await grant('Content Moderator', [
    'service:moderate:all',
    'message:moderate:all',
  ]);

  // Platform/System Admin
  await grant('Platform Administrator', [
    'user:read:all',
    'user:update:all',
    'user:create:all',
    'role:assign:all',
  ...
  await grant('System Administrator', [
    'config:platform:all',
    'user:impersonate:all',
  ]);
