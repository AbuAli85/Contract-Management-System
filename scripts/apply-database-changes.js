...
async function applyDatabaseChanges() {
  ...
  try {
    ...
    if (contractsError) {
      console.error(
        '❌ Error adding columns to contracts table:',
        contractsError
      );
    } else {
    ...
    await supabase.rpc('exec_sql', {
      sql: 'ALTER TABLE locations ENABLE ROW LEVEL SECURITY;',
    });
    ...
    await supabase.rpc('exec_sql', {
      sql: 'CREATE INDEX IF NOT EXISTS idx_locations_status ON locations(status);',
    });
    await supabase.rpc('exec_sql', {
      sql: 'CREATE INDEX IF NOT EXISTS idx_locations_country ON locations(country_en);',
    });
