...
async function generateDriftReport() {
  console.log('🔍 Analyzing RBAC drift...\n');

  // Extract permissions from code
  const codePermissions = await extractPermissionsFromCode();
  console.log(`📝 Found ${codePermissions.size} unique permissions in code`);
  ...
  const p0Issues = Array.from(codePermissions).filter(
    p => !dbPermissionNames.has(p)
  );
  ...
  const report = `# 🛡️ RBAC Drift Report
...
`;
  ...
  if (p0Issues.length > 0) {
    console.log(`🚨 P0 Critical Issues: ${p0Issues.length}`);
    p0Issues.forEach(p => console.log(`   - ${p}`));
    console.log('');
    process.exit(1);
  } else {
    console.log('✅ No P0 critical issues found');
    process.exit(0);
  }
}
