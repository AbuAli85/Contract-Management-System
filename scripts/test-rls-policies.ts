...
class RLSPolicyTester {
  ...
  private async testUnauthorizedInsert(): Promise<void> {
    ...
    if (!error) {
      throw new Error(
        'User was able to insert party data without proper permissions'
      );
    }
  }
  ...
  private async testUnauthorizedUpdate(): Promise<void> {
    ...
    if (parties && parties.length > 0) {
      ...
      if (!error) {
        throw new Error(
          'User was able to update party data without proper permissions'
        );
      }
    }
  }
  ...
  private async testUnauthorizedDelete(): Promise<void> {
    ...
    if (parties && parties.length > 0) {
      ...
      if (!error) {
        throw new Error(
          'User was able to delete party data without proper permissions'
        );
      }
    }
  }
  ...
  private async testRLSPolicyVerification(): Promise<void> {
    ...
    const { data: policies, error } =
      await this.supabase.rpc('test_rls_policies');
  ...
  private printResults(): void {
    ...
    const passed = this.testResults.filter(
      r => r.actualResult === 'PASS'
    ).length;
    const failed = this.testResults.filter(
      r => r.actualResult === 'FAIL'
    ).length;
