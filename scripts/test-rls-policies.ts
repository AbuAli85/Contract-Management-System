...
class RLSPolicyTester {
  ...
  private async testPromoterAccess(): Promise<void> {
    ...
    const { data: promoters, error } = await this.supabase.from("promoters").select("*").limit(5)
  ...
  private async testContractAccess(): Promise<void> {
    ...
    const { data: contracts, error } = await this.supabase.from("contracts").select("*").limit(5)
  ...
  private async testPartyAccess(): Promise<void> {
    ...
    const { data: parties, error } = await this.supabase.from("parties").select("*").limit(5)
  ...
  private async testUnauthorizedInsert(): Promise<void> {
    ...
    const { data, error } = await this.supabase.from("parties").insert(testData).select()
  ...
  private async testUnauthorizedUpdate(): Promise<void> {
    ...
    const { data: parties } = await this.supabase.from("parties").select("id, name_en").limit(1)
  ...
  private async testUnauthorizedDelete(): Promise<void> {
    ...
    const { data: parties } = await this.supabase.from("parties").select("id").limit(1)

    if (parties && parties.length > 0) {
      const { error } = await this.supabase.from("parties").delete().eq("id", parties[0].id)
  ...
  private async testEmailQueueAccess(): Promise<void> {
    ...
    const { data: emailQueue, error } = await this.supabase.from("email_queue").select("*").limit(5)
  ...
  private async testRLSPolicyVerification(): Promise<void> {
    ...
    const { data: policies, error } = await this.supabase.rpc("test_rls_policies")
  ...
  public async runAllTests(): Promise<void> {
    ...
    await this.runTest("User Profile Access Control", () => this.testUserProfileAccess(), "PASS")

    await this.runTest("Promoter Access Control", () => this.testPromoterAccess(), "PASS")

    await this.runTest("Contract Access Control", () => this.testContractAccess(), "PASS")

    await this.runTest("Party Access Control", () => this.testPartyAccess(), "PASS")
    ...
    await this.runTest("System Table Access Control", () => this.testSystemTableAccess(), "PASS")

    await this.runTest("Email Queue Access Control", () => this.testEmailQueueAccess(), "PASS")

    // Test RLS policy verification
    await this.runTest("RLS Policy Verification", () => this.testRLSPolicyVerification(), "PASS")
