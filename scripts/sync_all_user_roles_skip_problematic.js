...
async function syncAllUserRolesSkipProblematic() {
  try {
    console.log(
      '🔄 Starting SYNC (skip problematic user) user role synchronization...\n'
    );
    ...
    console.log(
      '🔄 Starting UPSERT synchronization (skipping problematic users)...\n'
    );
    ...
    for (const user of allUsers) {
      ...
      if (user.email === 'cleaning.provider@example.com') {
        console.log(
          `⏭️ Skipping problematic user: ${user.email} (${user.role})`
        );
        console.log(
          `  ℹ️ Known issue: Foreign key constraint prevents user_roles sync`
        );
    ...
    console.log(
      `📊 Sync Summary: ${syncedUsers} users synced, ${skippedUsers} users skipped\n`
    );
    ...
    console.log(
      `- Users skipped: ${skippedUsers} (cleaning.provider@example.com)`
    );
    console.log(
    ...
    console.log('\n⚠️ Known Issue:');
    console.log(
      '- cleaning.provider@example.com has foreign key constraint issues'
    );
    console.log(
      '- User is functional in other tables (provider_services, etc.)'
    );
    console.log(
      '- This is a data integrity issue that requires manual investigation'
    );
