...
async function debugCleaningProvider() {
  try {
    console.log(
      '🔍 Debugging cleaning.provider@example.com foreign key issue...\n'
    );
    ...
    if (userRolesError) {
      ...
      userRolesData.forEach((role, index) => {
        console.log(
          `  ${index + 1}. ID: ${role.id}, User ID: ${role.user_id}, Role: ${role.role}`
        );
      });
    }
    console.log('');
    ...
    const { data: userRolesSchema, error: userRolesSchemaError } =
      await supabase.from('user_roles').select('user_id').limit(1);
    ...
    console.log(
      '- Foreign key constraint violation suggests data type or reference issue'
    );
    console.log('');
    console.log('💡 Recommendations:');
    console.log(
      '1. Check if user_roles.user_id column type matches users.id column type'
    );
    console.log('2. Verify the user ID is not corrupted or malformed');
    console.log(
      '3. Check if there are any triggers or constraints interfering'
    );
  } catch (error) {
