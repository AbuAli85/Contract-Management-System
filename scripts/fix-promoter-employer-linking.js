...
async function fixPromoterEmployerLinking() {
  ...
  try {
    ...
    if (promotersWithoutEmployer && promotersWithoutEmployer.length > 0) {
      ...
      if (employers && employers.length > 0) {
        ...
        for (const promoter of promotersWithoutEmployer) {
          ...
          console.log(
            `   Current employer_id: ${promoter.employer_id || 'None'}`
          );
          ...
          const { data: contracts, error: contractsError } = await supabase
            .from('contracts')
            .select(
              'second_party_id, second_party:parties!second_party_id(name_en, type)'
            )
          ...
          if (!contractsError && contracts && contracts.length > 0) {
            ...
            if (
              ...
              contract.second_party.type === 'Employer'
            ) {
              suggestedEmployer = contract.second_party;
              console.log(
                `   📄 Found contract with employer: ${suggestedEmployer.name_en}`
              );
            }
          }
          ...
          if (updateError) {
            ...
            console.log(
              `   ✅ Assigned to employer: ${suggestedEmployer.name_en}`
            );
          }
        }
      } else {
    ...
    const { data: contractsWithMismatches, error: mismatchError } =
      await supabase
        .from('contracts')
        .select(
          `
    ...
    if (mismatchError) {
      console.error(
        '❌ Error fetching contracts with mismatches:',
        mismatchError
      );
    } else {
      console.log(
        `📋 Found ${contractsWithMismatches?.length || 0} contracts to check:`
      );
      ...
      if (contractsWithMismatches && contractsWithMismatches.length > 0) {
        for (const contract of contractsWithMismatches) {
          ...
          console.log(
            `   Promoter's employer_id: ${contract.promoter?.employer_id}`
          );
          ...
          if (
            ...
            console.log(
              "   🔧 Fixing: Promoter employer_id doesn't match contract employer"
            );
            ...
            if (updateError) {
              ...
              console.log(
                '   ✅ Updated promoter employer_id to match contract'
              );
            }
          } else {
    ...
    const { data: promoterSummary, error: summaryError } = await supabase.from(
      'promoters'
    ).select(`
        employer_id,
    ...
    console.log(
      '3. Test creating new contracts with proper employer assignments'
    );
  } catch (error) {
