...
async function main() {
  ...
  try {
    ...
    const criticalPermissions = [
      ...
      { resource: 'contract', action: 'delete', scope: 'own', name: 'contract:delete:own' },
      { resource: 'contract', action: 'read', scope: 'all', name: 'contract:read:all' },
      { resource: 'party', action: 'read', scope: 'own', name: 'party:read:own' },
      { resource: 'party', action: 'create', scope: 'own', name: 'party:create:own' },
      ...
      { resource: 'dashboard', action: 'view', scope: 'own', name: 'dashboard:view:own' },
    ];
    ...
    console.log(`✅ Assigned ${assignedCount} critical permissions to admin role\n`);
    ...
    if (tablePrefix === 'rbac') {
      ...
      try {
        ...
        if (refreshError) {
          console.error('   ⚠️ Failed to refresh materialized view:', refreshError.message);
        } else {
