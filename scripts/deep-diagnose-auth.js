  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(
          '   Token length:',
          tokenResponse.access_token?.length || 'N/A'
        );
      } catch (tokenError) {
        console.log(
        ...
        if (tokenError.message.includes('invalid_grant')) {
          console.log(
            '   🔧 SOLUTION: Service account key might be invalid or expired'
          );
        } else if (tokenError.message.includes('unauthorized_client')) {
          console.log(
            '   🔧 SOLUTION: Service account not authorized for these scopes'
          );
        }
      }
      ...
      try {
        ...
        console.log(
          '   User:',
          response.data.user?.displayName || response.data.user?.emailAddress
        );
      } catch (driveError) {
        ...
        if (driveError.message.includes('unregistered callers')) {
          ...
          console.log(
            '   1. APIs might not be fully enabled yet (wait 5-10 minutes)'
          );
          console.log(
            '   2. Service account might need additional permissions'
          );
      ...
      try {
        ...
        // Check permissions
        const hasPermission = templateInfo.data.permissions?.some(
        ...
        if (hasPermission) {
          ...
          console.log(
            '   ❌ Service account does NOT have template permission'
          );
          console.log(
            '   🔧 SOLUTION: Share template with:',
        ...
        if (templateError.message.includes('not found')) {
          console.log(
            '   🔧 SOLUTION: Template ID might be wrong or template deleted'
          );
    ...
    console.log(
      '4. Project billing not enabled → Enable billing in Google Cloud'
    );
