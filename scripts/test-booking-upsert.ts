...
async function testBookingUpsert() {
  ...
  try {
    const schemaResponse = await fetch(
      `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3001'}/api/debug/booking-schema`
    );
    ...
    if (!schemaData.summary?.bookings_table_exists) {
      console.error(
        '❌ Bookings table does not exist. Please run the migration first.'
      );
      return;
    }
    ...
    if (!schemaData.summary?.unique_constraint_exists) {
      console.warn(
        '⚠️ Unique constraint on booking_number may be missing. Upsert may not work correctly.'
      );
    }
  } catch (schemaError) {
  ...
  try {
    ...
    const updatedPayload = {
      ...
      total_price: 30.0, // Changed price
      scheduled_start: new Date(
        Date.now() + 5 * 24 * 60 * 60 * 1000
      ).toISOString(), // Changed date
    };
    ...
    const { data: secondResult, error: secondError } = await supabase
      ...
      .select(
        'id, booking_number, status, total_price, scheduled_start, created_at, updated_at'
      )
      .single();
    ...
    if (newResult.id !== firstResult.id) {
      ...
      console.log(
        '❌ FAIL: Should have created new record with different booking number'
      );
    }
