...
async function fixCorruptedUserUpdate() {
  try {
    console.log('🔧 Fixing corrupted UUID for cleaning.provider@example.com (UPDATE approach)...\n');
    ...
    if (providerServicesError) {
      ...
      console.log(`✅ Found ${providerServices.length} provider_services references`);
    }
    
    // Check other potential reference tables
    const referenceTables = [
      'bookings', 'contracts', 'notifications', 'audit_logs'
    ];
    ...
    for (const tableName of referenceTables) {
      try {
        const { data: references, error: refError } = await supabase
          .from(tableName)
          .select('id')
          .or(`user_id.eq.${oldId},created_by.eq.${oldId},updated_by.eq.${oldId}`)
          .limit(1);
    ...
    if (userRolesError) {
      ...
      console.log(`✅ Found ${existingUserRoles.length} existing user_roles entries`);
      ...
      for (const userRole of existingUserRoles) {
        if (userRole.role !== 'user') {
          ...
          if (updateError) {
            console.log(`⚠️ Could not update user_role ${userRole.id}:`, updateError.message);
          } else {
    ...
    if (verifyProfileError) {
      console.log('❌ Profile verification failed:', verifyProfileError.message);
    } else {
    ...
    if (verifyUserRoleError) {
      console.log('❌ User_roles verification failed:', verifyUserRoleError.message);
    } else {
