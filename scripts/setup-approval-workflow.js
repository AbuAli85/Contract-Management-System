...
async function setupApprovalWorkflow() {
  ...
  try {
    ...
    const { data: contracts, error } = await supabase
      .from('contracts')
      .select(
        'id, contract_number, status, approval_status, created_at, updated_at'
      )
      .or(
    ...
    console.log(
      `📋 Found ${contracts.length} contracts to move to approval workflow:`
    );
    ...
    for (const contract of contracts) {
      ...
      try {
        ...
        if (updateError) {
          ...
          console.log(
            `   ✅ Updated to: approval_status="${newApprovalStatus}"`
          );
          updatedCount++;
        }
      } catch (error) {
