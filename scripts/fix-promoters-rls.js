...
async function fixPromotersRLSPolicies() {
  ...
  try {
    ...
    if (insertError) {
      ...
      console.log(
        '📝 Please run the SQL commands from scripts/fix-promoters-rls-policies.sql in your Supabase SQL Editor.'
      );
    } else {
    ...
    if (policiesError) {
      ...
      console.log(
        `✅ Found ${policies?.length || 0} RLS policies for promoters table:`
      );
    ...
    console.log(
      '2. If you still get RLS errors, run the SQL manually in Supabase SQL Editor'
    );
    console.log(
      '3. Check the Supabase Dashboard > Authentication > Policies for verification'
    );
  } catch (error) {
