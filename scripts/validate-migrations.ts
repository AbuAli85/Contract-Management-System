...
class MigrationValidator {
  ...
  private checkSequentialOrdering(timestamps: string[]): void {
    ...
    for (let i = 1; i < sortedTimestamps.length; i++) {
      ...
      if (current - previous > 1) {
        ...
        this.result.warnings.push(`Gap detected between migrations: ${sortedTimestamps[i - 1]} -> ${sortedTimestamps[i]}`)
      }
    }

    // Check for duplicates
    const duplicates = sortedTimestamps.filter((timestamp, index) => 
      sortedTimestamps.indexOf(timestamp) !== index
    )
  ...
  private validateIdempotency(filePath: string): void {
    try {
      ...
      this.result.errors.push(`Failed to validate idempotency for ${path.basename(filePath)}: ${error}`)
    }
  }
