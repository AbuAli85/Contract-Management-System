...
class MigrationValidator {
    ...
    try {
      ...
      if (!content.includes('-- Migration:')) {
        this.result.errors.push(
          `Missing migration header in ${path.basename(filePath)}`
        );
        return false;
      }
      ...
      if (!content.trim()) {
        this.result.errors.push(
          `Empty migration file: ${path.basename(filePath)}`
        );
        return false;
      }
      ...
      if (content.includes('DROP TABLE') && !content.includes('IF EXISTS')) {
        this.result.warnings.push(
          `Potentially unsafe DROP TABLE in ${path.basename(filePath)}`
        );
      }

      if (content.includes('DELETE FROM') && !content.includes('WHERE')) {
        this.result.warnings.push(
          `Potentially unsafe DELETE FROM in ${path.basename(filePath)}`
        );
      }
      ...
      this.result.errors.push(
        `Failed to read migration file ${path.basename(filePath)}: ${error}`
      );
      return false;
    }
  }
  ...
  private checkSequentialOrdering(timestamps: string[]): void {
    ...
    if (duplicates.length > 0) {
      ...
      this.result.errors.push(
        `Duplicate timestamps found: ${duplicates.join(', ')}`
      );
    ...
    try {
      ...
      if (createTableMatches) {
        createTableMatches.forEach(match => {
          if (!match.includes('IF NOT EXISTS')) {
            this.result.warnings.push(
              `CREATE TABLE without IF NOT EXISTS in ${filename}: ${match}`
            );
          }
        });
      }
      ...
      if (createIndexMatches) {
        createIndexMatches.forEach(match => {
          if (!match.includes('IF NOT EXISTS')) {
            this.result.warnings.push(
              `CREATE INDEX without IF NOT EXISTS in ${filename}: ${match}`
            );
          }
        });
      }
      ...
      const alterTableMatches = content.match(
      ...
      if (alterTableMatches) {
        alterTableMatches.forEach(match => {
          if (!match.includes('IF NOT EXISTS')) {
            this.result.warnings.push(
              `ADD COLUMN without IF NOT EXISTS in ${filename}: ${match}`
            );
          }
        });
      }
  ...
  public validate(): MigrationValidationResult {
    ...
    for (const file of sqlFiles) {
      ...
      if (timestamp) {
        ...
        this.result.errors.push(
          `Could not extract timestamp from filename: ${file}`
        );
        this.result.isValid = false;
        continue;
      }
