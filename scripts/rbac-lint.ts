...
function checkRouteFile(filePath: string): RouteCheck {
  ...
  if (hasWithRBAC) {
    ...
    const isPublicRoute =
      /export\s+(const\s+)?(GET|POST|PUT|DELETE|PATCH)\s*=\s*(public|unauthenticated)/i.test(
        content
      ) ||
  ...
  if (
    ...
    !content.includes('getServerClient')
  ) {
    check.issues.push(
...
async function generateGuardLintReport() {
  console.log('🔍 Checking RBAC guard compliance...\n');
  ...
  const report = `# 🛡️ RBAC Guard Lint Report
...
${
  guarded.length === 0
    ...
    : guarded
        .map(c => `- \`${c.file}\` - \`${c.guardPermission || 'unknown'}\``)
        .join('\n')
}
...
${
  criticalIssues > 0
    ...
    : '✅ All routes are properly protected with RBAC guards.'
}
...
`;
  ...
  console.log(
    `   - Compliance: ${checks.length > 0 ? Math.round((guarded.length / checks.length) * 100) : 0}%\n`
  );
  ...
  if (criticalIssues > 0) {
    ...
    console.log('✅ All routes are properly guarded');
    process.exit(0);
  }
}
