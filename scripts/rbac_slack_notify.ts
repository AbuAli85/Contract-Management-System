...
if (!webhook) {
  console.error('SLACK_WEBHOOK_URL missing'); process.exit(1);
}
...
function summarizeDrift(): string {
  ...
  const p0 = /Used in code but NOT seeded[\s\S]*?^- /m.test(txt) ? '⚠️ check list' :
             /Used in code but NOT seeded:\s*0/.test(txt) ? '0' : '0?';
  return `Drift P0: ${p0}`;
}
...
(async () => {
  ...
  const env = process.env.GITHUB_REF || process.env.VERCEL_GIT_COMMIT_REF || 'local';
