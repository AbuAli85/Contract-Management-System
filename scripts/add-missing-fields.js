...
async function addMissingFields() {
  ...
  try {
    ...
    if (salaryError)
      console.error('❌ basic_salary error:', salaryError.message);
    ...
    if (allowancesError)
      console.error('❌ allowances error:', allowancesError.message);
    ...
    if (termsError)
      console.error('❌ special_terms error:', termsError.message);
    ...
    if (currencyError)
      console.error('❌ currency error:', currencyError.message);
    ...
    const { data: contracts, error: verifyError } = await supabase
      .from('contracts')
      .select(
        'id, basic_salary, allowances, special_terms, department, contract_type, currency'
      )
      .limit(5);
