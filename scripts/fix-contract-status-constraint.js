...
async function fixContractStatusConstraint() {
  ...
  try {
    ...
    if (insertError) {
      ...
      console.log('📝 Please run the SQL commands from scripts/016_add_processing_status.sql in your Supabase SQL Editor.')
    } else {
    ...
    if (constraintsError) {
      ...
      console.log(`✅ Found ${constraints?.length || 0} contract status constraints:`)
    ...
    console.log('3. Check the Supabase Dashboard > Database > Tables > contracts for verification')
