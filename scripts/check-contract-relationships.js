  ...
  try {
    ...
    if (contracts && contracts.length > 0) {
      contracts.forEach((contract, index) => {
        ...
        console.log(
          `   First Party: ${contract.first_party ? contract.first_party.name_en : "NULL"}`,
        )
        console.log(
          `   Second Party: ${contract.second_party ? contract.second_party.name_en : "NULL"}`,
        )
    ...
    if (nullError) {
      ...
      nullRelationships?.forEach((contract) => {
        console.log(
          `   - ${contract.contract_number}: first_party_id=${contract.first_party_id}, second_party_id=${contract.second_party_id}, promoter_id=${contract.promoter_id}`,
        )
      })
    }
    ...
    try {
      const response = await fetch(
        `${supabaseUrl.replace(".supabase.co", ".supabase.co")}/rest/v1/contracts?select=id,contract_number,status,first_party:parties!contracts_first_party_id_fkey(id,name_en),second_party:parties!contracts_second_party_id_fkey(id,name_en),promoter:promoters!contracts_promoter_id_fkey(id,name_en)&limit=3`,
        {
          headers: {
