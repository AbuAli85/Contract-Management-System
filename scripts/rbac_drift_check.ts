...
class RBACDriftChecker {
  ...
  private async parseSeededPermissions(): Promise<void> {
    ...
    try {
      ...
      if (!supabaseUrl || !supabaseServiceKey) {
        console.log(
          '   ⚠️  No database credentials, falling back to seed_rbac.sql file...'
        );
      ...
      if (error) {
        console.log(
          '   ⚠️  Database query failed, falling back to seed_rbac.sql file...'
        );
        await this.parseSeededPermissionsFromFile();
        return;
      }
      ...
      console.log(
        `   ✅ Found ${this.seededPermissions.size} seeded permissions from database`
      );
    } catch (error) {
      console.log(
        '   ⚠️  Database connection failed, falling back to seed_rbac.sql file...'
      );
      await this.parseSeededPermissionsFromFile();
    }
  }
  ...
  private async parseSeededPermissionsFromFile(): Promise<void> {
    try {
      ...
      const permissionRegex =
        /INSERT INTO permissions \(.*?\) VALUES\s*\((.*?)\)/gs;
      let match;
      ...
      console.log(
        `   ✅ Found ${this.seededPermissions.size} seeded permissions from file`
      );
    } catch (error) {
  ...
  private async parseCodeGuards(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `   ✅ Found ${totalGuards} permission guards across ${files.length} files`
      );
    } catch (error) {
  ...
  private async parseDocumentation(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `   ✅ Found ${this.inDocumentation.size} documented permissions`
      );
    } catch (error) {
  ...
  private async generateReport(): Promise<void> {
    ...
    for (const permission of this.inDocumentation) {
      if (
        !this.seededPermissions.has(permission) &&
        !this.usedInCode.has(permission)
      ) {
    ...
    console.log(
      `   🔴 P0 Critical: ${report.summary.p0Count} permissions used in code but NOT seeded`
    );
    console.log(
      `   🟡 P2 Low: ${report.summary.p2Count} permissions seeded but NOT used in code`
    );
    console.log(
      `   🟡 P2 Doc: ${report.summary.p2DocCount} permissions documented but not implemented`
    );
    ...
    if (report.summary.p0Count > 0) {
      ...
      console.log(
        '   This will cause runtime errors. Please fix before production deployment.'
      );
      process.exit(1);
    } else {
      console.log(
...
${
  report.p2Low.length === 0
    ? '✅ All seeded permissions are used'
    : report.p2Low.map(p => `- \`${p}\``).join('\n')
}

### Documented but NOT Implemented
${
  report.p2Documentation.length === 0
    ? '✅ All documented permissions are implemented'
...
### Seeded Permissions
${Array.from(this.seededPermissions)
  .sort()
  .map(p => `- \`${p}\``)
  .join('\n')}

### Used in Code
${Array.from(this.usedInCode)
  .sort()
  .map(p => `- \`${p}\``)
  .join('\n')}

### In Documentation
${Array.from(this.inDocumentation)
  .sort()
  .map(p => `- \`${p}\``)
  .join('\n')}
