...
async function testUpsertScenario(scenario: {
  name: string;
...
async function demonstrateConflictResolution() {
  ...
  companies?.forEach((company, index) => {
    ...
    console.log(
      `   Times Updated: ${new Date(company.updated_at) > new Date(company.created_at) ? 'YES' : 'NO'}`
    );
  });
}
...
async function main() {
  ...
  try {
    ...
    if (args.includes('--auto-cleanup')) {
      ...
      console.log(
        '💡 Run with --auto-cleanup to run tests and cleanup automatically'
      );
    }
  } catch (error) {
