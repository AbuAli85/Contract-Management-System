// Final import script for promoters with proper constraint handling
...
async function importPromotersFinal() {
  console.log('=== Final Import: Promoters with Proper Constraints ===');
  ...
  try {
    ...
    // Check for CSV file
    const csvFilePath = './promoters-export-2025-08-01.csv';
    ...
    console.log('✅ Found CSV file: promoters-export-2025-08-01.csv');
    ...
    fs.createReadStream(csvFilePath)
      ...
      .on('data', (row) => {
        ...
        // Handle gender constraint - only allow valid values
        const parseGender = (gender) => {
          if (!gender || gender === 'N/A') return null;
          const validGenders = ['male', 'female', 'other'];
          const lowerGender = gender.toLowerCase().trim();
          return validGenders.includes(lowerGender) ? lowerGender : null;
        };

        // Map to actual database schema with proper constraint handling
        const promoter = {
          name_en: row['Name (EN)'] || '',
          name_ar: row['Name (AR)'] || '',
