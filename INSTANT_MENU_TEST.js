...
if (moreButton) {
  ...
  setTimeout(() => {
    ...
    const menuContent = document.querySelector(
      'div[class*="absolute"][class*="z-50"]'
    );

    if (
      menuContent &&
      window.getComputedStyle(menuContent).display !== 'none'
    ) {
      console.log('✅ Menu is visible');
      results.menuOpened = true;

      // Count clickable items
      const clickableItems = menuContent.querySelectorAll(
      ...
      if (viewProfileItem) {
        ...
        const toastObserver = new MutationObserver(() => {
          ...
          if (toasts.length > 0) {
            ...
            console.log(
              `   Content: "${toasts[0].textContent.substring(0, 60)}..."`
            );
